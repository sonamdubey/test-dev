...
namespace Bikewale.Notifications.MailTemplates
{
    ...
    public class UsedBikesModelImagesMailTemplate : ComposeEmailBase
    {
        ...
        public UsedBikesModelImagesMailTemplate()
        {
            try
            {
                StringBuilder message = new StringBuilder();
                message.Append("<h4>Dear User,</h4>");
                message.Append("<p>There are bikes for which Used model image needs to be uploaded.</p> <p>Please update the bikes shown on panel.</p> ");
                message.AppendFormat("<p><a href=\"{0}/Models/UsedModelImageUpload/\">Click here to update the data</a></p>", Bikewale.Utility.BWOprConfiguration.Instance.BwOprHostUrlForJs);
