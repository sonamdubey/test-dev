	...
	{	
											...
											int outputHeight, int imagePosition) 
		{
			...
			switch(imagePosition)
			{
				...
				case 4:
					...
					Bitmap watermark4 = (Bitmap)Bitmap.FromFile( HttpContext.Current.Server.MapPath("/common/bw_watermark.png") );
		...
		public static string GetImagePath(string relativePath)
		{
			...
			if ( HttpContext.Current.Request.ServerVariables["HTTP_HOST"].IndexOf( "carwale.com" ) >= 0 )
				absolutePath = "https://img.carwale.com" + relativePath;
			else
        ...
        ///     Summary : Function will return the physical path by passing for the given absolute path
        /// </summary>
        /// <param name="RelativePath"></param>
        /// <returns></returns>
        public static string ImagePathForSavingImages(string RelativePath)
        {
            string savepath = HttpContext.Current.Server.MapPath(RelativePath).ToLower();

            if (savepath.IndexOf("bikewaleopr") >= 0)
            {
                savepath = savepath.Replace("bikewaleopr", "carwaleimg");
            }
