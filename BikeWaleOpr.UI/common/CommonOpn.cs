...
namespace BikeWaleOpr.Common
{
    public class CommonOpn
    {
        ...
        *****************************************************************************************************/
        ...
        // for given city and new bike version-id.
        /// <summary>
        ...
        public static double GetInsurancePremium(string bikeVersionId, string cityId, double price)
        {
            ...
            if (price > 0 && cc > 0)
            {
                try
                {
                    ...
                    string dataRateKey = "";//used to fetch corresponding data from database
                    double idvBike = 0;
                    ...
                    HttpContext.Current.Trace.Warn("rate : " + rate.ToString());

                    rate = rate * (1 - discount / 100.0);	//deduct the discount

                    HttpContext.Current.Trace.Warn("rate after discount: " + rate.ToString());
                    // get depreciation % from web.config.
                    //depreciation = 100 - double.Parse(ConfigurationSettings.AppSettings[depKey]);
                    depreciation = 100 - double.Parse(ConfigurationManager.AppSettings[depKey]);
                    HttpContext.Current.Trace.Warn("Depreciation : " + depreciation);

                    // calculate IDV 					
                    idvBike = price * depreciation / 100.0;
                    HttpContext.Current.Trace.Warn("idvBike : " + idvBike);

                    // calculate od.
                    premium = idvBike * rate / 100.0;
                    HttpContext.Current.Trace.Warn("premium : " + premium);

                    // Third-party Liability.					
                    //liaPremium = double.Parse(ConfigurationSettings.AppSettings["L:" + cc]);
                    ...
                    // Get the Premium.
                    premium += liabilities;
                    HttpContext.Current.Trace.Warn("premium after lliab: " + premium);

                    // Add service tax.
                    //premium += premium * double.Parse(ConfigurationSettings.AppSettings["ServiceTax"]) / 100.0;
                    premium += premium * double.Parse(ConfigurationManager.AppSettings["ServiceTax"]) / 100.0;
                    HttpContext.Current.Trace.Warn("premium ST : " + premium);

                    // round premium
                    premium = Math.Round(premium, 2);
                    HttpContext.Current.Trace.Warn("Final premium : " + premium);

                    //HttpContext.Current.Trace.Warn( "Premium Calculation Ends Here..." );
                }
                catch (Exception err)
                {
                    HttpContext.Current.Trace.Warn(err.Message);
        ...
        // This function will calculate registration charges
        ...
        public static double GetRegistrationCharges(string bikeVersionId, string cityId, double price)
        {
            ...
            switch (stateId)
            {
                ...
                */
                ...
                case 9:
                    //if (!isImported) roadTax = price * 0.052174;
                    //else roadTax = price * 0.10434;
                    roadTax = price * 0.06;
                    break;
