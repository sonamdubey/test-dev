...
namespace Bikewale.BAL.Compare
{
    ...
    public class BikeComparison : IBikeCompare
    {
        ...
        /// Description: Overload method for TransposeCompareBikeData with new BikeCompareEntity
        /// </summary>
        /// <param name="compareEntity"></param>
        /// <param name="versions"></param>
        private static void TransposeCompareBikeData(ref Entities.Compare.V2.BikeCompareEntity compareEntity, string versions)
        {
            ...
            try
            {
                ...
                if (compareEntity != null)
                {
                    ...
                    compareEntity.CompareReviews = new CompareMainCategory();
                    compareEntity.CompareReviews.Text = "Reviews";
                    compareEntity.CompareReviews.Value = "Reviews";
                    compareEntity.CompareReviews.Spec = new List<CompareSubMainCategory>();
                    ...
                    //Overall Rating
                    CompareSubCategory overallRating = new CompareSubCategory();
                    overallRating.Text = "Overall rating";
                    overallRating.Value = "Overall rating";
                    overallRating.CompareSpec = new List<CompareBikeData>();
                    foreach (var version in arrVersion)
                    {
                        overallRating.CompareSpec.Add(new CompareBikeData() { Value = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewRate.ToString()), Text = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewRate.ToString()) });
                    }
                    ratings.SpecCategory.Add(overallRating);
                    ...
                    foreach (var version in arrVersion)
                    {
                        mileage.CompareSpec.Add(new CompareBikeData() { Value = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.Mileage , "kmpl"), Text = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.Mileage , "kmpl")});
                    }
                    ratings.SpecCategory.Add(overallRating);

                    //Ratings Count
                    CompareSubCategory ratingsCount = new CompareSubCategory();
                    ratingsCount.Text = "Overall Rating Count";
                    ratingsCount.Value = "Overall Rating Count";
                    ratingsCount.CompareSpec = new List<CompareBikeData>();
                    foreach (var version in arrVersion)
                    {
                        ratingsCount.CompareSpec.Add(new CompareBikeData() { Value = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.RatingCount.ToString()), Text = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.RatingCount.ToString()) });
                    }
                    ratings.SpecCategory.Add(ratingsCount);

                    //Reviews Count
                    CompareSubCategory reviewsCount = new CompareSubCategory();
                    reviewsCount.Text = "Overall Reviews Count";
                    reviewsCount.Value = "Overall Reviews Count";
                    reviewsCount.CompareSpec = new List<CompareBikeData>();
                    foreach (var version in arrVersion)
                    {
                        reviewsCount.CompareSpec.Add(new CompareBikeData() { Value = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewCount.ToString()), Text = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewCount.ToString()) });
                    }
                    ratings.SpecCategory.Add(reviewsCount);
                    #endregion
                    compareEntity.CompareReviews.Spec.Add(ratings);
                    ...
                    #endregion
                    compareEntity.CompareReviews.Spec.Add(performanceParameters);
                    ...
                    //Overall Rating
                    CompareSubCategory mostHelpful = new CompareSubCategory();
                    mostHelpful.Text = "Most Helpful";
                    mostHelpful.Value = "Most Helpful";
                    mostHelpful.CompareSpec = new List<CompareBikeData>();
                    foreach (var version in arrVersion)
                    {
                        mostHelpful.CompareSpec.Add(new CompareBikeData() { Value = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewRate.ToString()), Text = Bikewale.Utility.FormatMinSpecs.ShowAvailable(compareEntity.Reviews.FirstOrDefault(m => m.VersionId == Convert.ToUInt32(version)).ModelReview.ReviewRate.ToString()) });
                    }
                    reviews.SpecCategory.Add(mostHelpful);
        ...
        {
            Entities.Compare.V2.BikeCompareEntity compareEntity = null;
            try
            {
                if (!string.IsNullOrEmpty(versions))
                {
