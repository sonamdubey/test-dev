...
namespace Bikewale.BAL.NewBikeSearch
{
    ...
    public class SearchQuery : ISearchQuery
    {
        ...
        public SearchQuery(IUnityContainer _container,InputBaseEntity _input)
        {
            container = _container;
            container.RegisterType<IProcessFilter, ProcessFilter>();

            processFilter = container.Resolve<IProcessFilter>();

            filterInputs = processFilter.ProcessFilters(_input);
        ...
        public string GetSelectClause()
        {
            ...
            try
            {
                selectClause = @" MA.NAME + ' ' + MO.NAME AS BikeName
		                        ...
		                        ,SD.Displacement
		                        ,SD.FuelType
		                        ,SD.MaxPower AS Power
		                        ,SD.FuelEfficiencyOverall
		                        ,SD.KerbWeight AS [Weight]
        ...
        public string GetRecordCountQry()
        {
            ...
            try
            {
                recordCountQuery = " Select Count(ModelId) FROM ( SELECT SELECT DENSE_RANK() OVER (ORDER BY MO.MinPrice) AS DenseRank "
        ...
        private void MileageClause()
        {
            ...
            try
            {
                ...
                {
                    if (Validate.ValidRange(Convert.ToInt32(filterInputs.Mileage), 1, 4))
                    {
        ...
        private void DisplacementClause()
        {
            ...
            try
            {
                for (int i = 0; i < filterInputs.Mileage.Length; i++)
                {
                    if (Validate.ValidRange(Convert.ToInt32(filterInputs.Mileage), 1, 6))
                    {
                        string displacementClause = GetDisplacementClause(filterInputs.Mileage[i]);
        ...
        public string GetSearchResultQuery()
        {
            ...
            try
            {
                searchResultQuery = " WITH CTE_BikeModels AS ( SELECT DENSE_RANK() OVER (ORDER BY MO.MinPrice) AS DenseRank, " + GetSelectClause()
