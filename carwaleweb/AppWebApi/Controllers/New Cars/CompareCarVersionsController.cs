...
namespace AppWebApi.Controllers.New_Cars
{
    public class CompareCarVersionsController : ApiController
    {
        public HttpResponseMessage Get(HttpRequestMessage request, string modelId)
        {
            if (CommonOpn.CheckId(modelId))
            {
                ...
                {
                    CompareCarVersions pva = new CompareCarVersions(modelId);
                    if (!pva.ServerErrorOccured)
                        return Request.CreateResponse<CompareCarVersions>(HttpStatusCode.OK, pva);
                    else
