...
{
    public class TyresAdapterDesktopSearch : IServiceAdapterV2
    {
        ...
        public TyresAdapterDesktopSearch(TyresBL tyres, ICarVersionCacheRepository carVersionCache, ICarModelCacheRepository carModelCache)
        {
            _tyres = tyres;
        ...
        {
            TyresDTO tyreDtoDesktop = new TyresDTO();
            tyreDtoDesktop.CarVersions = new List<CarModelsVersionsList>();
            try
            {
                ...
                foreach (var strModelId in modelIds)
                {
                    ...
                    if (modelBasicInfo.MakeId > 0)
                    {
                        ...
                        tyreDtoDesktop.CarVersions.Add(modelData);
                    }
                }            
            }
            catch (Exception ex)
