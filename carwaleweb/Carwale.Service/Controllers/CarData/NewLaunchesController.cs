...
{
    public class NewLaunchesController : ApiController
    {
        private readonly ICarModels _carModels;
        public NewLaunchesController(ICarModels carModels)
        {
        ...
        {
            NewCarLaunches newCarLaunches = new NewCarLaunches()
            {          
                NewLaunches = _carModels.GetLaunchedCarModelsV1(pagination, cityId)
            };

            if (newCarLaunches.NewLaunches.Count == 0)
                return NotFound();

            return ResponseMessage(new HttpResponseMessage() { Content = new StringContent(JsonConvert.SerializeObject(newCarLaunches)) });
        }
    }
}
