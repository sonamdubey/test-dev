<%@ Page Language="C#" AutoEventWireup="false" Inherits="Carwale.UI.Editorial.OtherModelsGallery" Trace="false" Debug="false" EnableViewState="false" EnableSessionState=False%>
...
<div id="omAlertMsg" runat="server"><img src="https://img.carwale.com/adgallery/alert.gif" /></div>
<Vspl:RepeaterPager id="rpgListings" ResultName="Cars" ShowHeadersVisible="false" PagerPosition="TopBottom" runat="server">
	...
	<itemtemplate>        
        <div class="img-placer">
			<a href="/<%#UrlRewrite.FormatSpecial(DataBinder.Eval(Container.DataItem, "MakeBase.MakeName").ToString()) %>-cars/<%# DataBinder.Eval(Container.DataItem, "ModelBase.MaskingName").ToString() %>/images/">			
            ...
            <div class="imgBox"><img alt='<%#DataBinder.Eval(Container.DataItem, "MakeBase.MakeName").ToString() + " " + DataBinder.Eval(Container.DataItem, "ModelBase.ModelName").ToString() %>' src='<%# ImageSizes.CreateImageUrl(Convert.ToString(DataBinder.Eval(Container.DataItem,"HostUrl")),ImageSizes._160X89,Convert.ToString(DataBinder.Eval(Container.DataItem,"OriginalImgPath")))%>' width="160px" height="100px"/></div>
			</a>
		</div>
