...
class OnVisible extends Component {
    ...
    onScroll() {
        const posTop = window.pageYOffset + window.innerHeight;
        const posLeft = window.pageXOffset + window.innerWidth;
        ...
        const left = box.left + (box.width * visbleTriggerRatio) + (pageXOffset - docLeft);
        const visible = top < posTop && left < posLeft;
        const end = () => {
