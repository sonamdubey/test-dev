 ...
 */
workbox.routing.registerRoute(/.*(?:localhost).*\.(?:png|gif|jpg|jpeg).*$/, workbox.strategies.cacheFirst({
...
workbox.routing.registerRoute(/.*(?:localhost).*\.(?:svg).*$/, workbox.strategies.cacheFirst({
...
workbox.routing.registerRoute(
  /.*(?:localhost).*(?:api).*([^.*api/newcarfinder/screen])/,
  workbox.strategies.cacheFirst({
    cacheName: 'cw-apis',
    plugins: [
      new workbox.expiration.Plugin({
