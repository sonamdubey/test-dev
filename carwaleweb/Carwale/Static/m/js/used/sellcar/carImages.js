var carImages = (function () {
    var container, dropzoneTemplate, otherImages, carImagesData = {};
	...
	function setImageScreen() {
	    if (!modalPopup.closeActiveModalPopup()) {
	        ...
	        parentContainer.setButtonTarget('history.back()', 'carImages.submitImages()');
	    }
	};
	...
	function submitImages() {
		...
		$.ajax(settings).done(function (response, msg, xhr) {		    
		    ...
		    $.ajax(settings).done(function (response) {
		        historyObj.addToHistory('accidentalQuestion');
		        if (!($('.partialCarCondition').html().trim().length)) {
		            partialCarCondition.html(response);
		        }
		        container.hide();
		        if (typeof events !== 'undefined') {
		            var eventObj = {
		        ...
		        sellCarTracking.forMobile("images", ($(".dz-complete").length).toString());
		    }).fail(function (xhr) {
		        if (xhr.status === 404) {
		            var settings = {
		                url: "/used/sell/plans/?isPartial=true",
		                type: "GET"
		            }
		            $.ajax(settings).done(function (response) {
		                if (typeof events !== 'undefined') {
		                    events.publish('showPlans', { container: container, response: response });
		                }
		            }).fail(function (xhr) {
		                parentContainer.removeLoadingScreen();
