...
window.onload = function () {
    ...
    var default_options = {
        ...
        labels: {
            ...
            'empty.messages': 'No Chat History Available',
            ...
            'location.share.title': 'Location Share',
    ...
    function Applozic(appOptions) {
        ...
        var MCK_UNREAD_COUNT_CALLBACK = appOptions.processUnreadCount; //carwale
        var MCK_HIDE_CHAT_LOADER = appOptions.hideLoader; //carwale
        ...
        var events = {
            'onConnectFailed': function onConnectFailed(resp) { },
            'onConnect': function onConnect(resp) { },
            'onMessageDelivered': function onMessageDelivered(resp) { },
            'onMessageRead': function onMessageRead(resp) { },
            'onMessageDeleted': function onMessageDeleted(resp) { },
            'onConversationDeleted': function onConversationDeleted(resp) { },
            'onUserConnect': function onUserConnect(resp) { },
            'onUserDisconnect': function onUserDisconnect(resp) { },
            'onConversationReadFromOtherSource': function onConversationReadFromOtherSource(resp) { },
            'onConversationRead': function onConversationRead(resp) { },
            'onMessageReceived': function onMessageReceived(resp) {
                console.log(resp);
            },
            'onMessageSentUpdate': function onMessageSentUpdate(resp) { },
            'onMessageSent': function onMessageSent(resp) { },
            'onUserBlocked': function onUserBlocked(resp) { },
            'onUserUnblocked': function onUserUnblocked(resp) { },
            'onUserActivated': function onUserActivated(resp) { },
            'onUserDeactivated': function onUserDeactivated(resp) { },
            'onTypingStatus': function onTypingStatus(resp) {
            ...
            'onOpenGroupMessage': function onOpenGroupMessage(obj) {
                if (openGroupSubscriber.indexOf(obj.headers.subscription) !== -1) {
                    ...
                    if (messageType === "APPLOZIC_03") {
                        ...
                        if (message.conversationId) {
                            ...
                            if ((IS_MCK_TOPIC_HEADER || IS_MCK_TOPIC_BOX) && ((typeof conversationPxy === 'undefined' ? 'undefined' : _typeof(conversationPxy)) !== 'object' || _typeof(MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId]) !== 'object')) {
            ...
            'connectToSocket': function connectToSocket(isFetchMessages) {
            ...
            'onMessage': function onMessage(resp) {
                ...
                if (messageType === "APPLOZIC_04" || messageType === "MESSAGE_DELIVERED") {
                    ...
                    if (typeof tabId === "undefined" || tabId === '') {
                        var  unreadCountText = document.querySelector("#li-user-" + contact.htmlId + " .mck-unread-count-text");
                        if(unreadCountText){
                            unreadCountText.innerHTML = mckMessageLayout.getUnreadCount('user_' + contact.contactId);
        ...
        _this.loadConvTab = function (optns) {
            if ((typeof optns === 'undefined' ? 'undefined' : _typeof(optns)) === 'object' && optns.userId && optns.convId) {
                mckMessageLayout.loadTab({
                    tabId: optns.userId,
        ...
        _this.loadTabWithTopic = function (optns) {
            if ((typeof optns === 'undefined' ? 'undefined' : _typeof(optns)) === 'object' && (optns.userId || optns.groupId) && optns.topicId) {
                var params = {
                    'tabId': optns.userId,
                ...
                if (typeof MCK_GETTOPICDETAIL === 'function' || optns.topicDetail) {
                    ...
                    if ((typeof topicDetail === 'undefined' ? 'undefined' : _typeof(topicDetail)) === 'object' && topicDetail.title !== 'undefined') {
        ...
        _this.reset = function (optns) {
            ...
            MCK_UNREAD_COUNT_CALLBACK = optns.processUnreadCount; //carwale
            MCK_HIDE_CHAT_LOADER = optns.hideLoader; // carwale
        ...
        _this.getMessageListByTopicId = function (params) {
            ...
            if (typeof params.callback === 'function') {
                ...
                if (conversationId && _typeof(MCK_CONVERSATION_MAP[conversationId]) === 'object') {
        ...
        function MckInit() {
            ...
            _this.onLoginSuccess = function (result, userPxy) {
                ...
                if ((typeof result === 'undefined' ? 'undefined' : _typeof(result)) === 'object' && result !== null && result.token) {
            ...
            _this.onInitApp = function (data) {
                ...
                if ($mckChatLauncherIcon.length > 0 && MCK_TOTAL_UNREAD_COUNT > 0) {
                    ...
                    $applozic('.chat-icon-lg').addClass('global-chat'); //carwale
                }
            ...
            _this.tabFocused = function () {
                var hidden = 'hidden';
                // Standards:
                if (hidden in d) d.addEventListener("visibilitychange", onchange); else if ((hidden === "mozHidden") in d) d.addEventListener("mozvisibilitychange", onchange); else if ((hidden === "webkitHidden") in d) d.addEventListener("webkitvisibilitychange", onchange); else if ((hidden === "msHidden") in d) d.addEventListener("msvisibilitychange", onchange);
                    // IE 9 and lower:
                ...
                // Visibility API)
                if (d[hidden] !== undefined) onchange({
            ...
            _this.manageOfflineMessageTime = function (userId) {
                if ((typeof MCK_OFFLINE_MESSAGE_DETAIL === 'undefined' ? 'undefined' : _typeof(MCK_OFFLINE_MESSAGE_DETAIL)) === 'object' && !isNaN(MCK_OFFLINE_MESSAGE_DETAIL.timeout)) {
        ...
        function MckMessageService() {
            ...
            _this.init = function () {
                    ...
                    if (e.keyCode === 13 && (e.shiftKey || e.ctrlKey)) {
                        ...
                        if (w.getSelection) {
                            var selection = w.getSelection(),
                                ...
                                textNode = d.createTextNode('\xA0'); // Passing
                ...
                $applozic(d).on('click', '.mck-btn-clear-messages', function (e) {
                ...
                $applozic(d).on("click", ".applozic-ct-launcher", function (e) {
                    ...
                    if (typeof MCK_GETCONVERSATIONDETAIL === "function") {
                        ...
                        if ((typeof conversationDetail === 'undefined' ? 'undefined' : _typeof(conversationDetail)) === 'object') {
                            if (conversationDetail.topicDetail && _typeof(conversationDetail.topicDetail) === 'object' && conversationDetail.topicDetail.title !== 'undefined') {
                ...
                $applozic(d).on("click", ".mck-close-sidebox", function (e) {
                    e.preventDefault();
                    $mck_sidebox.removeClass('in'); //carwale
                    setTimeout(function () {
                        //carwale
                        $mck_sidebox.mckModal('hide'); //carwale
                    }, 600); //carwale
                    ...
                    if (conversationId) {
                        ...
                        if ((typeof conversationPxy === 'undefined' ? 'undefined' : _typeof(conversationPxy)) === 'object') {
                ...
                $applozic(d).on('click', '.mck-btn-remove-member', function (e) {
                    ...
                    if (typeof groupId !== 'undefined' && typeof userId !== 'undefined') {
                        ...
                        if ((typeof group === 'undefined' ? 'undefined' : _typeof(group)) === 'object' && MCK_USER_ID === group.adminName) {
                ...
                $mck_msg_form.submit(function () {
                    ...
                    if (conversationId) {
                        ...
                        if ((typeof topicDetail === 'undefined' ? 'undefined' : _typeof(topicDetail)) === "object") {
            ...
            _this.sendMessage = function (messagePxy) {
                if ((typeof messagePxy === 'undefined' ? 'undefined' : _typeof(messagePxy)) !== 'object') {
                    return;
                }
                if (messagePxy.to) {
            ...
            _this.sendWelcomeMessage = function (params) {
                ...
                window.Applozic.ALApiService.addMessageInbox({
                    data: { sender: "sender", messageContent: "Welcome" }, success: function success(result) {
                        if (params.callback) {
            ...
            _this.replyMessage = function (msgKey) {
                ...
                if (_typeof(message.fileMeta) === "object" || message.contentType === 2) {
            ...
            _this.loadMessageList = function (params, callback) {
                ...
                window.Applozic.ALApiService.getMessages({
                    ...
                    success: function success(response) {
                        ...
                        var hasNoMessage = data && data.userDetails && data.userDetails.length == 1 && data.message && data.message.length == 0; // carwale
                        CONTACT_SYNCING = false;
                            ...
                            if (data + '' === 'null' || typeof data.message === 'undefined' || data.message.length === 0) {
                                ...
                                if (individual) {
                                    if (params.startTime) {
                                        ...
                                        $mck_no_messages.removeClass('n-vis').addClass('vis');
                                        var msgs = data.message && data.message.length > 0 ? data.message.length : 0; //carwale
                                        if (typeof MCK_ON_TAB_CLICKED === 'function' && hasNoMessage) {
                                            //carwale
                                            //carwale
                                            MCK_ON_TAB_CLICKED({ //carwale
                                                tabId: params.tabId, //carwale
                                                isGroup: params.isGroup, //carwale
                                                msgs: msgs, //carwale
                            ...
                            if (data + '' !== 'null' && data.status !== 'error') {
                                if (individual) {
                                    ...
                                    if (params.conversationId) {
                                        ...
                                        if ((typeof conversationPxy === 'undefined' ? 'undefined' : _typeof(conversationPxy)) === 'object' && conversationPxy.closed) {
                                            mckMessageLayout.closeConversation();
                                        }
                                    }
                                    if (!params.startTime) {
            ...
            _this.conversationReadUpdate = function (tabId, isGroup) {
                ...
                if (tabId && mckMessageLayout.getUnreadCount(ucTabId) > 0) {
                    ...
                    window.Applozic.ALApiService.conversationReadUpdate({
                        data: data, success: function success(response) {
            ...
            _this.getConversationId = function (params) {
                ...
                if (params.topicId) {
                    ...
                    window.Applozic.ALApiService.getConversationId({
                        ...
                        success: function success(data) {
                            if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' && data.status === "success") {
                                var groupPxy = data.response;
                                if ((typeof groupPxy === 'undefined' ? 'undefined' : _typeof(groupPxy)) === 'object' && groupPxy.conversationPxy !== 'undefined') {
            ...
            _this.getGroup = function (params) {
                ...
                window.Applozic.ALApiService.createGroup({
                    ...
                    success: function success(data) {
                        ...
                        if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' && data.status === 'success') {
                            var groupPxy = data.response;
                            if ((typeof groupPxy === 'undefined' ? 'undefined' : _typeof(groupPxy)) === 'object') {
                                var group = mckGroupUtils.addGroup(groupPxy);
        ...
        function MckMessageLayout() {
            ...
            var markup = '<div name="message" data-msgdelivered="${msgDeliveredExpr}" data-msgsent="${msgSentExpr}" data-msgtype="${msgTypeExpr}" data-msgtime="${msgCreatedAtTime}" data-msgcontent="${replyIdExpr}" data-msgkey="${msgKeyExpr}" data-contact="${toExpr}" id="${msgKeyExpr}" class="mck-m-b ${msgKeyExpr} ${msgFloatExpr} ${msgAvatorClassExpr}">' + '<div class="mck-clear">' + '<div class="blk-lg-12">' + '<div class="mck-msg-avator blk-lg-3">{{html msgImgExpr}}</div>' + '<div class="mck-msg-box ${msgClassExpr}">' + '<div class= "move-right mck-msg-text"></div>' + '<div class ="mck-msg-reply mck-verticalLine ${msgReplyToVisibleExpr}">' + '<div class="mck-msgto">${msgReplyTo} </div>' + '</div>' + '<div class ="mck-msg-reply mck-verticalLine ${msgReplyDivExpr}">' + '<div class="mck-msgreply-border ${textreplyVisExpr}">${msgReply}</div>' + '<div class="mck-msgreply-border ${msgpreviewvisExpr}">{{html msgPreview}}</div>' + '</div>' + '<div class="${nameTextExpr} ${showNameExpr}">${msgNameExpr}</div>' + '<div class="mck-file-text notranslate mck-attachment downloadimage ${downloadIconVisibleExpr}" data-filemetakey="${fileMetaKeyExpr}" data-filename="${fileNameExpr}" data-fileurl= "${fileUrlExpr}" data-filesize="${fileSizeExpr}"><div>{{html fileExpr}}</div> {{html downloadMediaUrlExpr}}</div>' + '<div class="mck-msg-text mck-msg-content"></div>' + '</div>' + '</div>' + '<div class="${msgFloatExpr}-muted mck-text-light mck-text-muted mck-text-xs mck-t-xs">${createdAtTimeExpr} <span class="${statusIconExpr} mck-message-status"></span></div>' + '</div>' + '<div class="n-vis mck-context-menu">' + '<ul>' + '<li><a class="mck-message-delete">${msgDeleteExpr}</a></li>' + '</ul>' + '</div>' + '</div>'; //carwale - removed few options
            var contactbox = '<li id="li-${contHtmlExpr}" class="${contIdExpr}" data-msg-time="${msgCreatedAtTimeExpr}">' + '<a class="${mckLauncherExpr}" href="#" target="_self" data-mck-conversationid="${conversationExpr}" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}">' + '<div class="mck-row" title="${contNameExpr}">' + '<div class="mck-conversation-topic mck-truncate ${contHeaderExpr}">${titleExpr}</div>' + '<div class="blk-lg-3">{{html contImgExpr}}</div>' + '<div class="blk-lg-9">' + '<div class="mck-row">' + '<div class="blk-lg-8 mck-cont-name mck-truncate"><div class="mck-ol-status ${contOlExpr}"><span class="mck-ol-icon" title="${onlineLabel}"></span>&nbsp;</div><strong>${contNameExpr}</strong></div>' + '<div class="mck-text-muted move-right mck-cont-msg-date mck-truncate blk-lg-4">${msgCreatedDateExpr}</div></div>' + '<div class="mck-row">' + '<div class="mck-cont-msg-wrapper blk-lg-6 mck-truncate msgTextExpr"></div>' + '<div class="mck-unread-count-box move-right mck-truncate ${contUnreadExpr}"><span class="mck-unread-count-text">{{html contUnreadCount}}</span></div>' + '</div></div></div></a></li>';
            //var convbox = '<li id="li-${convIdExpr}" class="${convIdExpr}">' + '<a class="${mckLauncherExpr}" href="#" data-mck-conversationid="${convIdExpr}" data-mck-id="${tabIdExpr}" data-isgroup="${isGroupExpr}" data-mck-topicid="${topicIdExpr}" data-isconvtab="true">' + '<div class="mck-row mck-truncate" title="${convTitleExpr}">${convTitleExpr}</div>' + '</a></li>';                        //carwale
            //var convbox = '<li id="li-${convIdExpr}" class="${convIdExpr} mck-product-box"> ' + ' <a class="${mckLauncherExpr} mck-padding0" href="javascript:void(0)" data-mck-conversationid="${convIdExpr}" data-mck-id="${tabIdExpr}" data-isgroup="${isGroupExpr}" data-mck-topicid="${topicIdExpr}" data-isconvtab="true"> ' + '<div class="blk-lg-10"><div class="mck-row"><div class="blk-lg-3 mck-product-icon"><img src="https://imgd.aeplcdn.com/370x208/cw/ucp/stockApiImg/OOEO2U5_1117297_1_8318945.jpg?q=85"></div><div class="blk-lg-9"><div class="mck-row"><div class="blk-lg-8 mck-product-title mck-truncate"><div class="mck-row mck-truncate" title="${convTitleExpr}">${convTitleExpr}</div></div><div class="move-right mck-product-rt-up mck-truncate blk-lg-4"><strong class="mck-product-key"></strong><span class="mck-product-value"></span></div></div><div class="mck-row"><div class="blk-lg-8 mck-truncate mck-product-subtitle">3.15L | 2011 | Petrol</div><div class="move-right mck-product-rt-down mck-truncate blk-lg-4"><strong class="mck-product-key"></strong><span class="mck-product-value"></span></div></div></div></div></div> ' + ' </a></li> '; var searchContactbox = '<li id="li-${contHtmlExpr}" class="${contIdExpr}"><a class="applozic-launcher" href="javascript:void(0)" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}"><div class="mck-row" title="${contNameExpr}">' + '<div class="blk-lg-3">{{html contImgExpr}}</div>' + '<div class="blk-lg-9"><div class="mck-row"><div class="blk-lg-12 mck-cont-name mck-truncate"><strong>${contNameExpr}</strong>' + '<div class="move-right mck-group-count-box mck-group-count-text ${displayGroupUserCountExpr}">${groupUserCountExpr}</div></div>' + '<div class="blk-lg-12 mck-text-muted">${contLastSeenExpr}</div></div></div></div></a></li>';      //carwale
            var convbox = '<li id="li-${convIdExpr}" class="${convIdExpr} mck-product-box"> ' + ' <a class="${mckLauncherExpr} mck-padding0" href="javascript:void(0)" data-mck-conversationid="${convIdExpr}" data-mck-id="${tabIdExpr}" data-isgroup="${isGroupExpr}" data-mck-topicid="${topicIdExpr}" data-isconvtab="true"> ' + '<div class="blk-lg-10"><div class="mck-row"><div class="blk-lg-3 mck-product-icon"><img src="${convImageExpr}"></div><div class="blk-lg-9"><div class="mck-row"><div class="blk-lg-8 mck-product-title mck-truncate"><div class="mck-row mck-truncate" title="${convTitleExpr}">${convTitleExpr}</div></div><div class="move-right mck-product-rt-up mck-truncate blk-lg-4"><strong class="mck-product-key"></strong><span class="mck-product-value"></span></div></div><div class="mck-row"><div class="blk-lg-8 mck-truncate mck-product-subtitle">${convSubTitleExpr}</div><div class="move-right mck-product-rt-down mck-truncate blk-lg-4"><strong class="mck-product-key"></strong><span class="mck-product-value"></span></div></div></div></div></div> ' + ' </a></li> '; var searchContactbox = '<li id="li-${contHtmlExpr}" class="${contIdExpr}"><a class="applozic-launcher" href="javascript:void(0)" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}"><div class="mck-row" title="${contNameExpr}">' + '<div class="blk-lg-3">{{html contImgExpr}}</div>' + '<div class="blk-lg-9"><div class="mck-row"><div class="blk-lg-12 mck-cont-name mck-truncate"><strong>${contNameExpr}</strong>' + '<div class="move-right mck-group-count-box mck-group-count-text ${displayGroupUserCountExpr}">${groupUserCountExpr}</div></div>' + '<div class="blk-lg-12 mck-text-muted">${contLastSeenExpr}</div></div></div></div></a></li>'; //carwale    
            ...
            _this.openConversation = function () {
                if (!$mck_sidebox.hasClass('in')) {
                    //carwale
                    $applozic('.mckModal').mckModal('hide');
                    $mck_sidebox.mckModal();
                }
                if (typeof MCK_HIDE_CHAT_LOADER === 'function') {
                    // carwale
                    MCK_HIDE_CHAT_LOADER(); // carwale
                }
                $mck_msg_to.focus();
            };
            ...
            _this.loadTab = function (params, callback) {
                    ...
                    if ($mck_text_box.html().length > 1 || $mck_file_box.hasClass('vis')) {
                        ...
                        if ($mck_file_box.hasClass('vis')) {
                            $applozic('.mck-file-box').each(function () {
                                ...
                                if ((typeof fileMeta === 'undefined' ? 'undefined' : _typeof(fileMeta)) === 'object') {
                ...
                if (params.tabId) {
                    ...
                    if (IS_MCK_LOCSHARE && w.google && _typeof(w.google.maps) === 'object') {
            ...
            _this.closeConversation = function (data) {
                if (typeof MCK_DISPLAY_TEXT === 'function') {
                    ...
                    if ((typeof displayText === 'undefined' ? 'undefined' : _typeof(displayText)) === 'object') {
            ...
            _this.getContact = function (contactId) {
                if (_typeof(MCK_CONTACT_MAP[contactId]) === 'object') {
            ...
            _this.addMessage = function (msg, contact, append, scroll, appendContextMenu) {
                ...
                if (_typeof(msg.metadata) === "object" && typeof msg.metadata.AL_REPLY !== "undefined") {
                    ...
                    if (typeof replyMsg !== "undefined") {
                        ...
                        if (_typeof(replyMsg.fileMeta) === "object" || replyMsg.contentType === 2) {
                ...
                if (_typeof(msg.fileMeta) === "object") {
                ...
                if (msg.contentType === 4) {
                    ...
                    if (!MCK_PRICE_WIDGET_ENABLED) emoji_template += '<br/><button class="mck-accept" data-mck-topic-price="' + priceText + '" data-mck-conversationid="' + msg.conversationId + '">Accept</button>';
                }
            ...
            _this.getImageForMessagePreview = function (message) {
                if (_typeof(message.fileMeta) === 'object') {
            ...
            _this.getContactImageByAlphabet = function (name) {
                ...
                if (first_alpha.match(letters)) {
                    ...
                    return '<div class="mck-videocall-image alpha__name alpha_' + first_alpha + '"><span class="mck-contact-icon contact-icon-position">' + first_alpha + '</span></div>'; //carwale
                } else {
            ...
            _this.initAutoSuggest = function (params) {
                ...
                var matcher1 = function matcher1(item) {
                ...
                var updater = function updater(item) {
                    var contact = typeaheadMap[item];
            ...
            _this.updateContact = function (contact, message, $listId) {
                ...
                if (message && message.createdAtTime > currentMessageTime) {
                    ...
                    $messageText.html('');
                    (typeof emoji_template === 'undefined' ? 'undefined' : _typeof(emoji_template)) === 'object' ? $messageText.append(emoji_template) : $messageText.html(emoji_template);
                    if (message.conversationId) {
            ...
            _this.addContact = function (contact, $listId, message) {
                ...
                if ((typeof conversationPxy === 'undefined' ? 'undefined' : _typeof(conversationPxy)) === 'object' && IS_MCK_TOPIC_HEADER) {
                    var topicDetail = MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId];
                    if ((typeof topicDetail === 'undefined' ? 'undefined' : _typeof(topicDetail)) === 'object') {
                        isContHeader = 'vis';
                ...
                (typeof emoji_template === 'undefined' ? 'undefined' : _typeof(emoji_template)) === 'object' ? $textMessage.append(emoji_template) : $textMessage.html(emoji_template);
            };
            ...
            _this.addDraftMessage = function (tabId) {
                FILE_META = [];
                if (tabId && _typeof(TAB_MESSAGE_DRAFT[tabId]) === 'object') {
            ...
            _this.showOfflineMessage = function () {
                if ((typeof MCK_OFFLINE_MESSAGE_DETAIL === 'undefined' ? 'undefined' : _typeof(MCK_OFFLINE_MESSAGE_DETAIL)) === 'object' && MCK_OFFLINE_MESSAGE_DETAIL.innerHTML) {
            ...
            _this.getMessageTextForContactPreview = function (message, contact, size) {
                ...
                if (typeof message !== 'undefined') {
                    ...
                    } else if (message.fileMetaKey && _typeof(message.fileMeta) === "object") {
            ...
            _this.updateDraftMessage = function (tabId, fileMeta) {
                if ((typeof fileMeta === 'undefined' ? 'undefined' : _typeof(fileMeta)) === 'object') {
                    ...
                    if (typeof tabId !== 'undefined' && _typeof(TAB_MESSAGE_DRAFT[tabId]) === 'object') {
            ...
            _this.updateUnreadCount = function (tabId, count, isTotalUpdate) {
                ...
                if (isTotalUpdate && $mckChatLauncherIcon.length > 0) {
                    ...
                    if (typeof MCK_UNREAD_COUNT_CALLBACK === 'function') {
                        //carwale
                        MCK_UNREAD_COUNT_CALLBACK(MCK_TOTAL_UNREAD_COUNT); //carwale
                    } //carwale
                    else {
                        //carwale
        ...
        function MckContactService() {
                ...
                window.Applozic.ALApiService.getContactList({
                    ...
                    success: function success(data) {
                        ...
                        if ($mck_sidebox_search.hasClass('vis') || $mck_gms_loading.hasClass('vis')) {
                            if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object' && data.users.length > 0) {
                                $applozic.each(data.users, function (i, user) {
                                    if (typeof user.userId !== 'undefined') {
                                        var contact = mckMessageLayout.getContact('' + user.userId);
                                        contact = typeof contact === 'undefined' ? mckMessageLayout.createContactWithDetail(user) : mckMessageLayout.updateContactDetail(contact, user);
            ...
            _this.createFriendList = function (params) {
                ...
                if (params.type) {
                    ...
                    window.Applozic.ALApiService.createOpenFriendList({
                        data: group,
                        success: function success(response) {
                    ...
                    window.Applozic.ALApiService.createUserFriendList({
                        data: group,
                        success: function success(response) {
                            ALStorage.setFriendListGroupName(params.groupName);
            ...
            _this.getFriendList = function (friendListGroupName, friendListGroupType) {
                ...
                window.Applozic.ALApiService.getFriendList({
                    data: { groupName: friendListGroupName, url: getFriendListUrl, async: false },
                    success: function success(response) {
        ...
        function MckGroupLayout() {
            ...
            _this.getDeletedAtTime = function (groupId) {
                if (_typeof(MCK_GROUP_MAP[groupId]) === 'object') {
            ...
            _this.onGroupFeed = function (response, params) {
                ...
                if (response.status === 'success') {
                    ...
                    if (params.isMessage && _typeof(params.message) === 'object') {
                        mckMessageLayout.populateMessage(params.messageType, params.message, params.notifyUser);
                    }
            ...
            _this.onUpdateGroupInfo = function (response, params) {
                ...
                var group = mckGroupUtils.getGroup(groupId);
                if ((typeof group === 'undefined' ? 'undefined' : _typeof(group)) === 'object' && groupInfo) {
            ...
            _this.getGroupFeedFromMessage = function (params) {
                ...
                if (message) {
                    ...
                    if (message.conversationId) {
                        var conversationPxy = MCK_CONVERSATION_MAP[message.conversationId];
                        if ((typeof conversationPxy === 'undefined' ? 'undefined' : _typeof(conversationPxy)) !== 'object' || _typeof(MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId]) !== 'object') {
        ...
        function MckMapLayout() {
            ...
            _this.openMapBox = function () {
                $mckMapContent.locationpicker({
                    ...
                    onchanged: function onchanged(currentLocation) {
        ...
        function MckFileService() {
            ...
            _this.init = function () {
                ...
                $applozic(d).on("click", '.mck-remove-file', function () {
                    ...
                    if ((typeof currFileMeta === 'undefined' ? 'undefined' : _typeof(currFileMeta)) === 'object') {
                ...
                if (uploadErrors.length > 0) {
                    ...
                    if (params.name === $applozic(".mck-file-box." + randomId + " .mck-file-lb a").html()) {
                        ...
                        xhr.addEventListener('load', function (e) {
                            ...
                            if ((typeof responseJson === 'undefined' ? 'undefined' : _typeof(responseJson)) === "object") {
                                var file_meta = responseJson.fileMeta;
                                var fileExpr = (typeof file_meta === 'undefined' ? 'undefined' : _typeof(file_meta)) === "object" ? '<a href="' + file_meta.url + '" target="_blank">' + file_meta.name + '</a>' : '';
            ...
            _this.uploadFile = function (params) {
                ...
                if (uploadErrors.length > 0) {
                    ...
                    if (params.name === $applozic(".mck-file-box." + randomId + " .mck-file-lb a").html()) {
                        ...
                        xhr.addEventListener('load', function (e) {
                            var responseJson = $applozic.parseJSON(this.responseText);
                            if (_typeof(responseJson.fileMeta) === "object") {
                                var file_meta = responseJson.fileMeta;
            ...
            _this.addFileBox = function (file) {
                ...
                var fileName = '';
                if (_typeof(file.fileMeta) === 'object') {
        ...
        function MckNotificationService() {
            ...
            _this.showNewMessageNotification = function (message, contact, displayName) {
                ...
                if (message.message) {
                    ...
                    (typeof msg === 'undefined' ? 'undefined' : _typeof(msg)) === 'object' ? $mck_preview_msg_content.append(msg) : $mck_preview_msg_content.html(msg);
