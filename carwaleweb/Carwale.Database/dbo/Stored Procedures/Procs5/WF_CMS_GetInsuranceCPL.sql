IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_CMS_GetInsuranceCPL]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_CMS_GetInsuranceCPL]
GO

	CREATE PROCEDURE [dbo].[WF_CMS_GetInsuranceCPL]
...
BEGIN
	...
	SELECT @Value = ISNULL(SUM(BookedAmount),0) FROM CMS_Campaigns WHERE  CampaignCategory = 5
