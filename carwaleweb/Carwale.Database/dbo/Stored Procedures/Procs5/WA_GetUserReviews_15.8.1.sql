IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WA_GetUserReviews_15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WA_GetUserReviews_15]
GO

	CREATE PROCEDURE [dbo].[WA_GetUserReviews_15.8.1]  
...
SELECT Mk.ID AS MakeId, Mk.Name AS MakeName, Mo.ID AS ModelId, Mo.Name AS ModelName, Mo.LargePic, Mo.SmallPic, Mo.OriginalImgPath ,    
...
LEFT JOIN UserProfile UP with (NOLOCK)ON UP.UserId = CU.ID, CustomerReviews AS CR     
