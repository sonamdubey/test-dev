IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_CMS_GetAutomotiveCPA]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPA]
GO

	CREATE PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPA]
...
BEGIN
	...
	SELECT @Value = ISNULL(SUM(BookedAmount),0) FROM CMS_Campaigns WHERE  CampaignCategory = 1
