IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_CMS_GetAutomotiveCPL]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPL]
GO

	CREATE PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPL]
...
BEGIN
	...
	SELECT @Value = ISNULL(SUM(BookedAmount),0) FROM CMS_Campaigns WHERE  CampaignCategory = 2
