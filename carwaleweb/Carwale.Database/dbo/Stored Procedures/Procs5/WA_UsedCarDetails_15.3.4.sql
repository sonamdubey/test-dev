...
*/
CREATE PROCEDURE [dbo].[WA_UsedCarDetails_15.3.4] 
	 @IsDealer INT
...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN TC_CarAdditionalInformation CA WITH (NOLOCK) 
			ON CA.StockId = SI.TC_StockId --Added by Aditi Dhaybar on 24/2/2015 for  warranty,rsa,service records
		LEFT JOIN AbSure_CarDetails ABC WITH(NOLOCK)
			ON ABC.StockId = SI.TC_StockId  --Added by Aditi Dhaybar on 25/2/2015 for  Absure related details 
			AND GETDATE() BETWEEN ABC.SurveyDate AND DATEADD(dd, 30, ABC.SurveyDate) AND ABC.CarScore >=60 --Changed By Deepak on 24th APR 2015
		LEFT JOIN AbSure_WarrantyTypes ABW	--Added by Aditi Dhaybar 0n 15/4/2015 for Absure warranty type details
