IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_SP_PageViews]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_SP_PageViews]
GO

	CREATE PROCEDURE [dbo].[WF_SP_PageViews]
...
BEGIN
	...
	SELECT ISNULL(PageViews,0) AS Val, EntryDate 
	FROM DailyLogs 
