IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[temp1AbSure_GetOverallSummary]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[temp1AbSure_GetOverallSummary]
GO
...
-- Modified By : Vinay Kumar prajapati  11th may 2015 
-- EXEC [AbSure_GetOverallSummary]  13175
-- =============================================
CREATE PROCEDURE [dbo].[temp1AbSure_GetOverallSummary] 
(
	@UserId          INT
)
AS
	...
	ELSE IF (SELECT  ISNULL(TU.RoleId,13) FROM TC_Users AS TU WITH(NOLOCK) WHERE TU.Id = @UserId) = 13
		...
		IF @IsAxaAdmin=1
		    BEGIN 
				...
				SELECT ABCS.TC_UserId AS Surveyor, TU.UserName, TU.IsAgency, COUNT(ABC.Id) AS Allocated,
					...
					INNER JOIN AbSure_CarSurveyorMapping ABCS WITH (NOLOCK) ON ABC.Id = ABCS.AbSure_CarDetailsId
					INNER JOIN TC_Users AS TU WITH (NOLOCK) ON TU.Id = ABCS.TC_UserId
					WHERE IsAgency = 0  AND TU.Id IN(SELECT ChildId FROM @TempTable) 
					GROUP BY ABCS.TC_UserId, TU.UserName, TU.IsAgency, TU.HierId.GetAncestor(1)
		...
		ELSE
			BEGIN 
				    SELECT ABCS.TC_UserId AS Id, TU.UserName, TU.IsAgency, COUNT(ABC.Id) AS Allocated,
						SUM(CASE WHEN ISNULL(ABC.Status,0) = 0 AND CONVERT(DATE, ABC.AppointmentDate) < CONVERT(DATE, GETDATE()) THEN 1 ELSE 0 END) AS Pending,	
					...
					FROM AbSure_CarDetails ABC WITH (NOLOCK)
						...
						INNER JOIN TC_Users AS TU WITH (NOLOCK) ON TU.Id = ABCS.TC_UserId
					WHERE IsAgency = 0  AND TU.Id IN(SELECT ChildId FROM @TempTable)
					GROUP BY ABCS.TC_UserId, TU.UserName, TU.IsAgency
					ORDER BY  TU.UserName
	    END
END
