IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_TMPageWiseData]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_TMPageWiseData]
GO
...
-- Modified By:	Tejashree Patil on 18 Dec 2013: Description : Model version page wise data(PageId=8).
---PAGE ID =8   Version  Model wise
--- EXEC  [dbo].[TC_TMPageWiseData] 	@PageId=7
 -- =============================================
CREATE PROCEDURE [dbo].[TC_TMPageWiseData] 
	@PageId TINYINT,
	...
	BEGIN
		...
		ELSE IF (@PageId=2)  --AM Model wise
	     BEGIN 
				...
				FROM TC_TMIntermediateLegacyDetail AS I
	...
	ELSE IF (@PageId=6) --- Zone Version wise
     ...
     ELSE IF (@PageId=7)  --Version  Month wise
	    BEGIN
			...
			FROM TC_TMIntermediateLegacyDetail AS I WITH (NOLOCK)
			JOIN Dealers           AS D WITH (NOLOCK) ON I.DealerId=D.Id
			JOIN CarVersions       AS V WITH (NOLOCK) ON V.Id=I.CarVersionId
			JOIN TC_SpecialUsers   AS S WITH (NOLOCK) ON S.TC_SpecialUsersId=D.TC_AMId
			JOIN TC_BrandZone      AS TCB WITH (NOLOCK) ON TCB.TC_BrandZoneId=D.TC_BrandZoneId
