IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[test_GetPrices]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[test_GetPrices]
GO

	
--==========================================================
-- Created : Vicky Lund, 20/07/2016
-- EXEC [GetPrices_v16_8_5] 229, 160
-- EXEC [GetPrices_v16_8_5] 552, 1
-- Modified : Vicky Lund, 05/10/2016, Create temp table schema
--==========================================================
CREATE PROCEDURE [dbo].[test_GetPrices] @ModelId INT
	,@CityId INT
AS
BEGIN
	DECLARE @StateId INT
