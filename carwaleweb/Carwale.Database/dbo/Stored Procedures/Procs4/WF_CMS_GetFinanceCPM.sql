IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_CMS_GetFinanceCPM]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_CMS_GetFinanceCPM]
GO
	CREATE PROCEDURE [dbo].[WF_CMS_GetFinanceCPM]
...
BEGIN
	...
	SELECT @Value = ISNULL(SUM(BookedAmount),0) FROM CMS_Campaigns WHERE  CampaignCategory = 4
