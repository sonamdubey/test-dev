IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_StockDetailsFetch_V16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_StockDetailsFetch_V16]
GO
...
-- Modified By : Chetan Navin on 11th Nov, 2016 (Removed sellinquiries table)
-- =================================================================================================================================   
CREATE PROCEDURE [dbo].[TC_StockDetailsFetch_V16.11.1] @OrderBy VARCHAR(100) = NULL
	,@MakeId INT = NULL
	,@ModelId INT = NULL
	,@MinPrice INT = NULL
	,@MaxPrice INT = NULL
	...
	,-- MOdified By  :   Vivek Rajak on 04-08-2015
...
BEGIN
	...
	SELECT @CarGroupType = CASE 
			WHEN CC.CarGroupType IS NULL
				AND CC.TopUpCarGroupType IS NULL
				THEN NULL
			ELSE ISNULL(CC.CarGroupType, '') + ',' + ISNULL(CC.TopUpCarGroupType, '')
			END
	...
	BEGIN
		...
		AS (
			SELECT DISTINCT St.id
				...
				,(
					SELECT TOP 1 InvCertifiedDate
					FROM TC_CarTradeCertificationData WITH (NOLOCK)
					WHERE ListingId = St.Id
					ORDER BY InvCertifiedDate DESC
					) InvCertifiedDate
				,SuspendedDate
				,CG.CarGroupTypeId
				,CG.IsActive cgIsActive
				,CC.ExpiryDate ccExpiryDate
				,CASE 
					WHEN @CarGroupType IS NULL
						AND CONVERT(DATE, CC.ExpiryDate) >= CONVERT(DATE, GETDATE()) --ST.IsSychronizedCW = 0 AND
						THEN 1
					ELSE CASE 
							WHEN (
									CG.CarGroupTypeId IN (
										SELECT DISTINCT CT.ListMember
										FROM fnSplitCSVValues(@CarGroupType) AS CT
										)
									AND CG.IsActive = 1 --ST.IsSychronizedCW = 0 AND
			...
			LEFT JOIN ConsumerCreditPoints CC WITH (NOLOCK) ON CC.ConsumerId = ST.BranchId
				AND CC.ConsumerType = 1 -- Added by Suresh.P  on 15/04/2016 left join
				...
				AND (
					...
					OR (
						...
						AND ST.Id IN (
							...
							WHERE TR.CertificationStatus = 1
								AND TR.DealerId = @BranchId
							)
						)
					OR (
						--@chkCertifiedCarsStatus IS NULL 
					...
					OR (
						...
						AND (
							...
							OR (
								...
								AND (
									CG.CarGroupTypeId IS NULL
									OR CG.IsActive = 0
									OR CG.CarGroupTypeId NOT IN (
										SELECT DISTINCT CT.ListMember
										FROM fnSplitCSVValues(@CarGroupType) AS CT
										)
									)
	...
	BEGIN
		...
		SELECT DISTINCT TS.Id AS StockId
			...
			,CMa.ID MakeId
			,CMo.ID ModelId
			,(CMa.NAME + ' ' + CMo.NAME + ' ' + CV.NAME) AS MakeModelVersion
			,TS.issychronizedcw
			,(
				SELECT TOP 1 id
				FROM tc_carphotos Cp WITH (NOLOCK)
			...
			,CASE 
				WHEN TS.IsSychronizedCW = 0
					AND @CarGroupType IS NULL
					AND CONVERT(DATE, CCP.ExpiryDate) >= CONVERT(DATE, GETDATE())
					THEN 1
				ELSE CASE 
						WHEN TS.IsSychronizedCW = 0
							AND (
		...
		LEFT JOIN TC_MMDealersMatchCount Mc WITH (NOLOCK) ON Mc.StockId = TS.Id
			AND Mc.DealerId = @BranchId
			AND CONVERT(DATE, Mc.LastUpdatedOn) = CONVERT(DATE, GETDATE())
		LEFT JOIN TC_MappingOfferWithStock OS WITH (NOLOCK) ON TS.Id = OS.StockId
			AND OS.IsActive = 1
		LEFT JOIN ConsumerCreditPoints CCP WITH (NOLOCK) ON CCP.ConsumerId = TS.BranchId
			AND CCP.ConsumerType = 1 -- Added by Suresh.P  on 15/04/2016 left join
			--LEFT JOIN CarGroupTypes AS CG WITH (NOLOCK) ON CG.ModelId = Mo.Id
		...
		BEGIN
			SELECT DT.*
			INTO #AllRowForInspection
			FROM #DashboardTempTable DT
			INNER JOIN AbSure_EligibleModels AE WITH (NOLOCK) ON DT.ModelId = AE.ModelId
				AND AE.IsActive = 1
				AND AE.IsEligibleCertification = 1
			LEFT JOIN AbSure_CarDetails ABCD WITH (NOLOCK) ON DT.StockId = ABCD.StockId
				AND ABCD.IsActive = 1
			WHERE ABCD.StockId IS NULL
				OR ABCD.STATUS = 3
				OR (
					ABCD.IsSurveyDone = 1
					AND DATEDIFF(DAY, ABCD.SurveyDate, GETDATE()) > 30
					AND ABCD.STATUS = 4
					) -- 3 FOR CANCEAL 
		...
		BEGIN
			SELECT DT.*
			INTO #AllRowForWarrnty
			FROM #DashboardTempTable DT
			INNER JOIN AbSure_EligibleModels AE WITH (NOLOCK) ON DT.ModelId = AE.ModelId
				AND AE.IsActive = 1
				AND AE.IsEligibleWarranty = 1
				AND DT.Kms <= 85000
				AND (
					DATEDIFF(MONTH, DT.MakeYear, GETDATE()) > 24
					AND DATEDIFF(MONTH, DT.MakeYear, GETDATE()) <= 72
					)
				AND DT.CarFuelType IN (
					1
					,2
					,3
					) -------- Condition taken from SP [AbSure_IsWarranty]
			LEFT JOIN AbSure_CarDetails ABCD WITH (NOLOCK) ON DT.StockId = ABCD.StockId
				AND ABCD.IsActive = 1
			WHERE (
					(
						...
						AND DATEDIFF(DAY, ABCD.SurveyDate, GETDATE()) > 30
						) --- 4 FOR accepted but date expired
					OR ABCD.StockId IS NULL
					OR ABCD.STATUS = 3
					)
