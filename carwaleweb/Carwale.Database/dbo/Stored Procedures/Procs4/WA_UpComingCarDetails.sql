IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WA_UpComingCarDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WA_UpComingCarDetails]
GO
	/*
Author: Rakesh Yadav
...
*/
CREATE PROCEDURE [dbo].[WA_UpComingCarDetails] @id INT
	,@MakeId INT = NULL
AS
BEGIN
	...
	FOR XML PATH('')
END
/****** Object:  StoredProcedure [cw].[GetCarModelsByMakeId]    Script Date: 7/30/2014 8:23:22 PM ******/
SET ANSI_NULLS ON
