IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_StockDetailsInsert_V16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_StockDetailsInsert_V16]
GO
...
-- Modified By : Chetan Navin on 24th Oct, 2016 (Called new version of sp's TC_StockDetailsUpdate & TC_StockDetailsUpdateAPI) 
-- =============================================
CREATE PROCEDURE [dbo].[TC_StockDetailsInsert_V16.10.1]
...
BEGIN
	...
	BEGIN
		-- Modified By Vivek Gupta on 24/09/2014
		EXECUTE [TC_StockDetailsUpdate_V16.10.1] @StockId
			,@BranchId
			,@VersionId
			,@StatusId
			,@MakeYear
