IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NewRecommendCars_v16_12_3]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NewRecommendCars_v16_12_3]
GO
...
-- Modification: 10/26/2016 by jitendra singh create temp table then insert data into it
-- Modification: 12/12/2016 by Bhairavee modify SP to show top version instead of model
-- =============================================
/*
...
*/
CREATE PROCEDURE [dbo].[NewRecommendCars_v16_12_3]
...
BEGIN
	...
	[FuelEconomy] [float],
	[ChildSafety] [float],
	[Powerwindows] [varchar](50),
	[ABS] [varchar](60),
	[CentralLocking] [varchar](50),
	[Displacement] [float],
	[Power] [varchar](60),
	[Gearbox] [varchar](60),
	[FinalScore] [float],
	...
	[TransmissonType] [varchar](15),
	[AvgPrice] [int],
	[SpecsSummary] [varchar](100),
	[SortScore] [int],
	[VCount] [int]
	)
	...
	SELECT RC.*,CF.FuelType,CS.Name SegmentType
		,CT.Descr TransmissonType,NCP.AvgPrice ,CV.SpecsSummary
	...
	FROM RecommendCars RC WITH(NOLOCK)
		INNER JOIN CarVersions CV WITH(NOLOCK) ON RC.Versionid = CV.ID
		INNER JOIN CarFuelType CF WITH(NOLOCK) ON CV.CarFuelType = CF.FuelTypeId
		INNER JOIN CarSegments CS WITH(NOLOCK) ON CV.SegmentId = CS.ID
		INNER JOIN CarTransmission CT WITH(NOLOCK) ON CV.CarTransmission = CT.Id
		INNER JOIN CarModels CM WITH(NOLOCK) ON RC.Modelid = CM.ID
		INNER JOIN Con_NewCarNationalPrices NCP WITH(NOLOCK) ON RC.Versionid = NCP.VersionId
	WHERE (NCP.AvgPrice BETWEEN @Budget*(1 - @Range/100) AND @Budget)
		AND CM.New = 1
		AND CV.New = 1
		AND CV.IsDeleted = 0		-- Modification: 11/10/2013 by amit verma  (added logic to elimintate deleted version)
		AND NCP.AvgPrice IS NOT NULL
	...
	INSERT INTO @Models
	SELECT T3.ModelID,T3.PriceMax,T3.PriceMin,T3.MinAvgPrice,T3.Score,T3.VCount,BodyStyle FROM (
	...
	SELECT DISTINCT 
		...
		,CASE @Preset
			...
			END  AS BodyStyle
	FROM #CarData T1
	INNER JOIN CarModels CM WITH(NOLOCK) ON T1.ModelID = CM.ID
	INNER JOIN CarVersions CV WITH(NOLOCK) ON T1.Versionid = CV.ID AND CV.New = 1 AND CV.IsDeleted = 0	-- Modification: 11/10/2013 by amit verma  (added logic to elimintate deleted version)
	WHERE T1.SortScore >= @CutOffScore
	) T2) T3 WHERE T3.RowNum BETWEEN ( ((@Index - 1) * @PageSize )+ 1) AND (@Index*@PageSize)

	SELECT * FROM 
	(SELECT T1.Makeid,T1.Modelid,T1.Versionid,CM.Name Makename,CMO.Name Modelname,CV.Name Versionname,T2.MinAvgPrice
			,T1.AvgPrice,T1.SpecsSummary,T1.SortScore,T1.FinalScore,CMO.MaskingName,CMO.HostURL,CMO.OriginalImgPath,T2.VCount,ROW_NUMBER() OVER(PARTITION BY T1.Modelid ORDER BY T1.AvgPrice) AS rownumber
	FROM @Models T2
	INNER JOIN #CarData T1 ON  T2.ModelID = T1.ModelID
	INNER JOIN CarMakes CM WITH(NOLOCK) ON T1.Makeid = CM.ID
	INNER JOIN CarModels CMO WITH(NOLOCK) ON T1.Modelid = CMO.ID
	INNER JOIN CarVersions CV WITH(NOLOCK) ON T1.Versionid = CV.ID
	WHERE T1.SortScore >= @CutOffScore)cte
	ORDER BY cte.SortScore DESC,cte.MinAvgPrice DESC
