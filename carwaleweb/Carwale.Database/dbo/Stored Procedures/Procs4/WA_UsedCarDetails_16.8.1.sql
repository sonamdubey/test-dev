IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WA_UsedCarDetails_16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WA_UsedCarDetails_16]
GO
...
--Modified by Shubham Agarwal on 05/08/2016 - Added source id checks for autobiz cars
*/
CREATE PROCEDURE [dbo].[WA_UsedCarDetails_16.8.1] 
	 @IsDealer INT
...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN TC_CarAdditionalInformation CA WITH (NOLOCK) ON CA.StockId = SI.TC_StockId AND SI.SourceId = 2 --Added by Aditi Dhaybar on 24/2/2015 for  warranty,rsa,service records
		LEFT JOIN AbSure_CarDetails          ABC WITH(NOLOCK)  ON ABC.StockId = SI.TC_StockId AND SI.SourceId = 2 --Added by Aditi Dhaybar on 25/2/2015 for  Absure related details 
		...
		select @StockId = TC_StockId,@Price=Price,@StockSourceId = SourceId from SellInquiries with (nolock) where Id = @InquiryId
