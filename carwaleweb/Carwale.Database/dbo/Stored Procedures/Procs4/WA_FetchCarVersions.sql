IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WA_FetchCarVersions]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WA_FetchCarVersions]
GO

	CREATE  PROCEDURE [dbo].[WA_FetchCarVersions] --EXECUTE WA_FetchCarVersions 269,10 
@ModelId INT,
@CityId INT
--Author: Rakesh Yadav 
