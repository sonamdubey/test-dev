IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[WF_CMS_GetAutomotiveCPM]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPM]
GO
	CREATE PROCEDURE [dbo].[WF_CMS_GetAutomotiveCPM]
...
BEGIN
	...
	SELECT @Value = ISNULL(SUM(BookedAmount),0) FROM CMS_Campaigns WHERE  CampaignCategory = 3
