IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_TaskListLoadSearch_V16_10_2]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_TaskListLoadSearch_V16_10_2]
GO
...
-- Modified by : Deepak changed in temp table
...
-- ======================================================================================================
CREATE PROCEDURE [dbo].[TC_TaskListLoadSearch_V16_10_2]
...
BEGIN
	...
	CREATE TABLE #TempRawDataAllLeads (CustomerId INT, CustomerName	VARCHAR(250),Email VARCHAR(150), Mobile	VARCHAR(20), TC_InquirySourceId	TINYINT,
										TC_LeadId	INT, TC_InquiryStatusId	TINYINT, NextFollowUpDate	DATETIME, InterestedIn	VARCHAR(300), CallType	TINYINT,
										LastCallComment	VARCHAR(1000), LatestInquiryDate	DATETIME, OrderDate	DATETIME, InquirySource	VARCHAR(250), UserId INT,
										TC_LeadStageId	TINYINT, TC_NextActionId	SMALLINT, InquiryType	VARCHAR(150), InquiryTypeId	TINYINT, IsVerified	BIT,
										LeadCreationDate DATETIME, BucketTypeId SMALLINT, ExchangeCar VARCHAR(300), Eagerness	VARCHAR(50), Location VARCHAR(150),
										Car	VARCHAR(350), LeadAge	SMALLINT, AssignedTo VARCHAR(150), TC_InquiriesLeadId	INT, BranchId	INT, TC_CallsId	int,
										TC_LeadDispositionId	SMALLINT, TC_BusinessTypeId	TINYINT, NumberForPaging	INT, RegistrationNumber	VARCHAR(50),ExpiryDate DATETIME,TC_NextActionDate DATETIME);
	INSERT INTO #TempRawDataAllLeads (CustomerId,
									CustomerName,
									Email,
									Mobile,
									TC_InquirySourceId,
	...
	WHERE TTL.TC_LeadStageId <> 3
		...
		AND (
			...
			OR TTL.TC_BusinessTypeId IN (
				4
				,6
				,7--Feedback calling leads Added by Tejashree Patil on 28 Sept 2016
	...
	CREATE TABLE #TempRawDataBucketLeads (CustomerId INT, CustomerName	VARCHAR(250),Email VARCHAR(150), Mobile	VARCHAR(20), TC_InquirySourceId	TINYINT,
										TC_LeadId	INT, TC_InquiryStatusId	TINYINT, NextFollowUpDate	DATETIME, InterestedIn	VARCHAR(300), CallType	TINYINT,
										LastCallComment	VARCHAR(1000), LatestInquiryDate	DATETIME, OrderDate	DATETIME, InquirySource	VARCHAR(250), UserId INT,
										TC_LeadStageId	TINYINT, TC_NextActionId	SMALLINT, InquiryType	VARCHAR(150), InquiryTypeId	TINYINT, IsVerified	BIT,
										LeadCreationDate DATETIME, BucketTypeId SMALLINT, ExchangeCar VARCHAR(300), Eagerness	VARCHAR(50), Location VARCHAR(150),
										Car	VARCHAR(350), LeadAge	SMALLINT, AssignedTo VARCHAR(150), TC_InquiriesLeadId	INT, BranchId	INT, TC_CallsId	int,
										TC_LeadDispositionId	SMALLINT, TC_BusinessTypeId	TINYINT, RegistrationNumber	VARCHAR(50),ExpiryDate DATETIME,TC_NextActionDate DATETIME, NumberForPaging	BIGINT)

	INSERT INTO #TempRawDataBucketLeads (CustomerId,
										CustomerName,
										Email,
										Mobile,
										TC_InquirySourceId,
	...
	CREATE TABLE #TempBucketData(TC_BucketLeadTypeId TINYINT, BucketName VARCHAR(20), TotalLead INT)
	...
	SELECT 'ALL (' + CONVERT(VARCHAR(30), COUNT(TC_CallsId)) + ')'
		,CASE @BusinessTypeId
			...
			WHEN 7
				THEN 34 -- Feedback Calling leads,Added by : Tejashree Patil on 28 Sept 2016
			END
	...
	WHERE
		(
			...
			OR (
				...
				AND AL.TC_LeadDispositionId = @LeadDispositionId
				AND (
					(
						@IsToday = 1
						AND DATEDIFF(dd, AL.TC_NextActionDate, GETDATE()) = 0
						)
					OR (
						@IsToday = 0
						AND DATEDIFF(dd, AL.TC_NextActionDate, GETDATE()) > 0
			...
			OR (
				AL.TC_LeadDispositionId = @LeadDispositionId
				AND @LeadDispositionId NOT IN (
					120
					,121
					)
	...
	DROP TABLE #TempRawDataBucketLeads
