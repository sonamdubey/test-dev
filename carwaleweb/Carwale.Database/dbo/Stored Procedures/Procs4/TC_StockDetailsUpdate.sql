IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_StockDetailsUpdate]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_StockDetailsUpdate]
GO
...
-- =============================================  
CREATE PROCEDURE [dbo].[TC_StockDetailsUpdate] @StockId BIGINT
	,
...
BEGIN
	...
	--No Action for migrated dealer
				...
				IF (@APPRequest = 1)
				BEGIN
					EXECUTE TC_StockDetailsUpdateAPI @StockId
						,@BranchId
						,@VersionId
						,@StatusId
						,@MakeYear
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN -- updating only if car is available to carwale  
							...
							SET CarVersionId = @VersionId
								...
								,--Added ModifiedBy Field(Tejashree/Reshma)on 29 Nov 2012 
								CertificationId = @CertificationId
								,Comments = @Comments
								,LastUpdated = @tempLastUpdatedDate
							...
							SET InteriorColor = @InteriorColor
								...
								,InstalledFeaturesMissingIds = @MissingInstalledFeatures
								,RegistrationPlace = @RegPlace -- Modified By :  Tejashree Patil on 29 Oct 2012 at 5 pm   
							WHERE SellInquiryId = (
									SELECT ID
									FROM SellInquiries WITH (NOLOCK)
									WHERE TC_StockId = @StockId AND SourceId = 2
							...
							--@PrevPrice <>  @Price AND  
							IF (dbo.HDFCVehicleEligiblity(@BranchId, @Price, @Owners, @DiffYear) = 1)
							BEGIN
								UPDATE Sellinquiries
								SET CalculatedEMI = dbo.CalculateEMI(@Price, @DiffYear, 16.5)
								WHERE TC_StockId = @StockId
									AND DealerId = @BranchId AND SourceId = 2
							END
							ELSE
							BEGIN
								UPDATE Sellinquiries
					...
					BEGIN
						...
						BEGIN
							UPDATE SellInquiries
							SET StatusId = 2
							WHERE TC_StockId = @StockId AND SourceId = 2 --making car unavailable to carwale  
						END
