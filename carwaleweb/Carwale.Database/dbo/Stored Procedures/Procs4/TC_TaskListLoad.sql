IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_TaskListLoad]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_TaskListLoad]
GO
...
-- Modified By : Suresh Prajapati on 22nd June, 2016
-- Description : Synced with live changes and coomented 60 days logic for 'ALL' Bucket leads
-- =============================================     
CREATE PROCEDURE [dbo].[TC_TaskListLoad]
...
BEGIN
	...
	SELECT SUM(DISTINCT ID) NoInq
		,1 NoFollowUp
		,IC.TC_LeadId LeadId
	INTO #TempTbl1
	FROM (
		SELECT count(B.TC_BuyerInquiriesId) ID
			,L.TC_LeadId
		...
		SELECT count(SL.TC_SellerInquiriesId) ID
			,L.TC_LeadId
		FROM TC_InquiriesLead L WITH (NOLOCK)
		...
		SELECT count(N.TC_NewCarInquiriesId) ID
			,L.TC_LeadId
	...
	SELECT C.id AS [CustomerId]
		...
		,TCAC.CallType
	----- ,TCAC.LastCallComment -- commented by deepak  TCAC.LastCallComment and added below block on 25/05/2016
		,(
			SELECT STUFF((
						SELECT ', ' + (CONVERT(VARCHAR(19), ActionTakenOn) + ': ' + ActionComments)
						FROM (
							SELECT TOP 2 ActionComments
								,ActionTakenOn
							FROM TC_Calls
							WHERE IsActionTaken = 1
								AND TC_LeadId = TCAC.TC_LeadId
	...
	------------------------------------------------ -------------------------------------- 
		...
		,CASE 
			WHEN TCIL.TC_LeadDispositionId IN (
				...
				THEN ''
			ELSE (CONVERT(VARCHAR, DATEDIFF(day, TCIL.CreatedDate, GETDATE())) + ' Days')
			END AS LeadAge
	...
	SELECT *
		,CASE 
			WHEN T.BucketTypeId = 6
				THEN 1
			ELSE CASE 
					WHEN T.BucketTypeId <> 6
						AND (
							SELECT DISTINCT ISNULL(BucketTypeId, 0)
							FROM #TempRawDataAllLeads
							WHERE TC_LeadId = T.TC_LeadId
								AND BucketTypeId = 6
							) = 6
						THEN 0
	...
	SELECT ROW_NUMBER() OVER (
			PARTITION BY T.TC_LeadId ORDER BY EventCreatedOn DESC
			) Rows
		,D.NAME DispositionName
		,D.TC_LeadDispositionId LeadDispositionId
		,T.*
	...
	WHERE TL.RowNumber = 1
		AND TL.Rows = 1 --Added: rows=1 by Kritika Choudhary on 14th March 2016
		AND (
			(
				@LeadBucketId = 1
	...
	WHERE TL.RowNumber = 1
		...
		AND (
			(
				@LeadBucketId = 1
				/*AND (
					CONVERT(DATE, ScheduledOn) >= CASE 
						WHEN @FromFolloupdate IS NULL
							THEN CONVERT(DATE, GETDATE() - 60)
						ELSE CONVERT(DATE, ScheduledOn)
						END
					)*/
				)
			OR BucketTypeId = @LeadBucketId
	...
	SELECT @RecordCount AS RecordCount
		...
		,CASE 
			WHEN @FromFolloupdate IS NOT NULL
				THEN COUNT(*)
			ELSE SUM(CASE 
						WHEN CONVERT(DATE, ScheduledOn) >= CONVERT(DATE, GETDATE() - 60)
							THEN 1
						ELSE 0
						END)
		...
		,SUM(CASE 
				WHEN BucketTypeId = 10
					AND @DealerDeals = 1 -- added by vivek gupta
					THEN 1
				ELSE 0
				END) AS DealLeads
	FROM #TempLeadWithDisposition AS TT1
	WHERE TT1.RowNumber = 1
		AND TT1.Rows = 1 --Added: rows=1 by Kritika Choudhary on 14th March 2016
	...
	SELECT
		...
		CASE 
			WHEN @FromFolloupdate IS NOT NULL
				THEN 'ALL (' + CONVERT(VARCHAR(30), COUNT(*)) + ')'
			ELSE 'ALL (' + CONVERT(VARCHAR(30), ISNULL(SUM(CASE 
								WHEN CONVERT(DATE, ScheduledOn) >= CONVERT(DATE, GETDATE() - 60)
									THEN 1
								ELSE 0
								END), '0')) + ')'
		...
		WHERE TC_BucketLeadTypeId > @CurrentBucketId
			...
			AND (
				...
				OR (
					...
					AND @DealerDeals = 1
					) -- bucket type 10 not to be shown to the dealers who does not have deal permision
				)
