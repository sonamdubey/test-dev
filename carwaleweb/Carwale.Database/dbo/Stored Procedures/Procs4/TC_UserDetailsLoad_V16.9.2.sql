...
-- =============================================
CREATE PROCEDURE [dbo].[TC_UserDetailsLoad_V16.9.2]
	...
	BEGIN
		...
		ELSE
			...
			ELSE
				...
				END
				ELSE
					IF @TC_DealerTypeId = 10 --Added by : Kritika Choudhary on 30th Sep 2016, for feedback calling dealer,show only sales EX (N) role 
					BEGIN
						SELECT TC_RolesMasterId AS Id
							,RoleName AS NAME
						FROM TC_RolesMaster WITH (NOLOCK)
						WHERE TC_RolesMasterId IN (4)
							AND IsActive = 1 -- Khushaboo Patil on 4th sept 2016 remove rolemasterId  1
					END
					ELSE
						SELECT TC_RolesMasterId AS Id
							,RoleName AS NAME
