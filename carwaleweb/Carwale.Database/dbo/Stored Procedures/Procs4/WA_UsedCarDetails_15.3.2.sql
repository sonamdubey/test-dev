...
*/
CREATE PROCEDURE [dbo].[WA_UsedCarDetails_15.3.2] 
	 @IsDealer INT
...
BEGIN
	...
	BEGIN
		SELECT 
			...
			,CA.FreeRSADetails
			 --Added by Aditi Dhaybar on 25/2/2015 for  Absure related details
			,LL.AbsureScore															
			,ABC.Id		AS AbsureId																							
			,'http://www.autobiz.in/absure/CarCertificate.aspx?carId=' + CONVERT(VARCHAR, ABC.Id) AS AbsureCertificateUrl
			,LL.AbsureWarrantyType AS AbsureWarranty
...
select @StockId = TC_StockId from SellInquiries with (nolock) where Id = @InquiryId
	...
	BEGIN
		...
		INNER JOIN LiveListings LL WITH (NOLOCK) 
			ON LL.ProfileId = 'S' + CONVERT(VARCHAR(15), CSI.ID) 
