IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[vwAdv_GetBestDealDetails_v16_11_6]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[vwAdv_GetBestDealDetails_v16_11_6]
GO
-- =============================================
-- Author:		Sourav Roy
...
-- Modified by: Mukul Bansal, added delivery timeline in select query. 
-- Modified by: Saket Thapliyal on 16 Novemeber, added query for offers Table in select query. 
-- =============================================
CREATE PROCEDURE [dbo].[vwAdv_GetBestDealDetails_v16_11_6] @ModelId INT
	,@CityId INT
	,@VersionId INT = NULL
AS
BEGIN
	...
	SELECT DO.StockId,DO.OfferWorth,DO.AdditionalComments,DOC.Description
	FROM TC_Deals_Offers DO WITH (NOLOCK)
	INNER Join TC_Deals_OfferCategories DOC WITH (NOLOCK) ON DO.CategoryId = DOC.CategoryId AND DO.StockId IN (SELECT StockId FROM #FinalDealsData)
	UNION  
	SELECT DSP.TC_Deals_StockId,DSP.Insurance, null, 'Free Insurance' 
	FROM TC_Deals_StockPrices DSP  WITH (NOLOCK)
	WHERE DSP.TC_Deals_StockId IN (SELECT StockId FROM #FinalDealsData) AND DSP.CityId = @CityId

	DROP TABLE #FinalDealsData
