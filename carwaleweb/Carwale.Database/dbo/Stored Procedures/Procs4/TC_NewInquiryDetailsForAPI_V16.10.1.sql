IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_NewInquiryDetailsForAPI_V16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_NewInquiryDetailsForAPI_V16]
GO
...
-- EXEC TC_NewInquiryDetailsForAPI 30044,5,243
...
-- Modified By : Ashwini Dhamankar on Oct 3,2016 (Fetched Make,Model and Version Name)
-- Modified By : Ashwini Dhamankar on Oct 25,2016 (fetched TDCarDetails)
-- =======================================================================
CREATE PROCEDURE [dbo].[TC_NewInquiryDetailsForAPI_V16.10.1] 
@TC_LeadId INT,
@BranchId INT,
@TC_UserId INT,
  ...
  SELECT TC_InquiriesLeadId,
	  ...
	  WHERE L1.BranchId=@BranchId ANd L1.TC_LeadId=@TC_LeadId
	    ...
	    SELECT DISTINCT(NI.TC_NewCarInquiriesId) AS Id 
		          ,(CASE L.TC_LeadInquiryTypeId WHEN 3 THEN 'New' END) AS Type
				  ,L.TC_CustomerId AS NS_CustomerId
				  ,TDC.TC_TDCarsId AS NS_TDCarId
				  ,TDC.TC_UsersId AS NS_TDConsultantId
		          ,V.Car,CONVERT(VARCHAR,NI.TDDate,112) AS NS_TDDate
				  ,NI.TDStatus AS NS_TDStatus
				  ,NI.TC_TDCalendarId AS NS_TDCalendarId
				  ,V.ModelId AS NS_ModelId
		          ,NI.CityId AS NS_CityId
				  ,NI.VersionId AS NS_VersionId
				  ,NI.TC_InquirySourceId AS NS_SourceId
				  ,NI.TDDate AS 'Testdrive date'
				  ,V.MakeId AS NS_MakeId
				  ...
				  ,(	CASE  
					...
					,L.TC_InquiriesLeadId AS InquiriesLeadId
					,L.TC_LeadInquiryTypeId AS LeadInquiryTypeId
					,V.Make AS MakeName
					,V.Model AS ModelName
					,V.Version As VersionName   -- Modified By : Ashwini Dhamankar on Oct 3,2016
					,TDC.TDCarDetails AS TDCarDetails  -- Modified By : Ashwini Dhamankar on Oct 25,2016
