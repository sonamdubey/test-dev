IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_InquiryDetailsLoad]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_InquiryDetailsLoad]
GO
...
-- =============================================
CREATE  PROCEDURE   [dbo].[TC_InquiryDetailsLoad]
...
------------------------------------------------------------------------------------------------------------------------------
			...
			(CASE  -- Modified By: Tejashree Patil on 12 May 2016, Fetch url from comment.
						  ...
						  END) AS 'Comments:'
			FROM TC_NewCarInquiries NI WITH (NOLOCK)
			INNER JOIN NewCarShowroomPrices NP WITH (NOLOCK) ON NI.VersionId = NP.CarVersionId 
			...
			WHERE NP.CityId = (SELECT CityId FROM Dealers WITH(NOLOCK) WHERE Id=@BranchId) AND NI.TC_NewCarInquiriesId = @InquiryId
