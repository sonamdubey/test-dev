IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_Insurance_SaveInquiry_V16_10_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_Insurance_SaveInquiry_V16_10_1]
GO
...
-- Modified By  :	Ashwini Dhamankar on Oct 24,2016 (Adde AssignedTo and CreatedBy)
-- =============================================
create PROCEDURE [dbo].[TC_Insurance_SaveInquiry_V16_10_1]
	 @BranchId INT
	,@INQLeadId INT = NULL
	...
	IF @InsuranceInquiryId > 0
		BEGIN
			...
			SET		TC_InquiriesLeadId		= ISNULL(@INQLeadId,TC_InquiriesLeadId)
					...
					--,ChequePickUpDate		= ISNULL(@ChequePickupDate,ChequePickUpDate)
					--,PayAtShowroomDate		= ISNULL(@PayAtShowroomDate,PayAtShowroomDate)
					,IDV					= ISNULL(@IDV,IDV)
					,NCB					= ISNULL(@NCB,NCB)
					...
					,AssignedTo				= ISNULL(@AssignedTo,AssignedTo)    --Modified by : Ashwini Dhamankar on Oct 24,2016 
		...
		BEGIN
			...
			INSERT INTO TC_Insurance_Inquiries(	TC_InquiriesLeadId,TC_LeadDispositionId,RegistrationNumber,VersionId, EntryDate,
												InsuranceProvider,PolicyNumber,ExpiryDate,IsClaimsExist,BranchId,ModifiedBy,ModifiedDate,IDV,NCB,Discount,Premium,HypothecationId,AssignedTo,CreatedBy)
			VALUES(	@INQLeadId, @TC_LeadDispositionId, @RegistrationNumber, @VersionId, GETDATE(), 
					@InsuranceProvider, @PolicyNumber, @ExpiryDate, @IsClaimsExist, @BranchId,@ModifiedBy,GETDATE(),@IDV,@NCB,@Discount,@Premium,@HypothecationId,@AssignedTo,@CreatedBy)
