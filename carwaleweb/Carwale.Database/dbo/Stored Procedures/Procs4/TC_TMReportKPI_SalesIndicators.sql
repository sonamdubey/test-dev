IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_TMReportKPI_SalesIndicators]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_TMReportKPI_SalesIndicators]
GO

	
--	Author		:	Vivek Singh(26th December 2013)

--	Description :-To get target and acheivement under different sales indicator heads under the Logged in user(Hierarchy Wise)
--   Copied from procedure [TC_TMReportKPI_Salesoverview]
...
CREATE Procedure [dbo].[TC_TMReportKPI_SalesIndicators] 
