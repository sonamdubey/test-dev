IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_UserDetailsLoad_V16_10_3]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_UserDetailsLoad_V16_10_3]
GO
...
-- exec TC_UserDetailsLoad_V16_10_3 5,3,243,1,243,0
...
-- =============================================
CREATE PROCEDURE [dbo].[TC_UserDetailsLoad_V16_10_3]
...
BEGIN
		...
		---- SERVICE CENTER DEALER//Modified By : Nilima More On 20th July 2016,fetch roleId based on dealerType.
			...
			----Insurance Dealer // Modified By Ruchira Patil on 24th aug 2016,fetch roles (DP,CRE,Team Leader) for Insurance.
				...
				BEGIN 
					...
					OR TC_RolesMasterId IN(1,4,12)) AND IsActive = 1 --Modified By Kritika Choudhary on 19th oct 2016,fetch role sales ex for Insurance.
				END
				ELSE IF @TC_DealerTypeId = 10--Added by : Kritika Choudhary on 30th Sep 2016, for feedback calling dealer,show only sales EX (N) role 
				BEGIN
					SELECT TC_RolesMasterId AS Id,RoleName AS NAME
					FROM TC_RolesMaster WITH (NOLOCK)
					WHERE TC_RolesMasterId IN (4) AND IsActive = 1 -- Khushaboo Patil on 4th sept 2016 remove rolemasterId  1
				END
				ELSE
					SELECT TC_RolesMasterId AS Id
					,RoleName AS NAME
	...
	BEGIN
		...
		FROM	TC_Users U WITH (NOLOCK)
				INNER JOIN TC_UsersRole R WITH (NOLOCK) ON R.UserId = U.Id AND (@UserId is null or  U.Id NOT IN (@UserId))
		WHERE	U.BranchId = @BranchId AND U.IsActive = 1 AND U.IsCarwaleUser = 0
	...
	BEGIN
		...
		--Modified by Deepak on 25th Oct 2016 (removed join on vwAllMMV because of duplicate data)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/****** Object:  StoredProcedure [dbo].[TC_UsersDelete]    Script Date: 15-Jul-15 7:01:33 PM ******/
SET ANSI_NULLS ON
