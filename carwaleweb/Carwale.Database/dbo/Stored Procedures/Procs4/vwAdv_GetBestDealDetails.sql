IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[vwAdv_GetBestDealDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[vwAdv_GetBestDealDetails]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[vwAdv_GetBestDealDetails] @ModelId INT
	,@CityId INT
	,@VersionId INT = NULL
AS
BEGIN
