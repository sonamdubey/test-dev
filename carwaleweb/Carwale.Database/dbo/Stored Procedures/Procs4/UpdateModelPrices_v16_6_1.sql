IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[UpdateModelPrices_v16_6_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[UpdateModelPrices_v16_6_1]
GO
...
-- modified by : sachin bharti (28th june 2016)
-- purpose : update Con_NewCarNationalPrices table and min max price in CarModels table if any change in version
-- execute [dbo].[UpdateModelPrices_v16_6_1] 4234,null,3
-- =============================================
CREATE PROCEDURE [dbo].[UpdateModelPrices_v16_6_1] 
	-- Add the parameters for the stored procedure here
	@CarVersionId INT,
	@CityId INT=NULL,   -----Parametre added by Manish on 24-04-2014
	@UpdatedBy INT = NULL --added by sachin bharti on 28-06-2016
AS
BEGIN
   ...
   -- DECLARE @CityId INT=10  
	...
	IF @CityId IS NULL
	 SET @CityId = 10 --for New Delhi
	DECLARE @CarModelId AS INT ,@MinPrice NUMERIC(18,0) = NULL,@MaxPrice NUMERIC(18,0) = NULL
	SELECT @CarModelId=CarModelId FROM CarVersions WITH (NOLOCK) WHERE ID=@CarVersionId  -----added by Manish on 24-04-2014
	--added by sachin bharti (28th june 2016)
	declare @currentdate datetime = getdate()
	EXECUTE [dbo].[Con_SaveNewCarNationalPrices] @CarVersionId,@UpdatedBy,@currentdate 
	IF @CityId=10 OR @CityId IS NULL
	BEGIN 
		
		--get minimum and maximum prices
		SELECT 	@MinPrice=MIN(NCP.Price),@MaxPrice = MAX(NCP.Price)
		FROM NewCarShowroomPrices NCP WITH(NOLOCK)
		INNER JOIN CarVersions CV WITH(NOLOCK) ON CV.ID = NCP.CarVersionId
		WHERE	NCP.CityId = @CityId   --- Need to consider delhi price only
				AND CV.New=1 AND CV.IsDeleted=0	AND CV.CarModelId=@CarModelId  ---Condition added by Manish on 24-04-2014 
		GROUP BY cv.CarModelId
		--update carmodels table for min,max and average prices
		--change by sachin bharti (28th june 2016)
		UPDATE CarModels
			...
			FROM (	SELECT MIN(a.AvgPrice)as AvgPrice from Con_NewCarNationalPrices a 
					join CarVersions b on a.VersionId = b.id and b.CarModelId = @CarModelId
	...
	BEGIN CATCH
		...
		VALUES('MysqlSync','UpdateModelPrices_v16_6_1',ERROR_MESSAGE(),'SyncCarModelsWithMysqlUpdate',@ID,GETDATE(),@UpdateType)
	END CATCH
	END 
