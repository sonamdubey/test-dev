IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[UsedCarValuationSuggestions_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[UsedCarValuationSuggestions_v15]
GO
...
-- Modified by: Sahil Sharma Added fuel and transmission type
-- =============================================
CREATE PROCEDURE [dbo].[UsedCarValuationSuggestions_v15.12.1] @Version INT
	,@MakeYear DATETIME
	,@Cityid INT
	,@CarKm BIGINT
	,@CarPrice [numeric] (
...
BEGIN
	...
	SELECT TOP 4 ProfileId
		...
		,MakeName
		,ModelName
		,CMO.MaskingName --Added By Sahil Sharma on 25-11-2015
		,CityName
		,AreaName
		,MakeYear
		,Price
		...
		,IsPremium
		,LL.OriginalImgPath
		,CFT.FuelType--Added by Sahil Sharma on 25-11-2015
		,CR.Descr AS TransmissionType--Added by Sahil Sharma on 25-11-2015
	FROM LiveListings LL WITH (NOLOCK)
	Inner Join CarModels CMO WITH (NOLOCK) on LL.ModelId=CMO.ID
	Left Join CarVersions CV WITH (NOLOCK) ON LL.VersionId=CV.ID --Added By Sahil Sharma on 25-11-2015
	Left JOIN CarFuelType CFT WITH (NOLOCK)ON CFT.FuelTypeId = CV.CarFuelType --Added By Sahil Sharma on 25-11-2015
	Left JOIN CarTransmission CR WITH (NOLOCK) ON CR.Id = CV.CarTransmission --Added By Sahil Sharma on 25-11-2015 
	...
	BEGIN
		...
		--set @CarPrice=1000
		SET @sql = ' select top ' + cast(@cnt AS CHAR(2)) + '  L.ProfileId,Seller,L.MakeName+'' ''+L.ModelName+'' ''+L.VersionName as CarName,L.MakeName,L.ModelName,CMO.MaskingName,L.CityName,L.AreaName,L.MakeYear,L.Price,L.Kilometers,L.FrontImagePath,L.HostURL,L.ImageUrlMedium,L.IsPremium, L.OriginalImgPath,CFT.FuelType,CR.Descr ' +
	...
	BEGIN
		...
		Inner Join CarModels CMO WITH (NOLOCK) on L.ModelId=CMO.ID
			Left Join CarVersions CV WITH (NOLOCK) ON L.VersionId=CV.ID --Added By Sahil Sharma on 25-11-2015
			Left JOIN CarFuelType CFT WITH (NOLOCK)ON CFT.FuelTypeId = CV.CarFuelType --Added By Sahil Sharma on 25-11-2015
			Left JOIN CarTransmission CR WITH (NOLOCK) ON CR.Id = CV.CarTransmission --Added By Sahil Sharma on 25-11-2015 
