IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_SaveCarConditionInfo_SP_2012]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_SaveCarConditionInfo_SP_2012]
GO
...
--    to the Database.          
-- =============================================          
CREATE PROCEDURE [dbo].[TC_SaveCarConditionInfo_SP_2012]          
...
BEGIN          
 ...
 BEGIN
	...
	IF EXISTS(SELECT Top 1 * FROM SellInquiries WHERE TC_StockId = @StockId AND StatusId=1)
		BEGIN
			UPDATE SellInquiries SET Comments = @Comments WHERE TC_StockId = @StockId
