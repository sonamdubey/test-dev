IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_TMPageWiseDataFromFinalTarget]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_TMPageWiseDataFromFinalTarget]
GO
...
--- EXEC  [dbo].[TC_TMPageWiseData] 	@PageId=7
 -- =============================================
CREATE PROCEDURE [dbo].[TC_TMPageWiseDataFromFinalTarget] 
	@PageId TINYINT,
	...
	BEGIN
		...
		ELSE IF (@PageId=2)  --AM Model wise
	     BEGIN 
				...
				FROM TC_DealersTarget AS I  WITH(NOLOCK)
