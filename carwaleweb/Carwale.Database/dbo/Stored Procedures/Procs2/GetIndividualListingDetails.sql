IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetIndividualListingDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetIndividualListingDetails]
GO
...
-- =============================================
CREATE  PROCEDURE   [dbo].[GetIndividualListingDetails] 
...
BEGIN
	...
	GROUP BY CSI.ID,MA.NAME + ' ' + MO.NAME + ' ' + VE.NAME ,VE.ID ,CSI.CarRegNo,CSI.EntryDate,CSI.Price,CSI.MakeYear
		...
		,LL.Inquiryid, CSI.IsListingCompleted, CSI.PaymentMode
		,LC.Free,LC.Paid
