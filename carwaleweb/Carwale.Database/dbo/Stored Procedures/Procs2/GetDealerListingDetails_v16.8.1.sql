IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetDealerListingDetails_v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetDealerListingDetails_v16]
GO
...
-- Modified by Purohith Guguloth on 3rd december, 2015 - Added an IsNull Condition on FirstName LastName
-- Modified by prachi phalak on 13-01-2016--Added  BodyStyleId and SubSegmentId for recommended cars.
-- Modified by Shubham Agarwal on 02/08/2016 - removed @CityId and @DealerId, Added sourceId checks for autobiz cars.
--------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GetDealerListingDetails_v16.8.1] 
@InquiryId AS INT
...
BEGIN
	...
	SELECT LL.ProfileId
		...
		,D.Lattitude
		,D.Longitude
		,ISNULL(D.FirstName,'') + ' ' + ISNULL(D.LastName,'') AS DealerName --Changed by Purohith Guguloth, added a condition for ISNULL
		,D.ID AS DealerId
		,SI.TC_StockId
	...
	BEGIN
		...
		EXEC [dbo].[Classified_SellersOffering_v16.8.1] @InquiryId

		EXEC [dbo].[GetDescriptionofUsedCarwithOffers_v16.8.1] @InquiryId
