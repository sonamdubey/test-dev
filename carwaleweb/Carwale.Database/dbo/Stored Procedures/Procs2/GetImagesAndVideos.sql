IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetImagesAndVideos]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetImagesAndVideos]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetImagesAndVideos] -- EXEC GetImagesAndVideos 8
	@Top INT = 8,
	@ApplicationId int
AS
BEGIN
	...
	AS (
		SELECT TOP (10) EB.Id
			...
			,EB.Title
			,EB.Description
			,(EI.HostUrl+EI.ImagePathCustom) IMAGE_URL
			,MK.NAME MakeName
			,MO.NAME ModelName
			,MO.MaskingName
			,ROW_NUMBER() OVER (
