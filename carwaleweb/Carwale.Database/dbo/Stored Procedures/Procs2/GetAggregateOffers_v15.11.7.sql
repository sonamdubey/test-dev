...
-- =============================================
CREATE PROCEDURE [dbo].[GetAggregateOffers_v15.11.7]-- exec  [dbo].[GetAggregateOffers_V15.8.1] -1,-1,-1,1
...
BEGIN
	...
	AS (
		...
		WHERE DO.IsActive = 1
			...
			AND
			((@PlatformId = 1 AND DO.DispOnOffersPgDesk = 1) OR (@PlatformId in (43,74,83) AND DO.DispOnOffersPgMob = 1))
		)
	SELECT OfferId
		,MaxOfferValue
		,DealerId
