IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetDealerListingDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetDealerListingDetails]
GO
...
--Modified by Purohith Guguloth on 16th june, 2015  --- added a column dealerLogoUrl to get the dealers certification logo 
--Modifies by Navead on 17/06/2015 -- added a column DealerProfileImageUrl for getting the dealers profile image
...
--------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GetDealerListingDetails] 
@InquiryId AS INT
