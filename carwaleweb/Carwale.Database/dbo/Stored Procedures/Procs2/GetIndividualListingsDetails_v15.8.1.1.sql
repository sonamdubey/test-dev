IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetIndividualListingsDetails_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetIndividualListingsDetails_v15]
GO
...
-----------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GetIndividualListingsDetails_v15.8.1.1] 
	@InquiryId AS INT
AS
BEGIN
