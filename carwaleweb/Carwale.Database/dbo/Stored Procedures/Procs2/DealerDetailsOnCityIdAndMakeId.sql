IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[DealerDetailsOnCityIdAndMakeId]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[DealerDetailsOnCityIdAndMakeId]
GO
...
-- =============================================          
CREATE  PROCEDURE [dbo].[DealerDetailsOnCityIdAndMakeId] (
	@CityId INT
	,@MakeId INT
	)
AS
