...
--modified by vinayak Instead of dealer id we are retrieving dealer info by campaign id on 1/09/2014
...
--Modified By : Vicky Lund, 30/03/16, Get NotificationMobile also
-- =============================================
CREATE PROCEDURE [dbo].[GetDealerSponsorshipDeatils_V.16.4.1]
	-- Add the parameters for the stored procedure here
	@CampaignId INT
	,@DealerId INT OUTPUT
	,@DealerName VARCHAR(30) OUTPUT
	,@DealerMobile VARCHAR(50) OUTPUT
	,@NotificationMobile VARCHAR(150) OUTPUT
...
BEGIN
	...
	SELECT @DealerId = ds.DealerId
		...
		,@DealerLeadBusinessType = dl.DealerLeadBusinessType --modified by vinayak
		...
		,@SendMail=ISNULL(ds.ShowEmail,1)
	...
	WHERE ds.Id = @CampaignId
		AND ds.IsActive = 1 --modified by vinayak Instead of dealer id we are retrieving dealer info by campaign id
END
