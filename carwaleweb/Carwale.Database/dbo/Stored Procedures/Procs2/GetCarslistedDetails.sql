IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetCarslistedDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetCarslistedDetails]
GO
...
--- [dbo].[GetCarslistedDetails] 60
-- =============================================
CREATE PROCEDURE [dbo].[GetCarslistedDetails] 
...
BEGIN
	...
	FROM SellInquiries as CS
	  ...
	  join Dealers as D ON D.ID=CS.DealerId
	  join vwCityDetails as CT on CT.CityId=D.CityId
	WHERE CS.Id IN (SELECT ProfileId  FROM TempCarResponse WHERE SellerType=1 )
	...
	join vwCityDetails as CT on CT.CityId=CS.CityId
	join vwMMV as M on M.VersionId=CS.CarVersionId
	WHERE CS.Id IN (SELECT ProfileId  FROM TempCarResponse WHERE SellerType=2)	
