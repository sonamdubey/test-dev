IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetHouseCrossSellRules]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetHouseCrossSellRules]
GO
...
-- EXEC [GetHouseCrossSellRules] 1
-- =============================================
CREATE PROCEDURE GetHouseCrossSellRules @CampaignId INT
AS
BEGIN
	SELECT FAR.Id
		,CASE 
	...
	LEFT OUTER JOIN OprUsers OU WITH (NOLOCK) ON FAR.AddedBy = OU.Id
