...
-- Modified By: Vicky Lund, 16/11/2015, Removed Dealer_NewCar dependency
-- =============================================
CREATE PROCEDURE [dbo].[GetOffers_v15.11.3]
...
BEGIN
	...
	SELECT TOP 1 @OfferId = DO.ID
		...
		,@DealerName = DNC.Organization -- added by ashish verma
	...
	WHERE DO.IsActive = 1
		...
		AND (
			...
			OR (
				DOD.CityId = @CityId
				AND ISNULL(DOD.ZoneId, 0) = ISNULL(@ZoneId, 0)
				)
			)
		AND (
			@CMKId = - 1
		...
		AND (
			...
			OR (
				@PlatformId IN (43, 74, 83)
				AND DispOnMobile = 1
				)
			OR OfferType != 3
			) --Added By Vikas J
		AND DNC.IsDealerActive = 1
		AND DNC.IsDealerDeleted = 0
		AND DNC.TC_DealerTypeId = 2
	ORDER BY DO.OfferType
		,NEWID()
END
