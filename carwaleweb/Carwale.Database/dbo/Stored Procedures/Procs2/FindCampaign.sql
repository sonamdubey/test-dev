IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[FindCampaign]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[FindCampaign]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[FindCampaign] @DealerId INT
	,@StateId INT
	,@CityIds VARCHAR(MAX)
	,@MakeId INT
...
BEGIN
	SELECT PDS.Id AS CampaignId
		...
		,CASE 
			WHEN PDS.EndDate IS NULL
				THEN ''
			ELSE CONVERT(VARCHAR(10), PDS.EndDate, 103)
			END EndDate
		,CASE 
			WHEN PDS.EndDate IS NULL
				THEN 'Lead wise'
