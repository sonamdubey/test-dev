...
CREATE PROCEDURE [dbo].[GetDealersForCity_v16.9.8] @CityId INT 
AS 
BEGIN
   SELECT    D.id AS DealerId, 
	...
	FROM	dealers AS D WITH (NOLOCK)
			LEFT JOIN sellinquiries Si WITH (NOLOCK) ON Si.DealerId = D.ID 
			INNER JOIN CT_AddOnPackages AD WITH(NOLOCK) ON AD.CWDealerId = D.ID 
			AND AD.IsActive = 1
