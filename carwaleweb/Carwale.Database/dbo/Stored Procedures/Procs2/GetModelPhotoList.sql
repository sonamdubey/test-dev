IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetModelPhotoList]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetModelPhotoList]
GO
...
-- exec GetModelPhotoList 458,10,1
--modified by natesh kumar on 27/11/14 for is active flag
--------------------------------------------------------
CREATE PROCEDURE [dbo].[GetModelPhotoList]
@ModelId INT,
...
BEGIN
	...
	BEGIN
		...
		INNER JOIN Con_EditCms_Basic AS BA WITH (NOLOCK) ON BA.Id = IMG.BasicId AND BA.ApplicationID = @ApplicationId
		...
		WHERE BA.ApplicationId = @ApplicationId AND BA.IsPublished = 1 AND BA.IsActive = 1 AND IMG.IsActive = 1 AND IMG.ModelId = @ModelId
		AND Mo.IsDeleted = 0 AND MK.IsDeleted = 0
		ORDER BY IMG.Sequence ASC, IMG.Id DESC
	END
	...
	BEGIN
		...
		WHERE BA.ApplicationId = @ApplicationId AND BA.IsPublished = 1 AND BA.IsActive = 1 AND IMG.IsActive = 1 AND IMG.ModelId = @ModelId
		AND Mo.IsDeleted = 0 AND MK.IsDeleted = 0 
		ORDER BY IMG.Sequence DESC, IMG.Id DESC
	END
END
