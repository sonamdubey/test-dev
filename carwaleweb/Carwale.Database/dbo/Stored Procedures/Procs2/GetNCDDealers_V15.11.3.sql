...
-- =============================================          
CREATE PROCEDURE [dbo].[GetNCDDealers_V15.11.3] (
	@CityId SMALLINT
	,@MakeId SMALLINT
	)
AS
...
(	SELECT D.Id AS Id
		...
		,CASE WHEN [dbo].[IsCampaignActiveBasic] (P.Id )=1 THEN 1 ELSE 0 	END    AS IsPremium 
	...
	LEFT JOIN PQ_DealerSponsored AS P WITH (NOLOCK) ON DNC.PQ_DealerSponsoredId = P.Id AND  P.IsActive=1
	LEFT JOIN  Microsite_DealerContent mdc  WITH (NOLOCK)  ON D.Id =  mdc.DealerId AND mdc.ContentCatagoryId = 2 AND mdc.IsActive = 1 and mdc.ContentSubCatagoryId is null
	LEFT JOIN Microsite_Images mi  WITH (NOLOCK) ON DNC.DealerId = mi.DealerId  AND mi.IsActive = 1 AND mi.IsBanner =1 and mi.BannerImgSortingOrder=0
	WHERE D.CityId = @CityId
		...
		--AND dbo.IsCampaignActiveBasic(P.Id) = 1
		) SELECT  Id
		...
		,EMailId
		,WebSite
		 ,CASE WHEN IsPremium=1 THEN  CampaignMobileNo ELSE  DealerLevelMobileNo END AS MobileNo 
		, City
		, State
		, Latitude 
		, Longitude 
	...
	SELECT D.Id AS Id
		...
		,mi.OriginalImgPath
		,mdc.DealerContent
		,ROW_NUMBER () OVER (ORDER BY D.Organization) RowOrder
	FROM DealerLocatorConfiguration AS DNC WITH (NOLOCK) 
