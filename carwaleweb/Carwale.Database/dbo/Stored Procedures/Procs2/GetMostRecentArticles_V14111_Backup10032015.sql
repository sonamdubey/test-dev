IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetMostRecentArticles_V14111_Backup10032015]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetMostRecentArticles_V14111_Backup10032015]
GO
...
--================================================================
-- exec [dbo].[GetMostRecentArticles_V14111] 2,8,50
CREATE PROCEDURE [dbo].[GetMostRecentArticles_V14111_Backup10032015] 
	@ApplicationId INT
...
BEGIN
	...
	BEGIN
		WITH cte
		AS (
			SELECT top 100  cb.Id AS BasicId
			...
			JOIN Con_EditCms_Images cei WITH (NOLOCK) ON cb.Id = cei.BasicId
						...
						AND cei.IsActive = 1
			JOIN SocialPluginsCount spc WITH (NOLOCK) ON cb.Id = spc.TypeId
			JOIN Con_EditCms_Cars C WITH (NOLOCK) ON C.BasicId = CB.Id
				AND C.IsActive = 1
