IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetPQVersionDetails_v16_11_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetPQVersionDetails_v16_11_1]
GO
...
-- Modified By: Jitndra singh on 10 Aug 2016, Fetch Version Masking Name
-- Modified By: Shubham Agarwal on 28 Sep 2016, Fetch RootId, BodyStyleId
-- Modified By: Piyush Sahu on 18 Nov 2016, Fetch Transmission Type
-- =============================================
CREATE PROCEDURE [dbo].[GetPQVersionDetails_v16_11_1]
	...
	@VersionMasking VARCHAR(30) = null OUTPUT,  -- Fetch Version Masking Name
	...
	@RootId int output, -- modified by Shubham Agarwal
	@BodyStyleId int output,
	@CarTransmission int output       -- added by piyush sahu
...
BEGIN
	...
	SELECT 
		...
		,@FuelType=CF.FuelType,@FuelTypeId=CF.FuelTypeId, @SubSegmentId = MO.SubSegmentID, @RootId = MR.RootId, @BodyStyleId = CV.BodyStyleId, @CarTransmission = CV.CarTransmission
		,@VersionMasking = CV.MaskingName -- Fetch Version Masking Name
	FROM CarVersions CV WITH (NOLOCK)
		...
		INNER JOIN CarModelRoots MR WITH (NOLOCK)  ON MR.MakeId=MA.Id
