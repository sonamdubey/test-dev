IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetPopularReviews_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetPopularReviews_v15]
GO
...
-- Modified by Sanjay On 07/08/2015 added OriginalImgPath in query
-- =============================================
CREATE PROCEDURE [dbo].[GetPopularReviews_v15.8.1] 
	@Top int = 3
AS
BEGIN
With CTE AS(
