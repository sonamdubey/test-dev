...
-- Modified by: Shalini Nair on 03/11/2015 added filter on offers according to platform & DispOffersOnDesk or DispOffersOnMob flag set
-- Modified by: Vinayak, 16/11/2015, removed dealer_newcar dependency
-- =============================================
CREATE PROCEDURE [dbo].[GetAggregateOffers_v15.11.3]-- exec  [dbo].[GetAggregateOffers_V15.8.1] -1,-1,-1,1
...
BEGIN
	...
	AS (
		...
		WHERE DO.IsActive = 1
			...
			--AND (@PlatformId = 1 AND DO.DispOnOffersPgDesk = 1) OR (@PlatformId = 43 AND DO.DispOnOffersPgMob = 1)
		)
	SELECT OfferId
		,MaxOfferValue
		,DealerId
	...
	FROM CTE
	LEFT JOIN Dealers D WITH (NOLOCK) ON D.Id = CTE.DealerId
	WHERE Row_Num = 1
	ORDER BY OfferType
		,MaxOfferValue DESC
END
