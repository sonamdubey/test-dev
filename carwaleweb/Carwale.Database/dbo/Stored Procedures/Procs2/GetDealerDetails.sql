IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetDealerDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetDealerDetails]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetDealerDetails]
...
BEGIN
	...
	SELECT DISTINCT STUFF((
		...
		,STUFF((
				...
				FROM TC_DealerMakes WITH (NOLOCK) WHERE DealerId = @DealerId
				FOR XML PATH('')
				), 1, 1, '') AS DealerMake
...
/****** Object:  StoredProcedure [dbo].[UpdateDealer_v16.1.4]    Script Date: 01/02/2016 16:40:52 ******/
SET ANSI_NULLS ON
