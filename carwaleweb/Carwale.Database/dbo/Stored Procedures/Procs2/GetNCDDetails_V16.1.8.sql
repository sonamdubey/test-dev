IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetNCDDetails_V16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetNCDDetails_V16]
GO
...
-- Modified By : Sanjay Soni on 22/12/2015 added @Id as output parameter
-- Modified By : Shalini Nair on 08/01/2015 retrieving masking number from MM_sellermobilemasking
-- Modified By : Vicky Lund on 04/03/2016 Used contract parameters instead of campaign parameters
-- =============================================          
CREATE PROCEDURE [dbo].[GetNCDDetails_V16.1.8] (
	@DealerId INT
	,
	--OUTPUT--
	@MakeId INT OUTPUT
...
BEGIN
	...
	INNER JOIN vwRunningCampaigns VRC WITH (NOLOCK) ON DS.Id = VRC.CampaignId			--Vicky Lund, 04/03/2016
