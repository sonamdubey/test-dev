IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetPopularCar_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetPopularCar_v15]
GO
...
-- Exec [GetPopularCar]
CREATE PROCEDURE [dbo].[GetPopularCar_v15.8.1]
@ApplicationId INT
AS
BEGIN
	with cte
		as(
			SELECT 
				...
				Mo.OriginalImgPath AS OriginalImgpath,
				...
				LEFT JOIN (SELECT EC.basicid, EC.modelid  FROM   con_editcms_cars EC  WITH (NOLOCK)
				...
				LEFT JOIN (SELECT CEI.modelid,Count(CEI.id) AS ImageCount FROM   con_editcms_images CEI  WITH (NOLOCK)
