...
-- Modified BY : Sanjay Soni on 31/03/2016 to insert and update notification mobile
...
-- Modified By: Shalini Nair on 12/04/2016 to update TC_ContractCampaignMapping using dealerId
-- MOdified by Sanjay Soni on 21/07/2016 added MM_DealerMobile
-- =============================================
CREATE PROCEDURE [dbo].[InsertorUpdatePQCampaign_v16.4.1] @CampaignId INT
	,@DealerId INT
	,@DealerName VARCHAR(200)
	,@Phone VARCHAR(50)
	,@DealerNotificationEmail VARCHAR(250)
	,@DealerNotificationMobile VARCHAR(150)
	,@UpdatedBy INT
	,@TotalGoal INT = NULL
	,@DailyGoal INT
	,@LeadPanel TINYINT
...
BEGIN
	...
	BEGIN
		...
		VALUES (
			...
			,@Phone
			,@IsActive
			,@DealerNotificationEmail
			,@DealerNotificationMobile
			,@Start_Date
			,@End_Date
			,@UpdatedBy
			,GETDATE()
	...
	BEGIN
		...
		WHERE Id = @CampaignId
			...
			BEGIN
				...
				IF((SELECT ISNULL((Select Mobile from MM_SellerMobileMasking WITH (NOLOCK) WHERE LeadCampaignId = @CampaignId
				AND ApplicationId = 1),'')) != @MM_DealerMobile)
				BEGIN
					UPDATE MM_SellerMobileMasking set Mobile = @MM_DealerMobile WHERE LeadCampaignId = @CampaignId
					AND ApplicationId = 1
				END
		...
		SET --DealerName = @DealerName,
			DealerId = @DealerId
			,Phone = @Phone
			,DealerEmailId = @DealerNotificationEmail
			,NotificationMobile = @DealerNotificationMobile
			,IsActive = @IsActive
			,UpdatedBy = @UpdatedBy
			,UpdatedOn = GETDATE()
			,DailyGoal = @DailyGoal
