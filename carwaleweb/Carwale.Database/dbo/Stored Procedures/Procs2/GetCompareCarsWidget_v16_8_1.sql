IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetCompareCarsWidget_v16_8_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetCompareCarsWidget_v16_8_1]
GO
...
-- Modified By : Ajay Singh on 29-July-2016 added a filter to remove discontinue versions on carversions table and carmodels table
create PROCEDURE [dbo].[GetCompareCarsWidget_v16_8_1] -- exec [dbo].[GetCompareCarsWidget_v16_8_1] 4
...
AS
  BEGIN
      ...
      WHERE  CCL.isactive = 1
             AND isarchived = 0
			 AND CV.New = 1 --added by ajay singh on 29 july 2016
			 AND CV.IsDeleted = 0
			 AND CMO.New = 1 --added by ajay singh on 29 july 2016
			 AND CMO.IsDeleted = 0
			 AND CVE.New = 1 --added by ajay singh on 29 july 2016
			 AND CVE.IsDeleted = 0
			 AND CMOD.New = 1 --added by ajay singh on 29 july 2016
			 AND CMOD.IsDeleted = 0
      ORDER  BY rowno
