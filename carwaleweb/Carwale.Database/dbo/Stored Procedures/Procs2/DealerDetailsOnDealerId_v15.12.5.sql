IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[DealerDetailsOnDealerId_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[DealerDetailsOnDealerId_v15]
GO
...
-- Modified By : Ashwini T on 14 dec 2015
-- =============================================          
CREATE PROCEDURE [dbo].[DealerDetailsOnDealerId_v15.12.5] (
	@DealerId INT
	--OUTPUT--
	...
	,@HostUrl VARCHAR(100) OUTPUT			--added by Ashwini T
	,@ProfilePhoto VARCHAR(250) OUTPUT
	)
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT  @OutDealerId = D.ID
		...
		,@StateId =S.ID
		,@FaxNo =d.FaxNo
		,@HostUrl = d.ProfilePhotoHostUrl
		,@ProfilePhoto = d.ProfilePhotoUrl
