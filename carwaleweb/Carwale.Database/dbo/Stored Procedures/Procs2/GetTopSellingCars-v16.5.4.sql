IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetTopSellingCars-v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetTopSellingCars-v16]
GO
...
-- Modified by AJAY SINGH on 11-05-2016 fetched two more column from pricecityid and pricecityname
CREATE PROCEDURE [dbo].[GetTopSellingCars-v16.5.4]
@DisplayCount INT=0
AS
BEGIN
		with cte
		as (SELECT    TSC.modelid ModelId, 
				  ...
				  CMD.PriceCityId,  --added by AJAY SINGH on 11/05/2016
				  CMD.PriceCityName,--added by AJAY SIGNH on 11/05/2016
		...
		/*GROUP BY  TSC.modelid, 
				  ...
				  CMD.MaskingName,
				  CMD.ReviewCount --added by shalini on 09/09/14*/ 
				  --This group by clause commented by Manish on 21-12-2015 since no aggregate function used.
