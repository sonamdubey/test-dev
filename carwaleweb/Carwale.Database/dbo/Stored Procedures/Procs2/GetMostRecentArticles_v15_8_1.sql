IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[GetMostRecentArticles_v15_8_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[GetMostRecentArticles_v15_8_1]
GO
...
CREATE PROCEDURE [dbo].[GetMostRecentArticles_v15_8_1] 
	@ApplicationId INT
...
BEGIN
	...
	BEGIN
		...
		AS (
			...
			LEFT JOIN Con_EditCms_BasicSubCategories BSC  WITH (NOLOCK) ON BSC.BasicId = cb.Id 
	...
	BEGIN
		...
		AS (
			SELECT cb.Id AS BasicId
				...
				,CEI.OriginalImgPath AS OriginalImgPath
				,M.Id AS MakeId
				,MO.ID AS ModelId
				,M.NAME AS MakeName
				,MO.NAME AS ModelName
