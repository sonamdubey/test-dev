IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[CRM_AP_FetchAMDailySummayData]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[CRM_AP_FetchAMDailySummayData]
GO
...
-- =============================================
-- Author:		Dilip Vasu
-- Modifier:	1. Amit Kumar (Added type 6 for potentially lost case.)
-- Create date: 11th May 2012
-- Description:	This proc selects Count of Assigned, PQ (Requested and Completed),PQ Pending, TD (Requested and Completed), Booked for AM
-- =============================================

 CREATE PROCEDURE [dbo].[CRM_AP_FetchAMDailySummayData]
(	
...
BEGIN
	...
	IF @Type = 1	--Assigned
		BEGIN
			...
			SET @varsql =  'SELECT COUNT(DISTINCT CDA.Id) Cnt, CDA.DealerId, ''1'' AS Type FROM CRM_CarDealerAssignment AS CDA WITH(NOLOCK), 
							...
							GROUP BY CDA.DealerId
						   
							UNION ALL 
							   
							SELECT COUNT(DISTINCT CDA.Id) Cnt, CDA.DealerId, ''2'' AS Type FROM CRM_CarDealerAssignment AS CDA WITH(NOLOCK), 
							NCS_RMDealers AS RM WITH(NOLOCK), NCS_SubDealerOrganization AS ND WITH(NOLOCK)
							WHERE RM.DealerId = CDA.DealerId AND RM.RMId IN (' + @ManagerId + ')
							AND RM.IsExecutive = 0 AND RM.DealerId = ND.DId'
							...
							SET @varsql = @varsql + ' GROUP BY CDA.DealerId
							
							UNION ALL 
							  
							SELECT COUNT(DISTINCT CDA.Id) Cnt, CDA.DealerId, ''3'' AS Type FROM CRM_CarDealerAssignment AS CDA WITH(NOLOCK), 
							NCS_RMDealers AS RM WITH(NOLOCK), NCS_SubDealerOrganization AS ND WITH(NOLOCK)
							WHERE RM.DealerId = CDA.DealerId AND RM.RMId IN (' + @ManagerId + ')
							AND RM.IsExecutive = 0 AND RM.DealerId = ND.DId'
	...
	ELSE IF @Type = 2    --PQ Pending
		...
		SET @varsql = ' SELECT COUNT(DISTINCT CCPL.id) Cnt, CDA.DealerId 
						...
						AND DATEDIFF(day, CCPL.PQRequestDate, GETDATE()) > 2 GROUP BY CDA.DealerId'
		END
	ELSE IF @Type = 3    --Others Like 36-PQCompleted, 44-PQRequested, 7-TDRequested, 14-TDCOmpleted, CarBooked-16
		BEGIN
		
		SET @varsql = 'SELECT COUNT(DISTINCT CCBL.ItemId) Cnt, CCBL.EventType, CDA.DealerId, ''1'' AS Type 
						...
						GROUP BY CDA.DealerId, CCBL.EventType
						
						UNION ALL 
						
						SELECT COUNT(DISTINCT CCBL.ItemId) Cnt, CCBL.EventType, CDA.DealerId, ''2'' AS Type 
						FROM CRM_EventLogs CCBL WITH(NOLOCK), CRM_CarDealerAssignment CDA WITH(NOLOCK), NCS_RMDealers AS RM WITH(NOLOCK), NCS_SubDealerOrganization AS ND WITH(NOLOCK)
						WHERE CCBL.ItemId = CDA.CBDId AND RM.DealerId = CDA.DealerId AND RM.DealerId = ND.DId
						AND RM.RMId IN (' + @ManagerId + ') AND CCBL.EventType IN(7,14,16,44,36) AND RM.IsExecutive = 0'
						...
						SET @varsql = @varsql + ' GROUP BY CDA.DealerId, CCBL.EventType  
					   						
						UNION ALL 
						
						SELECT COUNT(DISTINCT CCBL.ItemId) Cnt, CCBL.EventType, CDA.DealerId, ''3'' AS Type 
						FROM CRM_EventLogs CCBL WITH(NOLOCK), CRM_CarDealerAssignment CDA WITH(NOLOCK), NCS_RMDealers AS RM WITH(NOLOCK), NCS_SubDealerOrganization AS ND WITH(NOLOCK)
						WHERE CCBL.ItemId = CDA.CBDId AND RM.DealerId = CDA.DealerId AND RM.DealerId = ND.DId
						AND RM.RMId IN (' + @ManagerId + ') AND CCBL.EventType IN(7,14,16,44,36) AND RM.IsExecutive = 0'
	...
	ELSE IF @Type = 4
		BEGIN
			SET @varsql = 'SELECT COUNT(CCTL.id) Cnt, CDA.DealerId, ''1'' AS Type
						...
						SELECT COUNT(CCTL.id) Cnt, CDA.DealerId, ''2'' AS Type
						FROM CRM_CarDealerAssignment CDA WITH(NOLOCK)
						INNER JOIN NCS_RMDealers NRD WITH(NOLOCK) ON CDA.DealerId = NRD.DealerId
						INNER JOIN CRM_CarBasicData CBD WITH(NOLOCK) ON CDA.CBDId = CBD.Id
						INNER JOIN CRM_CarTDLog CCTL WITH(NOLOCK) ON CCTL.CBDId = CBD.ID
						...
						SELECT COUNT(CCTL.id) Cnt, CDA.DealerId, ''3'' AS Type
						FROM CRM_CarDealerAssignment CDA WITH(NOLOCK)
						INNER JOIN NCS_RMDealers NRD WITH(NOLOCK) ON CDA.DealerId = NRD.DealerId
						INNER JOIN CRM_CarBasicData CBD WITH(NOLOCK) ON CDA.CBDId = CBD.Id
						INNER JOIN CRM_CarTDLog CCTL WITH(NOLOCK) ON CCTL.CBDId = CBD.ID
	...
	ELSE IF @Type = 5
		BEGIN
			...
			SET @varsql =  'SELECT COUNT(DISTINCT CCBL.CBDId) Cnt, CDA1.DealerId, ''1'' AS Type
							...
							GROUP BY CDA1.DealerId
							
							UNION ALL
							
							SELECT COUNT(DISTINCT CCBL.CBDId) Cnt, CDA1.DealerId, ''2'' AS Type
							FROM CRM_CarBookingLog AS CCBL WITH(NOLOCK), CRM_CarDealerAssignment CDA1 WITH(NOLOCK), NCS_RMDealers AS RM1 WITH(NOLOCK)
							WHERE
							CCBL.CBDId = CDA1.CBDId AND CDA1.DealerId = RM1.DealerId
							AND CCBL.IsBookingNotPossible = 1
							...
							SELECT COUNT(DISTINCT CCBL.CBDId) Cnt, CDA1.DealerId, ''3'' AS Type
							FROM CRM_CarBookingLog AS CCBL WITH(NOLOCK), CRM_CarDealerAssignment CDA1 WITH(NOLOCK), NCS_RMDealers AS RM1 WITH(NOLOCK)
							WHERE
							CCBL.CBDId = CDA1.CBDId AND CDA1.DealerId = RM1.DealerId
		...
		ELSE IF @Type = 6
		BEGIN
			SET @varsql = 'SELECT COUNT(PLC.Id) Cnt, RM.RMId, PLC.DealerId DealerId ,''1'' Type FROM CRM_PotentiallyLostCase PLC 
							INNER JOIN NCS_RMDealers AS RM WITH(NOLOCK) ON RM.DealerId = PLC.DealerId
							INNER JOIN CRM_CarDealerAssignment CDA1 WITH(NOLOCK) ON CDA1.DealerId = RM.DealerId
							...
							AND RM.RMId IN ('+@ManagerId+') AND IsActionTaken = 0 GROUP BY RM.RMId ,PLC.DealerId
							
							UNION ALL
						
							SELECT COUNT(PLC.Id) Cnt,RM.RMId , PLC.DealerId DealerId , ''2'' Type FROM CRM_PotentiallyLostCase PLC 
							INNER JOIN NCS_RMDealers AS RM WITH(NOLOCK) ON RM.DealerId = PLC.DealerId
							INNER JOIN CRM_CarDealerAssignment CDA1 WITH(NOLOCK) ON CDA1.DealerId = RM.DealerId
							INNER JOIN NCS_SubDealerOrganization AS ND WITH(NOLOCK) ON RM.DealerId = ND.DId 
						...
						ELSE
							...
							SET @varsql = @varsql + ' GROUP BY RM.RMId , PLC.DealerId
							
							UNION ALL
							
							SELECT COUNT(PLC.Id) Cnt,RM.RMId, PLC.DealerId DealerId , ''3'' Type FROM CRM_PotentiallyLostCase PLC 
							INNER JOIN NCS_RMDealers AS RM WITH(NOLOCK) ON RM.DealerId = PLC.DealerId
							INNER JOIN CRM_CarDealerAssignment CDA1 WITH(NOLOCK) ON CDA1.DealerId = RM.DealerId
							INNER JOIN NCS_SubDealerOrganization AS ND WITH(NOLOCK) ON RM.DealerId = ND.DId 
							WHERE  RM.RMId IN ('+@ManagerId+') AND IsActionTaken = 0 '
	...
	IF @Type = 1
		BEGIN
			SET @varsql = 'SELECT DISTINCT DealerId,(ND.Name + ''['' + VC.City + '']'') Dealer, NRD.RMId
			FROM NCS_RMDealers NRD WITH(NOLOCK) 
			INNER JOIN NCS_Dealers ND ON ND.ID = NRD.DealerId
			INNER JOIN vwCity VC ON ND.CityId = VC.CityId
			WHERE ND.IsActive = 1 AND RMId IN (' + @ManagerId + ')'
