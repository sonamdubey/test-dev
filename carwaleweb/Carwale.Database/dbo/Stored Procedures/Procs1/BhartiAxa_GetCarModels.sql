IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[BhartiAxa_GetCarModels]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[BhartiAxa_GetCarModels]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[BhartiAxa_GetCarModels] 
	@MakeId int,
	@modelCond int = null
AS
BEGIN
...
BEGIN
	...
	--INNER JOIN DefaultModelMapping DMM with(nolock) ON DMM.DefaultModelId = CWMV.CarModelId -- Avishkar 21-10-2015 Added for showing default model IDs
	Where CWM.CarMakeId=@MakeId and CWM.New=1
	Order by Text
END
	ELSE
BEGIN
