IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Con_EditCms_PublishArticle_v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Con_EditCms_PublishArticle_v16]
GO
...
CREATE PROCEDURE [dbo].[Con_EditCms_PublishArticle_v16.2.7]  
@BasicId INT,  
@Path VARCHAR(50),  
@AddToForum BIT,  
@CustomerId INT,  
...
BEGIN  
  ...
  DECLARE @PushNotification BIT

  SELECT @PushNotification = PushNotification FROM Con_EditCms_Basic WITH(NOLOCK) WHERE ID = @BasicId
  
  --Update road test to publish in basic table 
  UPDATE Con_EditCms_Basic  
  SET  
  IsPublished = 1, PublishedDate = GETDATE(), IsDealerFriendly =  @IsDealerFriendly --,IsNotified = 1--changed by ajay singh
  WHERE  
  ID = @BasicId 
  ...
  BEGIN
  UPDATE Con_EditCms_Basic SET IsNotified = 1
  WHERE  
  ID = @BasicId 
  END
  
  /*  commented below block by Ajay Singh on 4/5/2016 */
  
  /*--ELSE
  --BEGIN
  --UPDATE Con_EditCms_Basic SET IsNotified = 0
  --WHERE  
  --ID = @BasicId 
  --END
...
--EXEC Con_EditCms_PublishRoadTest 131
