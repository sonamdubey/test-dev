IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[CW_InsertShowroomPrices_v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[CW_InsertShowroomPrices_v16]
GO
...
-- Modified: Shalini Nair,19/08/2016 to save UpdatedBy in CW_NewCarShowroomPrices and save the LastUpdatedBy in PriceLog_ItemValues
-- Modified: Anuj Dhar, 07/10/2016 to save UpdatedBy in CW_NewCarShowroomPrices
CREATE PROCEDURE [dbo].[CW_InsertShowroomPrices_v16.8.7] @CarVersionId NUMERIC
	,-- Car Version Id
	@CityId NUMERIC
	,-- 
...
BEGIN
	...
	SELECT @Id = PQN.Id
		...
		,@LastUpdatedBy = PQN.UpdatedBy
		,@LastUpdatedOn = PQN.LastUpdated
	...
	BEGIN
		...
		BEGIN
			...
			SET CarVersionId = @CarVersionId
				...
				,UpdatedBy = @UpdatedBy -- Added by Anuj Dhar
			WHERE ID = @Id
		END
	END
	ELSE
