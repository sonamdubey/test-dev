IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[BW_GetDealerDetails_16062016]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[BW_GetDealerDetails_16062016]
GO
...
-- Modified by	:	Sumit Kate on 16 Jun 2016
-- Summary		:	Return the On Road Price for all the versions of Secondary Dealersw
-- E.g. : EXEC BW_GetDealerDetails_16062016 18995,112,1
-- Modified
-- =============================================
CREATE PROCEDURE [dbo].[BW_GetDealerDetails_16062016]
...
BEGIN	
	...
	SELECT DISTINCT
		...
		,(
			CASE 
				...
				WHEN P.Id IN (82,85) THEN 1
			END
		) AS DealerPackageTypeSort
		,(
			CASE 
				WHEN P.Id IN (84,87) THEN 1
				WHEN P.Id IN (83,86) THEN 2
				WHEN P.Id IN (82,85) THEN 3
			END
		) AS DealerPackageType
	FROM Dealers AS D WITH (NOLOCK)	
	...
	SELECT * INTO #tmpSecondaryDealer FROM cte 	

	SELECT * FROM #tmpSecondaryDealer
	ORDER BY DealerPackageTypeSort,NEWID()  -- To order the list by randomly

	--Create dealer-Version matrix of size M x N where M : secondary dealer Count and N - active version count
	SELECT
		bv.Id AS Versionid,
		d.ID AS DealerId,
		@CityId AS CityID
	INTO #dealerVersion
	FROM BikeVersions bv WITH(NOLOCK)
	CROSS JOIN #tmpSecondaryDealer d 
	WHERE bv.BikeModelId = @ModelId AND BV.IsDeleted = 0
		AND BV.New = 1

	-- Get the Dealer Prices for all the versions and all dealers
	SELECT 
		bv.DealerId,
		bv.Versionid AS BikeVersionId,
		bv.CityID,
		SUM(nsp.ItemValue) AS OnRoadPrice
	INTO #secDealerPrice
	FROM #dealerVersion bv	
	LEFT JOIN BW_NewBikeDealerShowroomPrices nsp WITH(NOLOCK) ON bv.Versionid = nsp.BikeVersionId AND bv.DealerId = nsp.DealerId AND bv.CityID = nsp.CityId
	LEFT JOIN BW_PQ_CategoryItems CI WITH (NOLOCK) ON nsp.ItemId = CI.Id AND CI.IsActive = 1
	GROUP BY bv.DealerId, bv.Versionid, bv.CityId
	
	-- Update the Onroad price with Bikewale Price where dealer price is null
	UPDATE price
	SET OnRoadPrice = ISNULL(sp.Price,0) + ISNULL(sp.RTO,0) + ISNULL(sp.Insurance,0)
	FROM #secDealerPrice AS price
	INNER JOIN [NewBikeShowroomPrices] sp WITH(NOLOCK)
	ON price.BikeVersionId = sp.BikeVersionId AND price.CityID = sp.CityId AND sp.IsActive = 1
	WHERE price.OnRoadPrice IS NULL
	
	-- Update the Onroad price with version Price where price is null
	UPDATE price
	SET OnRoadPrice = 0
	FROM #secDealerPrice AS price
	INNER JOIN BikeVersions BV WITH(NOLOCK)
	ON price.BikeVersionId = BV.Id 
	WHERE price.OnRoadPrice IS NULL
	
	SELECT * FROM #secDealerPrice

	DROP TABLE #tmpSecondaryDealer
	DROP TABLE #dealerVersion
	DROP TABLE #secDealerPrice
END
