IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[DCRM_FindContract]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[DCRM_FindContract]
GO
...
-- EXEC DCRM_FindContract -1,-1,' ',-1,0,'DealerName','ASC',null,null,null,1
-- Modified By :  Vinay Kumar Prajapati 2nd Feb 2015  
-- Purpose  : to get expired(within and above 6 month) and renewed contract
-- Modified By : Sunil M. Yadav On 17th june 2016 , contract statrtdate , endDate and dealer zone condition modified.
-- =============================================
CREATE PROCEDURE [dbo].[DCRM_FindContract] 
     @DealerId INT
	,@StateId INT
	,@CityIds VARCHAR(MAX)
	,@MakeId INT
	...
	,@DealerStatus BIT = NULL -- Sunil M. Yadav On 17th june 2016
AS
BEGIN
	...
	SELECT CCM.DealerId,DPT.TransactionId, CCM.ContractId,CCM.CampaignId AS CampaignId,D.Organization DealerName,D.IsDealerDeleted,D.TC_DealerTypeId,
	  ...
	  OU.UserName,PDS.IsThirdPartyCampaign CarTradeLeads
		,CASE 
			WHEN (
					(
						(
		...
		--		THEN ''
		--	ELSE CCM.StartDate --CONVERT(VARCHAR(10), CCM.StartDate, 103)
		--	END StartDate		
		--,CASE 
		--	WHEN CCM.EndDate IS NULL OR  CCM.EndDate ='1900-01-01 00:00:00.000'
		--		THEN ''
		--	ELSE  CCM.EndDate --CONVERT(VARCHAR(10), CCM.EndDate, 103)
		--	END EndDate
		,CCM.StartDate														
		,CCM.EndDate
		,CASE 
		...
		--Below feilds : Added by Amit Yadav on 29 Jan 2016
		...
		,S.Name AS State,DAR.Name AS Zone,C.Name AS City--To get the State,Zone,City		-- Sunil M. Yadav On 17th june 2016
		,(SELECT OU1.UserName FROM OprUsers AS OU1(NOLOCK) WHERE OU1.Id=DUD.UserId AND DUD.RoleId=3 ) AS L3Name --L3 associated to the dealer
		,(SELECT OU2.UserName FROM OprUsers AS OU2(NOLOCK) WHERE OU2.Id=DUDBE.UserId AND DUDBE.RoleId=4 ) AS BOName --Back Office executive associated to the dealer
		,(SELECT OU.UserName FROM DCRM_ADM_MappedUsers AS DAM1(NOLOCK) LEFT JOIN OprUsers OU(NOLOCK) ON OU.ID= DAM1.OprUserId WHERE DAM1.NodeRec=DAMU.NodeRec.GetAncestor(1) AND DAM1.IsActive = 1) AS L2Name--L2 associated to that L3
		,(SELECT OU.UserName FROM DCRM_ADM_MappedUsers AS DAM1(NOLOCK) LEFT JOIN OprUsers OU(NOLOCK) ON OU.ID= DAM1.OprUserId WHERE DAM1.NodeRec=DAMU.NodeRec.GetAncestor(2) AND DAM1.IsActive = 1) AS L1Name--L1 associated to that L3
        ...
        ELSE  NULL  END  AS CostPerLead
		 ------------------------------------------------------------------------------
		  -- Added By Vinay Kumar Prajapati 3rd Feb 2016		
          -- @ContractStatus: 5 - Expired  Above 6 month,
		  -- @ContractStatus: 7 - Expired not renewed within 6 month,
		  -- @ContractStatus: 8 - Renewed 
		,CASE @ContractStatus
			 WHEN 5 THEN (SELECT TOP 1  (CASE WHEN ISNULL(DATEDIFF(DAY,  CCM.EndDate, STCM.StartDate),0) >  180 THEN 1 ELSE 0 END) 
						   FROM TC_ContractCampaignMapping AS STCM WITH(NOLOCK)
						   INNER JOIN PQ_DealerSponsored PDSS WITH (NOLOCK) ON PDSS.Id = STCM.CampaignId
			               WHERE STCM.StartDate >=CCM.EndDate AND STCM.DealerId= CCM.DealerId  AND STCM.ContractStatus=1  
						   ORDER BY STCM.StartDate)   
			 WHEN 7 THEN (SELECT TOP 1  (CASE WHEN ( ISNULL(DATEDIFF(DAY,  CCM.EndDate, STCM.StartDate),0) <=  180 AND ISNULL(DATEDIFF(DAY,  CCM.EndDate, STCM.StartDate),0) > 0)  THEN 0 ELSE 1 END)
						  FROM TC_ContractCampaignMapping AS STCM WITH(NOLOCK) 
						  INNER JOIN PQ_DealerSponsored PDSS WITH (NOLOCK) ON PDSS.Id = STCM.CampaignId
						  WHERE STCM.StartDate >=CCM.EndDate AND STCM.DealerId= CCM.DealerId  AND STCM.ContractStatus=1  ORDER BY STCM.StartDate)
			 WHEN 8 THEN (SELECT TOP 1  (CASE WHEN (ISNULL(DATEDIFF(DAY, STCM.EndDate, CCM.StartDate),0) <=  180 AND ISNULL(DATEDIFF(DAY, STCM.EndDate, CCM.StartDate),0) > 0) THEN 1 ELSE 0 END)
			              FROM TC_ContractCampaignMapping AS STCM WITH(NOLOCK) 
						   INNER JOIN PQ_DealerSponsored PDSS WITH (NOLOCK) ON PDSS.Id = STCM.CampaignId
						  WHERE STCM.ContractStatus=3 AND STCM.DealerId= CCM.DealerId  ORDER BY STCM.EndDate DESC) 
   ...
   LEFT JOIN DCRM_ADM_UserDealers AS DUDBE WITH(NOLOCK) ON DUDBE.DealerId= D.ID AND DUDBE.RoleId = 4
   ...
   LEFT JOIN DCRM_ADM_MappedUsers AS DAMU WITH(NOLOCK) ON DAMU.OprUserId=DUD.UserId AND DAMU.IsActive = 1
   LEFT JOIN DCRM_ADM_RegionCities DARC WITH(NOLOCK) ON DARC.CityId = D.CityId
   LEFT JOIN DCRM_ADM_Regions DAR WITH(NOLOCK) ON DAR.Id = DARC.RegionId AND DAR.IsActive = 1 -- Sunil M. Yadav On 17th june 2016
   ...
   WHERE
   	  (( CCM.ContractStatus = @ContractStatus OR @ContractStatus = 0) -- Get All Data for @ContractStatus =0 
				 OR (	
					((@ContractStatus = 5 OR @ContractStatus = 7) AND CCM.ContractStatus = 3 ) OR -- get All Expired Contract data for status 5 or 7
						((@ContractStatus = 6 AND CCM.ContractStatus = 1)  --- get All active data for status 6(Renewed)
						AND (
   ...
   AND (@DealerStatus IS NULL  OR D.Status = @DealerStatus)  -- Sunil M. Yadav On 17th june 2016
	ORDER BY CASE 
    ...
    -- Added By Vinay Kumar prajapati 
    -- @ContractStatus: 5 - Expired  Above 6 month,
	-- @ContractStatus: 7 - Expired not renewed within 6 month,
	-- @ContractStatus: 8 - Renewed 
	...
	IF @ContractStatus =5 
		BEGIN
			SELECT * FROM #TempTable AS TT WHERE  (TT.DefinitionStatus = 1  OR (TT.DefinitionStatus IS NULL AND DATEDIFF(DAY,TT.EndDate,GETDATE()) > 180 ))
		END
	ELSE IF  @ContractStatus =7
		BEGIN	 
			SELECT * FROM #TempTable AS TT WHERE  (TT.DefinitionStatus = 1   OR (TT.DefinitionStatus IS NULL AND (ISNULL(DATEDIFF(DAY,TT.EndDate,GETDATE()),0) <= 180 AND ISNULL(DATEDIFF(DAY,TT.EndDate,GETDATE()),0) > 0)))
		END 
