IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Con_EditCms_Images_SaveRQ_new_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Con_EditCms_Images_SaveRQ_new_v15]
GO
...
CREATE PROCEDURE [dbo].[Con_EditCms_Images_SaveRQ_new_v15.8.1] @ID INT
	,@BasicId INT
	,@ImageCategoryId INT
	,@Caption VARCHAR(250)
	,@LastUpdatedBy INT
...
BEGIN
	BEGIN TRY
		...
		BEGIN
			...
			UPDATE Con_EditCms_Images
			SET OriginalImgPath = @ImagePath + @ImageName + '-' + CONVERT(VARCHAR, @ImageId) + '.jpg?wm=' + CONVERT(varchar, ISNULL(@IsWatermark, 0))
			WHERE Id = @ImageId
		END
		ELSE
			IF @ID <> - 1
