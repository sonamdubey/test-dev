IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Classified_SellersOffering]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Classified_SellersOffering]
GO
...
CREATE PROCEDURE [dbo].[Classified_SellersOffering]

	-- Input Parameter
	@ID NUMERIC(18,0)
	AS
	BEGIN 
		SELECT cv.TC_CarValueAdditionsId, cv.ValueAddName 
