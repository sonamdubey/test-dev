IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Absure_SaveCarSurveyorMappingTest]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Absure_SaveCarSurveyorMappingTest]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[Absure_SaveCarSurveyorMappingTest] 
	@TC_UserId				BIGINT,
...
BEGIN
		...
		BEGIN
			...
			SELECT	@BranchId = ISNULL(@BranchId,DealerId),@StockId = StockId,
