IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[ClientMissedCallVerification_16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[ClientMissedCallVerification_16]
GO
...
-- Modified by Shubham Agarwal on 09/08/2016 - Changed column name IsSentToAutobiz to IsSentToSource, Changed orderby createdTime to Id in first query
-- Modified by Shubham Agarwal on 04/10/2016 - Removed dependency on ClassifiedLeads
-- =============================================
CREATE PROCEDURE [dbo].[ClientMissedCallVerification_16.9.1] 
