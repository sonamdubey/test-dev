IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[CW_GetUserEligibleAmount_V16_10_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[CW_GetUserEligibleAmount_V16_10_1]
GO
...
-- Modified : on 4 Oct 2016, added condition for customer stability and experience 
-- =============================================
CREATE PROCEDURE [dbo].[CW_GetUserEligibleAmount_V16_10_1]
    @FinanceLeadId INT = -1,
	@IncomeTypeId INT,
...
WHERE   M.CW_IncomeTypeId=@IncomeTypeId 
AND M.CW_AgeGroupId=@CW_AgeGroupId 
AND M.MinIncome<=@AnnualIncome 
AND M.CW_CarTierId=@CarTier  
AND M.IsActive=1
AND M.ResidenceTypeId = @ResidenceTypeId
AND R.MinStability <= @StabilityTime
AND I.Minexperience <= @CustomerExp
