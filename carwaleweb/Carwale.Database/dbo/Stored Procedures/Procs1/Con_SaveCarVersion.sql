IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Con_SaveCarVersion]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Con_SaveCarVersion]
GO
...
-- exec [dbo].[Con_SaveCarVersion] 4042,'35 TDI Premium + Sunroof','35-tdi-premium-sunroof',552,2,1,2,1,1,1,1,0,0,0,0,0,11,GETDATE(),3,null,0,getdate(),null,null
-- exec [dbo].[Con_SaveCarVersion] 3620,'35 TDI Premium','35 TDI Premium',552,2,1,null,null,1,1,1,0,0,0,1,0,null,null,3,null,null,null,null,null
CREATE PROCEDURE [dbo].[Con_SaveCarVersion] @ID NUMERIC
...
declare  @Discontinuation_date datetime =null,@ReplacedByVersionId smallint = null, @VUpdatedBy numeric = null, @VUpdatedOn datetime = null, @CarFuelType tinyint = null, @Environment varchar(150) =null , @CarTransmission tinyint = null
	...
	BEGIN
		PRINT 'Delete Car'
		UPDATE CarVersions SET IsDeleted = 1 ,VUpdatedOn = getdate(), VUpdatedBy = @UpdatedBy WHERE ID = @ID
		...
		BEGIN CATCH
			...
			VALUES('MysqlSync','Con_SaveCarVersion',ERROR_MESSAGE(),'SyncCarVersionsWithMysqlUpdate',@Id,GETDATE(),@UpdateType)
		END CATCH					
	--	EXEC [ac].[UpdateKeywordsByVersionId] @ID,3 -- -- -- Commented by Manish on 25-07-2014
	END
	ELSE
	BEGIN         
	IF @ID = - 1
	BEGIN
		...
		--SET @CV_ID = @@IDENTITY         -- Commented by Manish on 07-10-2014
		...
		BEGIN CATCH
			INSERT INTO CarWaleMyslSyncExceptions(ModuleName,SPName,ErrorMsg,TableName,FailedId,CreatedOn,InputParameter)
			VALUES('MysqlSync','Con_SaveCarVersion',ERROR_MESSAGE(),'SyncCarVersionsWithMysql',@cv_Id,GETDATE(),null)
		END CATCH		
		--print @CV_ID
		-- AC DATA SYNCING BEGIN
	--	EXEC [ac].[UpdateKeywordsByVersionId] @CV_ID,1     -- -- Commented by Manish on 25-07-2014
		-- AC DATA SYNCING END
	...
	BEGIN
		...
		END CATCH		
		-- AC DATA SYNCING BEGIN
	--	EXEC [ac].[UpdateKeywordsByVersionId] @ID,2       -- -- Commented by Manish on 25-07-2014
		-- AC DATA SYNCING END
	...
	BEGIN CATCH
		INSERT INTO CarWaleMyslSyncExceptions(ModuleName,SPName,ErrorMsg,TableName,FailedId,CreatedOn,InputParameter)
		VALUES('MysqlSync','Con_SaveCarVersion',ERROR_MESSAGE(),'SyncCarVersionsWithMysqlUpdate',@CarModelId,GETDATE(),@UpdateType)
	END CATCH			
	END
END
