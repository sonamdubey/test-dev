IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[AP_UnpaidMailerTrackGetInsertBKP]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[AP_UnpaidMailerTrackGetInsertBKP]
GO
...
CREATE PROCEDURE [dbo].[AP_UnpaidMailerTrackGetInsertBKP]	
AS
BEGIN
	SET NOCOUNT ON
