IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[BW_GetDealerPriceQuote]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[BW_GetDealerPriceQuote]
GO
...
-- Modified By	:	Sangram Nandkhileon 05 Oct 2015 : Added Join and column to check if offer has view terms or not
-- Modified By	:	Sumit Kate on 08 Oct 2015
-- Summary		:	Return the On Road Price For Other varients
-- Modified By	:	Sushil Kumar on 09 Nov 2015
-- Summary		:	Return the Booking amount for each varient
---------------------------------------------------------
CREATE PROCEDURE [dbo].[BW_GetDealerPriceQuote] 
	@CityId INT
	,@VersionId INT
	,@DealerId INT = NULL
AS
