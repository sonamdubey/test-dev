IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[CW_SaveFinanceLeads_v16_10_4]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[CW_SaveFinanceLeads_v16_10_4]
GO
...
-- =============================================
create PROCEDURE [dbo].[CW_SaveFinanceLeads_v16_10_4]
...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT Id FROM CW_FinanceLeads WITH(NOLOCK) WHERE Mobile like @Mobile and EntryDateTime between DateAdd(DD,-7,GETDATE()) and GETDATE())
		BEGIN
        	SET @PushToClient = 1
        END
        ELSE
