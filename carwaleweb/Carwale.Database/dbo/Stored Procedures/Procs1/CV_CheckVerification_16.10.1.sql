IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[CV_CheckVerification_16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[CV_CheckVerification_16]
GO
...
-- Modified by Shubham Agarwal on 09/08/2016 - Changed column name IsSentToAutobiz to IsSentToSource
-- Modified by Shubham Agarwal on 05/10/2016 - Removed classifiedLeads dependency
CREATE PROCEDURE [dbo].[CV_CheckVerification_16.10.1] @MobileNo AS VARCHAR(50)
