IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Classified_SellInquiries_partial_16_10_4]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Classified_SellInquiries_partial_16_10_4]
GO
...
-- Added by Prachi Phalak column ShareToCT to CustomerSellInquiries table
CREATE PROCEDURE [dbo].[Classified_SellInquiries_partial_16_10_4]        
...
--Added by Aditi Dhaybar on 3/2/2015 to add AreaId 
 ...
 --Aditi Dhaybar Added AreaId on 3/2/2015    
  ...
  CustomerName,CustomerEmail,CustomerMobile,AreaId,IsShowContact,ShareToCT)            
  ...
  @CustomerName,@CustomerEmail,@CustomerMobile,@AreaId,@ShowContactDetails,@ShareToCT)  
 ...
 ELSE        
  ...
  SET CityId = @CityId, 
	  ...
	  ShareToCT = @ShareToCT -- Added column ShareToCT to CustomerSellInquiries table
