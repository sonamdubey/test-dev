IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[InsertUsedCarPurchaseInquiry]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[InsertUsedCarPurchaseInquiry]
GO
...
-- Added by Aditi Dhaybar on 24/09/2014 for the LeadTracking Source Id 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  
    
--THIS PROCEDURE IS FOR INSERTING AND UPDATING RECORDS FOR SellInquiries TABLE    
    
CREATE PROCEDURE   [dbo].[InsertUsedCarPurchaseInquiry]    
 ...
 BEGIN    
  ...
  BEGIN      
    ...
    INSERT INTO UsedCarPurchaseInquiries(CustomerId, SellInquiryId,     
     ...
     NoOfCars, BuyTime, CarModelIds, CarModelNames, Comments, RequestDateTime, IsApproved, SourceId,IPAddress,LTSrc)    
...
/****** Object:  StoredProcedure [dbo].[FetchCustomerDetails]    Script Date: 10/3/2014 11:44:48 AM ******/
SET ANSI_NULLS OFF
