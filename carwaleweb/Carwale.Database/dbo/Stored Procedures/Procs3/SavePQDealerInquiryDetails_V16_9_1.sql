IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[SavePQDealerInquiryDetails_V16_9_1]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[SavePQDealerInquiryDetails_V16_9_1]
GO
...
-- MOdified By : Vinayak Mishra added column ab test 3/10/16
-- =============================================
create PROCEDURE [dbo].[SavePQDealerInquiryDetails_V16_9_1]
...
BEGIN
	...
	BEGIN
		INSERT INTO PQDealerAdLeads (
			...
			,Browser 
			,OperatingSystem
			,ABTest --added column ab test 3/10/16
			)
		VALUES (
			@PQId
			,@DealerId
			...
			,@Browser 
			,@OperatingSystem
			,@ABTest -- added abtest to pqdealeradleads 3/10/16
			)
