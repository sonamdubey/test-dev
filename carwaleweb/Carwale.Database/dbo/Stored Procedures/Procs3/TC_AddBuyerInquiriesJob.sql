IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_AddBuyerInquiriesJob]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_AddBuyerInquiriesJob]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[TC_AddBuyerInquiriesJob]          
@BranchId Numeric,
@StockId BIGINT,
...
BEGIN		
	...
	Declare @TC_TimeDuration_Id int
	...
	BEGIN TRY
		BEGIN TRANSACTION TranA
			...
			EXECUTE TC_AddTCInquiriesJob @CustomerName,@Email,@Mobile,@Location,@Buytime,@CustomerComments,@BranchId,@VersionId,@Comments,@InquiryStatus,@NextFollowup,@AssignedTo,1,@InquirySource,@UserId,@TC_InquiriesId OUTPUT,1,@CreatedDate
