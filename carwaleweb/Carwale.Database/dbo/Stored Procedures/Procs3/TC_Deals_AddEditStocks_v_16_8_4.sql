IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_Deals_AddEditStocks_v_16_8_4]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_Deals_AddEditStocks_v_16_8_4]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[TC_Deals_AddEditStocks_v_16_8_4] 
	@BranchId INT, 
	@CarVersionId INT, 
	@MakeYear DATETIME, 
	@VersionColorId INT,
	...
	@OnRoadPrices [TC_Deals_CarStockPrices] READONLY, 
...
BEGIN
	...
	BEGIN
		DECLARE @VINId INT, @ExistingOffer VARCHAR(500), @ExistingInteriorColor VARCHAR(50), @ExistingTermsConditions VARCHAR(500), @ExistingOfferValue INT,
				@IsPriceChanged INT, @IsVINChanged INT, @IsSKUApproved BIT, @IsOfferColorChanged BIT, @PriceUpdated BIT;
		...
		IF @DealsStockId IS NULL
			...
			INSERT INTO TC_Deals_Stock (BranchId, CarVersionId, MakeYear, VersionColorId, InteriorColor, EnteredOn, EnteredBy, 
											LastUpdatedOn, LastUpdatedBy, Offers, TermsConditions, isApproved, PriceUpdated)
			VALUES (@BranchId, @CarVersionId, @MakeYear, @VersionColorId, @InteriorColor, GETDATE(), @UserId, GETDATE(), @UserId, @Offers,@TermsAndConditions, 0, @IsPriceUpdated)
			...
			INSERT INTO TC_Deals_StockPrices (TC_Deals_StockId, CityId, DiscountedPrice, ActualOnroadPrice, Offer_Value, EnteredOn, EnteredBy)
			SELECT @RetVal, CityId, OfferPrice, OnRoadPrice, OfferValue, GETDATE(), @UserId FROM @OnRoadPrices
		...
		ELSE -- Existing SKU/ Edit Stock
			...
			DECLARE @tblPriceChanged TABLE( CityId INT,OnRoadPrice INT,OfferPrice INT, OfferValue INT)
			...
			ELSE 
				BEGIN
					SELECT CityId, DiscountedPrice, ActualOnroadPrice, Offer_Value FROM TC_Deals_StockPrices WITH (NOLOCK) WHERE TC_Deals_StockId = @DealsStockId 
					EXCEPT SELECT CityId, OfferPrice,OnRoadPrice, OfferValue FROM @OnRoadPrices
			...
			IF @IsPriceChanged  = 1
				BEGIN
					...
					UPDATE SP SET SP.ActualOnroadPrice = P.OnRoadPrice , SP.DiscountedPrice = P.OfferPrice, SP.Offer_Value = P.OfferValue
					...
					INSERT INTO TC_Deals_StockPrices (TC_Deals_StockId, CityId, DiscountedPrice, ActualOnroadPrice, Offer_Value, EnteredBy,EnteredOn)
					SELECT DISTINCT @DealsStockId, TBL.CityId,  TBL.OfferPrice,  TBL.OnRoadPrice, TBL.OfferValue, @UserId, GETDATE() FROM @OnRoadPrices TBL 
				...
				BEGIN
					...
					BEGIN
						...
						SET InteriorColor = @InteriorColor, EnteredBy = @UserId, LastUpdatedOn = GETDATE(), LastUpdatedBy = @UserId, 
							Offers = @Offers , TermsConditions = @TermsAndConditions, PriceUpdated = @IsPriceUpdated
						WHERE Id = @DealsStockId
