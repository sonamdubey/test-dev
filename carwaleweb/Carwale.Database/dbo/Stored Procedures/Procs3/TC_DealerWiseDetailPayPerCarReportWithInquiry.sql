IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_DealerWiseDetailPayPerCarReportWithInquiry]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_DealerWiseDetailPayPerCarReportWithInquiry]
GO
...
-- -- =============================================
CREATE PROCEDURE [dbo].[TC_DealerWiseDetailPayPerCarReportWithInquiry]
@Month TINYINT,
@YEAR  SMALLINT,
...
WITH CTE AS (
				...
				ISNULL ([No. of Cars Removed during the day(D)],0) AS [No. of Cars Removed during the day(D)],
				 TotalInquiryReceivedTillDate,
		         TotalInquiryReceivedYesterday
				...
				ISNULL(COUNT(DISTINCT (LL.Inquiryid)),0) [No. of Duplicate Car Uploaded],
				   ...
				   JOIN SellInquiries AS SEL WITH (NOLOCK) ON SEL.ID=LL.Inquiryid AND SI.DealerId=SEL.DealerId
			...
			LEFT JOIN 
			     (SELECT d.id AS DealerId, 
				         COUNT(DISTINCT UCPI.ID) TotalInquiryReceivedTillDate,
				          COUNT(DISTINCT(CASE WHEN CONVERT(DATE,UCPI.RequestDateTime)=CONVERT(DATE,GETDATE()-1) THEN  UCPI.ID END )) TotalInquiryReceivedYesterday   
				      FROM  
