IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[Microsites_InsertImageUploadData]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[Microsites_InsertImageUploadData]
GO
...
-- =============================================				  
CREATE PROCEDURE [dbo].[Microsites_InsertImageUploadData] 
...
BEGIN
	...
	if(@IsBanner=1)
		begin
			set @BannerSorting = (select COUNT(id) from Microsite_Images where DealerId=@DealerId and IsBanner=1 and IsActive=1)
		end
	else
		begin
    ...
    -- Insert statements for procedure here
		...
		DirectoryPath,HostURL,IsBanner,BannerImgSortingOrder,LandingURL, ImageCategoryId, ImgPathCustom600, ImgPathCustom300,OriginalImgPath) values 
		(@DealerId,@ThumbImage,@LargeImage,@EntryDate,@DirectoryPath,@HostURL,
		@IsBanner,@BannerSorting,@LandingBanner, @ImageCategoryId, @ImgPathCustom600, @ImgPathCustom300,@OriginalImgPath )
