IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NewRecommendCars_NEWBKP]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NewRecommendCars_NEWBKP]
GO
...
EXEC [dbo].[NewRecommendCars_NEWBKP] 1,100,10,1000000,30,'',900,9,4,1,8,5,3,6,2,7,'',0,0,0,0,0,0,1,@CarCount OUTPUT,@MakeidsOut OUTPUT
SELECT @CarCount,@MakeidsOut
*/
CREATE PROCEDURE [dbo].[NewRecommendCars_NEWBKP]
