IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_CustomerDetailSave_V16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_CustomerDetailSave_V16]
GO
...
-- Modified By  :	Ashwini Dhamankar on Oct 12,2016 (Added parameter @City)
-- =============================================
create PROCEDURE [dbo].[TC_CustomerDetailSave_V16.10.1] 
	@BranchId INT
			...
			ELSE
				BEGIN
					...
					INSERT INTO TC_CustomerDetails(CustomerName,Mobile,AlternateNumber,Email,BranchId,Location,Buytime,Comments,CreatedBy
								,Address,TC_InquirySourceId,CW_CustomerId,Salutation,LastName,TC_CampaignSchedulingId,City) 
					VALUES (@CustomerName,@CustomerMobile,@CustomerAltMob,@CustomerEmail,@BranchId,@Location,@Buytime,@Comments,@CreatedBy
								,@Address,@SourceId,@CW_CustomerId,@Salutation,@LastName,@TC_CampaignSchedulingId,@City)
					...
					INSERT INTO TC_CustomerDetailsLog(CustomerId, CustomerName,Mobile,AlternateNumber,Email,BranchId,Location,Buytime,Comments,CreatedBy
								,Address,TC_InquirySourceId,CW_CustomerId,Salutation,LastName,City) 
					VALUES (@CustomerId, @CustomerName,@CustomerMobile,@CustomerAltMob,@CustomerEmail,@BranchId,@Location,@Buytime,@Comments,@CreatedBy
								,@Address,@SourceId,@CW_CustomerId,@Salutation,@LastName,@City)
