IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[ProcessSellersInquiries]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[ProcessSellersInquiries]
GO

	CREATE procedure [dbo].[ProcessSellersInquiries]
(
@DealerId BIGINT
)
as
...
begin
	...
	@InquirySource SMALLINT,	@profileId varchar(10)	
	...
	@Comments=csi.Comments,	@InquiryStatus=CSI.StatusId	
	From AP_DealerPackageInquiries  AS DPI 
