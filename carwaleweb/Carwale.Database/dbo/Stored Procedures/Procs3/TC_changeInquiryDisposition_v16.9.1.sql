IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[TC_changeInquiryDisposition_v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[TC_changeInquiryDisposition_v16]
GO
...
-- Modified by : Khushaboo patil on 30th sept 2016 close lead for feedback call.
-- Modified by : Nilima More On 30th Sept 2016,handled feedback multiple inquiries condition
-- exec [TC_changeInquiryDisposition_v16.9.1] 17,138,8,null,null,null,null,null
-- =============================================  
Create PROCEDURE  [dbo].[TC_changeInquiryDisposition_v16.9.1]
...
BEGIN
	...
	-- modified by : Khushaboo patil on 30th sept 2016 close lead for feedback call
	IF @InquiryType = 8--Feedbackcall Inquiries
	BEGIN
		
			SELECT @InquiriesLeadId =  TC_InquiriesLeadId FROM TC_FeedbackCalling_Inquiries WITH (NOLOCK) WHERE TC_FeedbackCalling_InquiriesId = @InquiryId
			SELECT @LeadId = TC_LeadId, @LeadOwnerId = TC_UserId FROM TC_InquiriesLead WITH (NOLOCK) WHERE TC_InquiriesLeadId = @InquiriesLeadId  AND IsActive = 1
			IF @TC_UserId IS NULL 
			BEGIN
			SELECT @UserId = TC_UserId FROM TC_InquiriesLead WITH (NOLOCK) WHERE TC_InquiriesLeadId = @InquiriesLeadId AND IsActive = 1
			...
			SET TC_LeadDispositionId = @Disposition 
			WHERE TC_FeedbackCalling_InquiriesId  =@InquiryId
			
			SELECT @FeedbackCount = COUNT(TC_FeedbackCalling_InquiriesId) FROM TC_FeedbackCalling_Inquiries WITH (NOLOCK) 
			WHERE (TC_LeadDispositionId IS NULL)  
			AND TC_InquiriesLeadId = @InquiriesLeadId 

			IF @FeedbackCount = 0 -- if feedback inquiries are not prestent then dispose new car lead
				BEGIN
					IF EXISTS(SELECT TC_InquiriesLeadId FROM TC_InquiriesLead WITH (NOLOCK) WHERE TC_InquiriesLeadId = @InquiriesLeadId AND TC_LeadDispositionId IS NULL  AND IsActive = 1)
						BEGIN
							UPDATE TC_InquiriesLead SET TC_LeadStageId = 3, TC_LeadDispositionId = @Disposition WHERE TC_InquiriesLeadId = @InquiriesLeadId  AND IsActive = 1
			...
			ELSE
				BEGIN
					EXEC TC_DispositionLogInsert @UserId,@Disposition,@InquiryId,8,@LeadId,NULL,@DispositionReason,@SubDisposition 
					
					SET @CarDetailUpdateRequired=0
				END
				
			
			SELECT @InquiriesLeadCount = COUNT(IL.TC_InquiriesLeadId) FROM  TC_InquiriesLead	 AS IL	 WITH(NOLOCK) 
																	  JOIN  TC_FeedbackCalling_Inquiries AS TFI WITH (NOLOCK) 
																	  ON    IL.TC_InquiriesLeadId = TFI.TC_InquiriesLeadId 
																	  WHERE IL.TC_LeadId = @LeadId AND (TFI.TC_LeadDispositionID IS NULL) AND IL.IsActive = 1
			
			IF @InquiriesLeadCount = 0 AND @FeedbackCount = 0 -- If  Inquiries and leads are not present then dispose tc_Lead
				BEGIN
					UPDATE TC_Lead SET TC_LeadStageId = 3, LeadClosedDate = GETDATE() WHERE TC_LeadId = @LeadId
					UPDATE TC_CustomerDetails SET ActiveLeadId = NULL, IsleadActive = 0 
					WHERE ActiveLeadId = @LeadId
					...
					SET @ActiveCallDeleteRequired=1 -- Active Call deletion required
				END
			ELSE IF @InquiriesLeadCount <> 0 AND @FeedbackCount = 0 
				BEGIN
					EXEC TC_DispositionLogInsert @UserId,@Disposition,@InquiriesLeadId,8,@LeadId,NULL,@DispositionReason,@SubDisposition 
				END
