IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NewRecommendCars_16_7_2]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NewRecommendCars_16_7_2]
GO
...
*/
CREATE PROCEDURE [dbo].[NewRecommendCars_16_7_2]
...
BEGIN
	...
	FROM RecommendCars RC WITH(NOLOCK)
		...
		LEFT JOIN Con_NewCarNationalPrices NCP WITH(NOLOCK) ON RC.Versionid = NCP.VersionId
...
*/		
	...
	,PriceMax
	,PriceMin
	,T.MinAvgPrice AS ModelMinAvgPrice
	,BodyStyleId
	,CMO.MaskingName
	FROM @Models T
	...
	T1.Modelid,T1.Versionid,CM.Name Makename,CMO.Name Modelname,CV.Name Versionname
			,T1.AvgPrice,T1.SpecsSummary,T1.SortScore,T1.FinalScore,CMO.MaskingName
	FROM @Models T2
	...
	WHERE T1.SortScore >= @CutOffScore
	ORDER BY T1.SortScore DESC,T1.AvgPrice DESC
