IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NCS_FAAddRates]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NCS_FAAddRates]
GO

	


CREATE PROCEDURE [dbo].[NCS_FAAddRates]
	@Id				NUMERIC,
	@FAID			NUMERIC,
	@ZoneId			NUMERIC,
	@GroupId		NUMERIC,
