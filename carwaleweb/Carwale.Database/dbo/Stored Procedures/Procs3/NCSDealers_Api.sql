IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NCSDealers_Api]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NCSDealers_Api]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[NCSDealers_Api] --[dbo].[NCSDealers_Api] 9,1
	-- Add the parameters for the stored procedure here
		@MakeId INT,
		@CityId INT
AS
BEGIN
    SELECT DISTINCT NCSD.ID AS ID, ISNULL(NCSD.AreaName  + ' - ','') + NCSD.DealerTitle As DealerName ,NCSD.Address As Area -- Modified By - Vinayak 7th oct 2014, Added AreaName in select statement'
