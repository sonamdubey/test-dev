...
-- Modified By Rakesh Yadav On 18 Jan 2016,Retrieved Displaydate
-- Modified By Ajay Singh on 27 june 2016,order by display date as replacement of oredr by basicid
-- =============================================
CREATE PROCEDURE [dbo].[RetrieveVideosByModel_15.9.5] --EXEC [dbo].[RetrieveVideosByModel_14.11.3] 126,1
...
BEGIN
	...
	BEGIN
		...
		FROM (
			...
			ORDER BY CB.DisplayDate DESC --added by Ajay Singh on 27 june 2016
			) AS T
		WHERE (
				(
		...
		BEGIN
			...
			FROM (
				SELECT DISTINCT TOP (1000) CV.basicid
					...
					,CS.NAME AS SubCatName
					,Row_number() OVER (
						Partition BY CC.BasicId
						ORDER BY CC.BasicId DESC
						) AS RowNo
					,CC.MakeId AS MakeId
					,CC.ModelId AS ModelId
					,CI.HostURL AS HostUrl --Modified by Ashwini Todkar
