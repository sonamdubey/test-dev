IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[InsertUsedCarPurchaseInquiry_15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[InsertUsedCarPurchaseInquiry_15]
GO
...
CREATE  PROCEDURE   [dbo].[InsertUsedCarPurchaseInquiry_15.10.5]    
 ...
 BEGIN    
  ...
  BEGIN      
    ...
    INSERT INTO UsedCarPurchaseInquiries(CustomerId, SellInquiryId,     
     ...
     NoOfCars, BuyTime, CarModelIds, CarModelNames, Comments, RequestDateTime, IsApproved, SourceId,IPAddress,LTSrc,Cwc,UtmaCookie,UtmzCookie)    
