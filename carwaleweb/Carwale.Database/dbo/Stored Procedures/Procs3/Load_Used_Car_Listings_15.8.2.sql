...
-- Modified by Sahil Sharma for Dealer of the Month Enhancement on used landing page
-- =============================================
CREATE PROCEDURE [dbo].[Load_Used_Car_Listings_15.8.2]
...
BEGIN
	...
	--Added by Sahil Sharma for Dealer of the Month Enhancement on used landing page
	SELECT TOP (9) LL.MakeName, 
			LL.ModelName, 
			CMO.MaskingName,
			LL.VersionName, 
			LL.ProfileId, 
			...
			LL.OriginalImgPath, 
			LL.HostURL,
			CFT.Descr CarFuelType
	FROM LiveListings LL WITH(NOLOCK)
	INNER JOIN CarModels CMO WITH(NOLOCK) on LL.ModelId=CMO.ID
	INNER JOIN CARVERSIONS CV WITH(NOLOCK) on LL.VersionId=CV.ID
	INNER JOIN CarFuelTypes CFT WITH(NOLOCK) on CV.CarFuelType=CFT.CarFuelTypeId
	WHERE  LL.SellerType=1
	AND LL.ShowDetails = 1
	AND  LL.OriginalImgPath IS NOT NULL
	AND LL.DealerId = @DealerId
	ORDER BY LL.EntryDate DESC

	SELECT COUNT(Inquiryid) CountDealerOfTheMonthCars
