IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[NewRecommendCars_07JAN2014]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[NewRecommendCars_07JAN2014]
GO
...
EXEC [dbo].[NewRecommendCars] 1,100,10,1000000,30,'',900,9,4,1,8,5,3,6,2,7,'',0,0,0,0,0,0,1,@CarCount OUTPUT,@MakeidsOut OUTPUT
SELECT @CarCount,@MakeidsOut
*/
CREATE PROCEDURE [dbo].[NewRecommendCars_07JAN2014]
...
*/		
	...
	SELECT T.ModelID, CMA.Name MkName,CMO.Name MoName,'http://'+CMO.HostURL+'/cars/'+CMO.LargePic img
	,CMO.Summary
	,VCount
	,Score
	,PriceMax
	...
	SELECT --T1.*
	T1.Modelid,T1.Versionid,T1.Makename,T1.Modelname,T1.Versionname
			,T1.NCPPrice,T1.SpecsSummary,T1.SortScore,T1.FinalScore
	FROM #CarData T1
	RIGHT JOIN @Models T2
