IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[PQ_GetDealerSponsorship_V14]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [dbo].[PQ_GetDealerSponsorship_V14]
GO
...
----modified by vinayak <31/10/2014> display of ads based on counts and type.
-- =============================================
CREATE PROCEDURE [dbo].[PQ_GetDealerSponsorship_V14.10.2.2] --[dbo].[PQ_GetDealerSponsorship_V14.10.2.2] 409,1,1
...
BEGIN
	...
	and ((PDS.TotalCount<PDS.TotalGoal and PDS.DailyCount<PDS.DailyGoal) or PDS.Type <> 2)--modified by vinayak
