...
SELECT *
   FROM sys.views
     ...
     name = 'vwTC_InquiriesForExcel' AND
     DROP VIEW dbo.vwTC_InquiriesForExcel
GO
...
-- Modified By: Nilesh Utture On 31st October, 2012 Added Field in View named IL.LastFollowUpComment              
-- ====================================================================================         
CREATE VIEW [dbo].[vwTC_InquiriesForExcel] AS           
...
SELECT CD.CustomerName,        
       ...
       INQ.InquiryType AS InquiryTypeId,       
       U.UserName,           
       ST.Status as Status,           
       INQ.SourceId,           
       it.InquiryType,      
      ...
      COALESCE(SINQ.Id,SI.CWInquiryId) AS profileid,          
       ...
       IL.InquiryCount AS InqCount,  
       (CASE ISNULL(SI.Owners,'')   
  WHEN '' THEN '' ELSE dbo.TC_GetOwnerValue(SI.Owners)END)AS OwnerType,  
       --COALESCE(dbo.TC_GetOwnerValue(SI.Owners),dbo.TC_GetOwnerValue(CC.Owners)) AS OwnerType,  
       --NULL AS regno,           
       --NULL AS makeyear,           
       --NULL As colour,           
       --NULL AS price,           
...
FROM   TC_Inquiries INQ WITH(NOLOCK)           
       ...
       LEFT OUTER JOIN TC_CarCondition CC WITH(NOLOCK)  ON  CC.StockId= STO.Id    
