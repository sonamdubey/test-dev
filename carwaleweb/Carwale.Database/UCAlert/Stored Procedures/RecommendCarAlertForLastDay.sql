IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[UCAlert].[RecommendCarAlertForLastDay]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [UCAlert].[RecommendCarAlertForLastDay]
GO
...
-- Description : We are taking all the customer who inquired one car last day and not inquired any car in seven days before last day.
-- After taking the inquiry details of the customer we are executing algorithm and find the recommended car.
...
-- Modified By: Manish on 02-01-2013 capturing alert freuency is 3(for automated mail).
...
-- Modified by: Manish on 23-04-2014 for addition of the column IsFirstmail in UCAlert.RecommendUsedCarAlert table 
-- Modified by : Kundan on 21-03-2016  changed column refernce FrontImagePath to  OriginalImgPath
-- =============================================
CREATE PROCEDURE [UCAlert].[RecommendCarAlertForLastDay]
	-- Add the parameters for the stored procedure here
AS
BEGIN

     TRUNCATE TABLE UCAlert.RecommendUsedCarAlert 
     
    DECLARE  @Day TINYINT=6      ----How many days old inquiries
    DECLARE  @TotalMatchingCar TINYINT =4  --Maximum recommend car return by query ----changed 5 to 4 on 20-03-2014 by Manish
	...
	DECLARE  @VersionId INT
     --------------------------------------------------------------------------------------------------------------------------   
     -------------This query find out the the customer which has inquired only one dealer or only one  individual car today but not inquired any car in last sevendays
     ----For Dealer
         INSERT INTO @TblCustomerId  (CustomerId)
         (SELECT Distinct UCPI.CustomerID 
           FROM  UsedCarPurchaseInquiries AS UCPI WITH (NOLOCK)
           WHERE CONVERT(DATE,UCPI.RequestDateTime)=CONVERT(DATE,GETDATE()-1)
          -- GROUP BY  UCPI.CustomerID HAVING COUNT(UCPI.Id)=1
            EXCEPT 
          SELECT CustomerId  FROM UsedCarPurchaseInquiries AS UCPI WITH (NOLOCK)
           WHERE CONVERT(DATE,RequestDateTime) BETWEEN CONVERT(DATE,GETDATE()-@Day)  AND CONVERT(DATE,GETDATE()-2)
          )
           EXCEPT 
         ( SELECT UCPI.CustomerID 
     ...
     ----For Individual 
         INSERT INTO @TblCustomerId  (CustomerId)
          ...
          -- GROUP BY  UCPI.CustomerID HAVING COUNT(UCPI.Id)=1
            EXCEPT 
          SELECT CustomerId  FROM ClassifiedRequests AS UCPI WITH (NOLOCK)
           WHERE CONVERT(DATE,RequestDateTime) BETWEEN CONVERT(DATE,GETDATE()-@Day)  AND CONVERT(DATE,GETDATE()-2)
          )
           EXCEPT 
         ( SELECT UCPI.CustomerID 
   ...
   WITH CTE1 AS (
					SELECT  LL.Inquiryid  AS InquiryId,     ---capturing cardetails of the inquiries for dealer car
							...
							CASE WHEN  LL.OriginalImgPath IS NULL OR LL.OriginalImgPath ='' THEN '/cw/static/no-cars.jpg'  ELSE  LL.OriginalImgPath  END AS ImageUrl,
						 ...
						 JOIN  @TblCustomerId AS C  ON C.CustomerId=UCPI.CustomerID
						 ...
						 AND   CONVERT(DATE,UCPI.RequestDateTime)=CONVERT(DATE,GETDATE()-1) 
						 AND   CU.IsFake=0
						UNION ALL
				 SELECT  LL.Inquiryid AS InquiryId,                ---capturing cardetails of the inquiries for individual car
						...
						LL.VersionId   AS VersionId            ----------- Added by Manish on 20-03-2014 
						 ...
						 AND   CONVERT(DATE,UCPI.RequestDateTime)=CONVERT(DATE,GETDATE()-1) 
						 AND CU.IsFake=0
				),CTE2 AS ( SELECT *,ROW_NUMBER() OVER (PARTITION BY BuyerCustomerId ORDER BY PRICE, RequestDateTime DESC) RowNum FROM CTE1
				           )
         ...
         insert into UCAlert.UserCarAlerts(CustomerId,
                                           Email,
                                           CityId,
                                           MakeId,
                                           modelId,
                                           FuelTypeId,
                                           EntryDateTime,
										   AlertFrequency,-- added by manish on 02-01-2013 for automated mail.
                                           isAutomated)
                                    SELECT BuyerCustomerId,
                                           Email,
                                           CityId,
                                           MakeId,
                                           ModelId,
      ...
      BEGIN
			...
			( SELECT 
                ...
                + '&dist=50'
                /*+ '&kms=' +  CONVERT(VARCHAR,(SELECT TOP 1 CarkmId FROM carwale_com.UCAlert.CarKms WHERE LL.Kilometers Between LowerVal and UpperVal))*/ AS alertUrl,
			 ...
			 SELECT    Top  (@TotalMatchingCar)   CustomerId,
					   ...
					   1     ---- added by manish on 23-04-2014 for first mail
				FROM CTE1 ;
	       ...
	       IF (@TotalRecordInserted<@TotalMatchingCar)
			...
			INSERT INTO UCAlert.RecommendUsedCarAlert 
						...
						SELECT 
                                ...
                                + '&dist=50' AS alertUrl,
								GETDATE() AS CreatedOn,
								CASE WHEN  LL.OriginalImgPath IS NULL OR LL.OriginalImgPath ='' THEN '/cw/static/no-cars.jpg'  ELSE  '110x61'+LL.OriginalImgPath END AS ImageUrl,
								@MakeName AS CarMake,
								@ModelName  As CarModel,
		...
		INSERT INTO ScheduledJobExceptions (
						            ...
						            VALUES ('UsedCarRecommendationDailyInsertJob',
									        'UCAlert.RecommendCarAlertForLastDay',
											 ERROR_MESSAGE(),
											 NULL,
											 NULL,
											 GETDATE()
