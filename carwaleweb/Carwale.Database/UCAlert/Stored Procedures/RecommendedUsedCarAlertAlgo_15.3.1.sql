IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[UCAlert].[RecommendedUsedCarAlertAlgo_15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [UCAlert].[RecommendedUsedCarAlertAlgo_15]
GO
...
-- Modified by : Jugal on 24-04-2014, Added Parameter @CarsWithPhotos and added a condtion in Where Clause 
-- Approved/Checked by : Manish on 25-06-2014 03:08 pm
-- Modified by : Kirtan Shetty on 11 November 2014 (Added the CityName parameter in the second Select Query)
-- Condition modified by Supriya Bhide on 18 Feb 2015 to now recommend cars without photos.
...
CREATE PROCEDURE [UCAlert].[RecommendedUsedCarAlertAlgo_15.3.1] 
@Price  INT,
@CarVersionId INT,
@CityId INT,
@NoOfRecommendCar TINYINT,
@CarsWithPhotos bit = 0
AS 
  BEGIN
		 ...
		 ELSE 
		   ...
		   --------Making the link for display car details in profile id field-------------
			  SELECT TOP (@NoOfRecommendCar) LOWER(Replace(LL.MakeName,' ',''))+'-'+CM.MaskingName+'-'+LL.ProfileId AS ProfileId,
											 ...
											 LL.CityName AS CityName, -- Added by Kirtan Shetty on 11 November 2014
			( 
			...
			*@CarPhotoMultiplier)
			 ...
			 WHERE 
			  ...
			  AND LL.PhotoCount > 0 -- Condition modified by Supriya Bhide on 18 Feb 2015 to now recommend cars without photos.
			  --AND LL.PhotoCount > (CASE WHEN @CarsWithPhotos = 1 THEN 0 ELSE -1 END) --Condition Added by Jugal on  24-04-2014--
