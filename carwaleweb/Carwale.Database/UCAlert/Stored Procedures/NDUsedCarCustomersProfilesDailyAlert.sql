IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[UCAlert].[NDUsedCarCustomersProfilesDailyAlert]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [UCAlert].[NDUsedCarCustomersProfilesDailyAlert]
GO
...
-- =============================================
CREATE PROCEDURE [UCAlert].[NDUsedCarCustomersProfilesDailyAlert] 
@UsedCarAlertId int
...
Car_Model, '+
			...
			'FROM UCALERT.NDUsedCarAlertCustomerList as C WITH(NOLOCK) INNER JOIN '+
			' UCAlert.NewlyAddedDailyCars   nc WITH(NOLOCK)  ON   C.CityId=nc.cityid '+
	...
	UPDATE  UCAlert.DailyAlerts SET CustomerAlert_City=@UserCity,
