IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetTopNewLaunches-v16]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetTopNewLaunches-v16]
GO
...
-- Modified by : Piyush sahu on 5/11/2016 created version sp of GetTopNewLaunches

CREATE PROCEDURE [cw].[GetTopNewLaunches-v16.5.4] (
	@cnt TINYINT = 8
	,--default count value
