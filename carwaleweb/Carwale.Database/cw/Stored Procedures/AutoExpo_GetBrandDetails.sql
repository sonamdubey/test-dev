IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[AutoExpo_GetBrandDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[AutoExpo_GetBrandDetails]
GO
...
-- =============================================
CREATE PROCEDURE [cw].[AutoExpo_GetBrandDetails] --exec AutoExpo_GetBrandDetails 18
  ...
  --     ORDER BY Row_No
	...
	SELECT Top(@TopCount) CB.Id AS BasicId
	 ...
	 WHERE Row_No=1
		...
		Inner Join CarMakes CM On CM.Id=@MakeId and CMo.CarMakeId=CM.Id and CM.New=1 and CM.Futuristic=0 and CM.IsDeleted=0 and CMo.New=1 and CMO.IsDeleted = 0
