IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetForumSubCategoryDetails]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetForumSubCategoryDetails]
GO
...
-- =============================================
-- MOdified By:		Avishkar  27/06/2014 Removed ISNULL and Distinct 
-- =============================================
CREATE  PROCEDURE [cw].[GetForumSubCategoryDetails] 
...
BEGIN
	...
	SELECT FST.ID as ID,
		 ...
		 --Avishkar  27/06/2014 Removed ISNULL and Distinct 
		--ISNULL((SELECT DISTINCT(HandleName) FROM UserProfile UP WHERE UP.UserId = C.id),'anonymous')  AS HandleName,
		--ISNULL((SELECT DISTINCT(HandleName) FROM UserProfile UP WHERE UP.UserId = CP.Id),'anonymous')  AS PostHandleName,
