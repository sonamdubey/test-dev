IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetDealerUsedCarProfile_14]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetDealerUsedCarProfile_14]
GO
...
-- =============================================
CREATE PROCEDURE  [cw].[GetDealerUsedCarProfile_14.12.2]
...
BEGIN
	...
	SELECT @InquiryID=SI.ID, @MakeId=LL.MakeId , @ModelId=LL.ModelId , @VersionId=LL.VersionId ,
		...
		@SellerID=Ds.ID ,@ExpiryDate=NULL, @CarCityId=LL.CityId , 
