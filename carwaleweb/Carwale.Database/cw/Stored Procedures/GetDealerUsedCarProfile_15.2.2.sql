...
/*Modified By Reshma Shetty 16/05/2013 @StateCode,@LogoUrl has been assigned NULL and 
  ...
  --Modified By Supriya Bhide on 17 April 2015 for handling Old & New Questions for Car Conditions for Absure
  --Modified By Purohith Guguloth 21.05.2015 Added a new query to get sold car details
-- =============================================
CREATE PROCEDURE  [cw].[GetDealerUsedCarProfile_15.2.2]
...
-- For Getting the Data Related to EMI options
	...
	--WHERE Q.IsActive = 1	-- Modified By Supriya Bhide on 17 April 2015
		...
		WHERE ABCD.CarScore >=60 AND ABCD.IsSurveyDone = 1 --Modified by Aditi Dhaybar on 24/4/2015
...
--Modified By Purohith Guguloth 21.05.2015 Added a new query to get sold car details
	IF @Price IS NULL
		BEGIN
			SELECT @City = CY.Name,@CarCityID = CY.ID,@Make = CM.Name, @RootName = CMR.RootName  
			FROM SellInquiries AS SI WITH(NOLOCK)
			LEFT JOIN Dealers AS D WITH (NOLOCK) ON D.ID = SI.DealerId
			LEFT JOIN Cities AS CY WITH (NOLOCK) ON CY.ID = D.CityId
			...
			INNER JOIN CarMakes AS CM WITH(NOLOCK) ON CM.ID = CMO.CarMakeId
			INNER JOIN CarModelRoots AS CMR WITH(NOLOCK) ON CMR.RootId = CMO.RootId
