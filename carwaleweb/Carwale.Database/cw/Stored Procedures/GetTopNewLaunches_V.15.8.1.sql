IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetTopNewLaunches_V]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetTopNewLaunches_V]
GO
...
-- Modified ,Added ApplicationId=1 (by vikas on 25-7-2015)
--==============================================
CREATE PROCEDURE [cw].[GetTopNewLaunches_V.15.8.1]
...
BEGIN      
 ...
 SET NOCOUNT ON;  
	...
	WITH CTE
		...
		(
		SELECT 
			Distinct  Sp.Id,Ma.Name as Make,Mo.Name as Model,Mo.Id as ModelId,Mo.MaskingName, SP.LaunchDate AS LaunchDate,Mo.LargePic,Mo.SmallPic,MO.HostURL as HostUrl,E.BasicId, CEI.ImageCount, Mo.MinPrice, Mo.MaxPrice,Mo.OriginalImgPath
			...
			LEFT JOIN (SELECT EC.BasicId, EC.ModelId FROM Con_EditCms_Cars EC  WITH (NOLOCK)  INNER JOIN Con_EditCms_Basic EB  WITH (NOLOCK) ON EB.Id = EC.BasicId AND EB.CategoryId = 8 AND EB.IsPublished = 1 AND EB.ApplicationID=1) AS E ON E.ModelId = MO.ID  
			LEFT JOIN (SELECT CEI.ModelId, Count(CEI.Id) AS ImageCount FROM Con_EditCms_Images CEI  WITH (NOLOCK) INNER JOIN Con_EditCms_Basic EB  WITH (NOLOCK) ON EB.Id = CEI.BasicId AND EB.IsPublished = 1 AND EB.CategoryId IN (10,8) AND EB.ApplicationID=1 GROUP By CEI.ModelId) AS CEI ON CEI.ModelId = MO.ID   
