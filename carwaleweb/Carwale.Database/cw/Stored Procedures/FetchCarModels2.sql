IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[FetchCarModels2]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[FetchCarModels2]
GO
...
-- =============================================
CREATE PROCEDURE [cw].[FetchCarModels2]
	@MakeId SMALLINT = 0
AS
BEGIN
...
SELECT
    ...
    (SELECT ID FROM CarVersions WITH (NOLOCK) WHERE CarModelId = Mo.ID AND New = 1 AND IsDeleted = 0) AND AvgPrice > 0)AS MinPrice,
    (SELECT TOP 1 MAX(AvgPrice) FROM Con_NewCarNationalPrices WITH (NOLOCK) WHERE Con_NewCarNationalPrices.VersionId IN
    (SELECT ID FROM CarVersions  WITH (NOLOCK) WHERE CarModelId = Mo.ID AND New = 1 AND IsDeleted = 0) AND AvgPrice > 0)AS MaxPrice
INTO #tempModel
FROM 
	CarModels Mo WITH(NOLOCK)
	INNER JOIN CarMakes Ma WITH (NOLOCK)
