IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetForumPageDetails_v15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetForumPageDetails_v15]
GO
...
-- Modified by: Sanjay on 10-08-2015 
-- =============================================      
CREATE procedure [cw].[GetForumPageDetails_v15.8.1]      -- execute cw.GetForumPageDetails  35995,1,20
...
WITH CTE AS(
		...
		IsNull(UP.RealOriginalImgPath,'') As OriginalImgPath,
		UP.HostURL, 
