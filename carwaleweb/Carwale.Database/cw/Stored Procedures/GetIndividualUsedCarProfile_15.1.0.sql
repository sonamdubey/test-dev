IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[GetIndividualUsedCarProfile_15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[GetIndividualUsedCarProfile_15]
GO
...
-- =============================================
CREATE PROCEDURE [cw].[GetIndividualUsedCarProfile_15.1.0] 
...
--Modified By Purohith Guguloth 21.05.2015 Added a new query to get sold car details
	IF @Price IS NULL
		BEGIN
			...
			FROM CustomerSellInquiries AS CSI WITH(NOLOCK)
			INNER JOIN Cities AS CY WITH(NOLOCK) ON CY.ID = CSI.CityId
