IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[cw].[RecentlyLaunchedCars_V15]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [cw].[RecentlyLaunchedCars_V15]
GO
...
-- Modified by Supriya Khartode on 29/1/2015 to add applicationid=1 filter on Con_EditCms_Basic table.
CREATE PROCEDURE [cw].[RecentlyLaunchedCars_V15.8.1]         
...
BEGIN            
 ...
 SELECT distinct * FROM(        
  SELECT *,ROW_NUMBER()OVER(ORDER BY LaunchDate DESC)Row FROM(       
   ...
   Mo.SmallPic AS ModelSmallImage,     
   MO.HostURL,
   MO.OriginalImgPath,        
   ...
   LEFT JOIN (    
    ...
    FROM Con_EditCms_Cars EC 
	INNER JOIN Con_EditCms_Basic EB 
	ON EB.Id = EC.BasicId AND EB.IsPublished = 1 AND EB.ApplicationID = 1 AND EB.CategoryId = 8 -- roadtests    			
   ) AS E     
   ON E.ModelId = MO.ID         
