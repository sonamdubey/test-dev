IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[CRM].[WeeklyReportNew]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [CRM].[WeeklyReportNew]
GO
...
CREATE PROCEDURE [CRM].[WeeklyReportNew]
 ...
 BEGIN
   ...
   --Gets Count of PQ (Requested, Completed, Not Required, Avg Days)
	SET @varsql =  'SELECT COUNT(CCPL.id) TCount,CCPL.IsPQRequested,CCPL.IsPQCompleted,CCPL.IsPQNotRequired,CL.LeadStageId,CDA.Status, 
		CII.ClosingProbability,DATEDIFF(day, CCPL.PQRequestDate, CCPL.PQCompleteDate) PQAvgDays ,ISNULL(CCBD.BookingStatusId, -1) BookingStatusId
		FROM CRM_CarDealerAssignment CDA WITH(NOLOCK) 
		INNER JOIN CRM_CarBasicData CBD WITH(NOLOCK) ON CDA.CBDId = CBD.ID 
		INNER JOIN CRM_Leads AS CL WITH(NOLOCK) ON CBD.LeadId = CL.ID 
		INNER JOIN vwMMV VW ON CBD.VersionId = VW.VersionId
	...
	SET @varsql += ' GROUP BY CCPL.IsPQRequested,CCPL.IsPQCompleted,CCPL.IsPQNotRequired,CII.ClosingProbability,CCPL.PQRequestDate, CCPL.PQCompleteDate, CL.LeadStageId,CCBD.BookingStatusId ,CDA.Status'
	--PRINT (@varsql)
	EXEC(@varsql)
	
	--Gets Count of TD (Requested, Completed, Not Required, Avg Days,TDDirect)
	SET @varsql = 'SELECT CCTL.id TCount,CII.ClosingProbability, CCTL.IsTDCompleted, CCTL.ISTDNotPossible,CL.LeadStageId,ISNULL(CCBD.BookingStatusId, -1) BookingStatusId,
		CCTL.IsTDRequested, CONVERT(VARCHAR(10), CCTL.TDRequestDate, 111) TDRequestDate,CCTL.IsTDDirect AS IsTDDirect,CDA.Status,
