IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[CRM].[WeeklyReportDetailsNew]') 
    AND xtype IN (N'P')
)
    DROP PROCEDURE [CRM].[WeeklyReportDetailsNew]
GO
...
CREATE PROCEDURE [CRM].[WeeklyReportDetailsNew]
 @Type        SMALLINT,
 @Status      SMALLINT,
 @Eagerness   SMALLINT,
 @SiteId      SMALLINT,
 ...
 BEGIN
    ...
    BEGIN
        ...
        VW1.Make+ + '+@SingleQuotes+' '+@SingleQuotes+' +VW1.Model + + '+@SingleQuotes+' '+@SingleQuotes+' +VW1.Version LostName,
        ...
        ELSE IF(@Status = 1 AND @Lost = '-1' )--Lost
            SET @varsql += ' AND CDA.Status IN (41,60,61)'        
        ELSE IF(@Status = 2 AND @Lost = '-1')--Not Interested
            SET @varsql += ' AND CDA.Status IN (40,42,50)'
        
        IF (@Lost = '1')--Lost And Not interested both
            BEGIN
                SET @varsql += ' AND CDA.Status IN (40,41,42,50,60,61)  '
            END
        ...
        IF(@Status = 2 OR @Lost = '1' )--Not Interested  CDA.CreatedOn AS DealerAssigned,CONVERT(VARCHAR(15),CDA.CreatedOn,106) AS DealerAssigned,
        BEGIN
    ...
    BEGIN
        SET @varsql = 'WITH CP AS ( SELECT CC.Id,CCPL.Id CCPLId,(CC.FirstName + '+@SingleQuotes+' '+@SingleQuotes+' + CC.LastName) CustomerName, 
            CC.Email, CC.Mobile, CC.Landline, C.Name CityName, CDA.CreatedOn AS DealerAssigned, 
            CCPL.PQRequestDate ,CCPL.PQCompleteDate,CCTL.TDRequestDate ,CCTL.TDCompleteDate, ISNULL(CCBD.BookingStatusId, 0) AS BookingStatusId,
            
            ROW_NUMBER() OVER (PARTITION BY CCPL.Id ORDER BY CCPL.PQRequestDate DESC) AS RowNumber,
            
            (VW.Make + '+@SingleQuotes+' '+@SingleQuotes+' + VW.Model + '+@SingleQuotes+' '+@SingleQuotes+' + VW.Version) CarName, 
            ND.ID Did,CL.Id AS LeadId,ND.Name Dealer, CL.CreatedOn,'+@SingleQuotesTwice+' Comments,'+@SingleQuotesTwice+' LostDate,
            '+@SingleQuotesTwice+' LostName,'+@SingleQuotesTwice+' ReasonLost,CBD.ID CBDID,
        ...
        AND CDA.DealerId IN ('+@DealerId+')'
            ...
            IF(@Active = '1')
                SET @varsql += ' AND CL.LeadStageId <> 3 AND CDA.Status NOT IN(40,41,42,50,60,61)'
            
            IF(@Booked='1')
			SET @varsql += ' AND CCBD.BookingStatusId=16 ' 
            ...
            SET @varsql += ' ) SELECT DISTINCT Id,CCPLId, CustomerName, Email,Mobile,Landline,CityName,DealerAssigned,CarName,Did,DealerCode,ReasonLost,
				LeadId,Dealer,CreatedOn,Comments,LostDate,LostName,Pending,Dispositions,SourceName,CarCity,CBDID,
				PQRequestDate,PQCompleteDate,TDRequestDate,TDCompleteDate,ClosingProbability
				FROM CP WHERE RowNumber = 1'
				IF(@Active = '1')
					SET @varsql += ' AND BookingStatusId NOT IN(16,51,10)'
