...
namespace Bikewale.Sitemap.MainSitemap
{
    ...
    internal class SiteMapRepository
    {
        ...
        private readonly string strModelCompare = @"SELECT distinct 6 AS UrlType, m1.MakeMaskingName AS makemasking1, m1.MaskingName as modelmasking1, m2.MakeMaskingName AS makemasking2, m2.MaskingName as modelmasking2 FROM (SELECT DISTINCT a.modelid, a.similarModelId, a.compareOrder FROM similarbikemodels AS a LEFT JOIN similarbikemodels AS b ON a.modelid = b.similarModelId AND a.similarModelId = b.modelid WHERE a.modelid < b.modelid OR b.modelid IS NULL ORDER BY a.modelid , a.compareOrder) AS sb INNER JOIN bikemodels m1 ON sb.modelid = m1.id AND m1.IsDeleted = 0 and m1.new = 1 and m1.futuristic = 0 INNER JOIN bikemodels m2 ON sb.similarModelId = m2.id AND m2.IsDeleted = 0 and m2.new = 1 and m2.futuristic = 0;";
