...
namespace Bikewale.ManufacturerCampaign.DAL
{
	public class ManufacturerCampaignRepository : Interface.IManufacturerCampaignRepository
	{
		...
		/// Modified by :- Subodh Jain 10 july 2017
		/// summary :- Get manufacturer list
		/// Modified by : Prabhu Puredla on 4 july 2018
		/// Description : Removed dapper code
		/// </summary>
		/// <returns></returns>
		public ManufacturerCampaignPopup getManufacturerCampaignPopup(uint campaignId)
		{
			...
			try
			{
				using (DbCommand cmd = DbFactory.GetDBCommand("getmanufacturercampaignpopup_04072018"))
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(DbFactory.GetDbParam("par_campaignId", DbType.Int32, campaignId));
					...
					using (IDataReader dr = MySqlDatabase.SelectQuery(cmd, ConnectionType.ReadOnly))
					{
						if (dr != null && dr.Read())
						{
							...
							{
								PopupHeading = Convert.ToString(dr["PopupHeading"]),
								PopupDescription = Convert.ToString(dr["PopupDescription"]),
								PopupSuccessMessage = Convert.ToString(dr["PopupSuccessMessage"]),
								PinCodeRequired = SqlReaderConvertor.ToBoolean(dr["PincodeRequired"]),
								EmailOption = (EnumEmailOptions)SqlReaderConvertor.ToInt32(dr["EmailOption"]),
								DealerRequired = SqlReaderConvertor.ToBoolean(dr["DealerRequired"])
							};
						}
					}
				}
		...
		/// <summary>
		/// Modified By : Prabhu Puredla on 4 july 2018
		/// Description : Changed par_emailrequired parameter to par_emailOption
		/// </summary>
		/// <param name="objCampaign"></param>
		public void saveManufacturerCampaignPopup(ManufacturerCampaignPopup objCampaign)
		{
			try
			{
				using (DbCommand cmd = DbFactory.GetDBCommand("savemanufacturercampaignpopup_04072018"))
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(DbFactory.GetDbParam("par_campaignid", DbType.String, objCampaign.CampaignId));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_PopupHeading", DbType.String, objCampaign.PopupHeading));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_PopupDescription", DbType.String, objCampaign.PopupDescription));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_PopupSuccessMessage", DbType.String, objCampaign.PopupSuccessMessage));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_EmailOption", DbType.Int32, (uint)objCampaign.EmailOption));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_PincodeRequired", DbType.Int32, objCampaign.PinCodeRequired ? 1 : 0));
					cmd.Parameters.Add(DbFactory.GetDbParam("par_DealerRequired", DbType.Int32, objCampaign.DealerRequired ? 1 : 0));
		...
		public Entities.ManufacturerCampaignEntity GetCampaigns(uint modelId, uint cityId, ManufacturerCampaignServingPages pageId)
		{
			...
			try
			{
				...
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.CommandText = "getmanufacturercampaignbymodelcity_06072018";
					...
					using (IDataReader dr = MySqlDatabase.SelectQuery(cmd, ConnectionType.ReadOnly))
					{
						if (dr != null && dr.Read())
						{
							...
							config.LeadCampaign.EmailRequired = (EnumEmailOptions)Utility.SqlReaderConvertor.ParseToUInt32(dr["EmailRequired"]);
							...
							if (dr.NextResult() && dr.Read())
							{
								...
								config.EMICampaign.EmailRequired = (EnumEmailOptions)Utility.SqlReaderConvertor.ParseToUInt32(dr["EmailRequired"]);
