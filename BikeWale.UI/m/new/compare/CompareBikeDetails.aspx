        ...
        <div id="divBikeHeader" type="divBikeImage" style="border-bottom:1px solid #b3b4c6;position:relative;">
            <div style="position:relative;" class="compareBikeContainer">
                <div class="compareBikeBox">
                    <%--<div id="divImgContainer" class="divImage"><a href="/m/<%=bikeDetails.Rows[0]["MakeMaskingName"] %>-bikes/<%=bikeDetails.Rows[0]["ModelMaskingName"] %>/" style="color:black;text-decoration:none;font-weight:bold;" class="ui-link"><img src="<%= !String.IsNullOrEmpty(bikeDetails.Rows[0]["HostURL"].ToString()) ? Bikewale.Common.ImagingFunctions.GetPathToShowImages("/bikewaleimg/models/" + bikeDetails.Rows[0]["largePic"].ToString(),bikeDetails.Rows[0]["HostURL"].ToString()) :  "https://imgd3.aeplcdn.com/0x0/bikewaleimg/images/noimage.png" %>"></a></div>--%>
                    <div id="divImgContainer" class="divImage"><a href="/m/<%=bikeDetails.Rows[0]["MakeMaskingName"] %>-bikes/<%=bikeDetails.Rows[0]["ModelMaskingName"] %>/" style="color:black;text-decoration:none;font-weight:bold;" class="ui-link"><img src="<%= Bikewale.Utility.Image.GetPathToShowImages(bikeDetails.Rows[0]["OriginalImagePath"].ToString(),bikeDetails.Rows[0]["HostURL"].ToString(),Bikewale.Utility.ImageSize._310x174) %>"></a></div>
                    ...
                    <div class="new-line5 <%=bikeDetails.Rows[0]["Price"].ToString() == "0" ? "hide" : "" %>">Price : Rs. <%=Bikewale.Common.CommonOpn.FormatPrice(bikeDetails.Rows[0]["Price"].ToString()) %></div>
                    <div class="new-line5 <%=bikeDetails.Rows[0]["Price"].ToString() != "0" ? "hide" : "" %>"><a href="/m/pricequote/default.aspx?version=<%=bikeDetails.Rows[0]["BikeVersionId"] %>" class="getquotation" data-modelId="<%=bikeDetails.Rows[0]["Model"]%>" >Check On-Road Price</a></div>
                </div>
            <div style="width:23px;height:24px;position:absolute;top:51.5px;z-index:1;right:-10px;background: url('https://imgd1.aeplcdn.com/0x0/bw/static/design15/old-images/d/icons-sheet.png') no-repeat scroll 0 0 transparent;background-position: 0 -1148px;"></div></div>
                
            <div style="position:relative;" class="compareBikeContainer">
                <div class="compareBikeBox colorborderLt">
                    <%--<div id="divImgContainer1" class="divImage"><a href="/m/<%=bikeDetails.Rows[1]["MakeMaskingName"] %>-bikes/<%=bikeDetails.Rows[1]["ModelMaskingName"] %>/" style="color:black;text-decoration:none;font-weight:bold;" class="ui-link"><img src="<%= !String.IsNullOrEmpty(bikeDetails.Rows[1]["HostURL"].ToString()) ? Bikewale.Common.ImagingFunctions.GetPathToShowImages("/bikewaleimg/models/",bikeDetails.Rows[1]["HostURL"].ToString()) + bikeDetails.Rows[1]["largePic"] :  "https://imgd3.aeplcdn.com/0x0/bw/static/design15/old-images/d/nobike.jpg" %>"></a></div>--%>
