...
upcoming = function () {
        ...
        try {
            ...
            if (pnum && !isNaN(pnum) && !ele.parent().hasClass("active")) {
                ...
                if (selHash) {
                    ...
                    if (curmakeId && curmakeId != "0") self.Filters()["makeId"] = curmakeId;
                }
                self.CurPageNo(pnum);
                self.getUpcomingBikes();
            }
    ...
    self.setMakeFilter = function (e) {
        ...
        self.CurPageNo(1);
        self.getUpcomingBikes($('#make-slideIn-drawer'));
    };   
    ...
    self.setYearFilter = function (e) {
        ...
        self.Filters()["pageNo"] = 1;
        self.CurPageNo(1);
        self.getUpcomingBikes($('#year-slideIn-drawer'));
    };

    self.getUpcomingBikes = function (ele) {
        ...
        if (self.PreviousQS() != qs) {
            $('.item-image-content img').attr('src', '');
            self.IsLoading(true);
            self.models([]);
            self.PreviousQS(qs);
            ...
            .always(function () {
                ...
                $('html, body').scrollTop($('#upcoming-bikes').offset().top);
            });
        }
        else {
            slideInDrawer.close(ele);
    ...
    self.setPageFilters = function (e) {
        ...
        if (currentQs != "") {
            ...
            if (self.Filters()["makeId"] && self.Filters()["makeId"] != "") {
                var selOption = $("#brand-slideIn-drawer ul li[data-makeid='" + self.Filters()["makeId"] + "']");
                self.selectedMake(selOption.data("makename"));
            }

            if (self.Filters()["year"] && self.Filters()["year"] != "") {
                var selOption = $("#year-slideIn-drawer ul li[data-bikeyear='" + self.Filters()["year"] + "']");
