...
<%if(TotalPages > 1 ) { %>
    ...
    <span class="pagination-control-prev <%= String.IsNullOrEmpty(prevPageUrl) ? "inactive" : "" %>">
        <a href="<%=prevPageUrl %>" class="bwmsprite prev-page-icon" data-hash="<% string.Format("city={0}&make={1}&model={2}", CityId, MakeId, ModelId); %>"></a>
    </span>
    <span class="pagination-control-next <%= String.IsNullOrEmpty(nextPageUrl) ? "inactive" : "" %>">
        <a href="<%=nextPageUrl %>" class="bwmsprite next-page-icon" data-hash="<% string.Format("city={0}&make={1}&model={2}", CityId, MakeId,ModelId); %>"></a>
    </span>
</div>
<div class="clear"></div>
<% } %>
