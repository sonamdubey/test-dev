            ...
            <div class="swiper-card">
                <a href="<%=string.Format("/m/used/{0}-{1}-bikes-in-{2}/",MakeMaskingName,ModelDetails.ModelMaskingName,CityMaskingName) %>" title="<%=string.Format("Used {0} {1} bikes in {2}",MakeName,ModelDetails.ModelName,CityName)%>" class="card-target-block">
                    <div class="card-image-placeholder">
                        <img class="swiper-lazy" data-src="<%=Bikewale.Utility.Image.GetPathToShowImages(ModelDetails.OriginalImagePath,ModelDetails.HostUrl,Bikewale.Utility.ImageSize._110x61) %>" src="" alt="<%=string.Format("Used {0} {1} bikes in {2}",MakeName,ModelDetails.ModelName,CityName)%>"/>
