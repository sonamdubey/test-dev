...
<div id="modelPriceInNearbyCities" class="padding-top20 padding-right20 padding-bottom10 padding-left20 font14">
    <h2 class="font14 margin-bottom15"><%= MakeName %> <%= ModelName %> price in cities near <%= CityName %> <span class="text-light-grey text-unbold">(<% if(!IsDiscontinued) { %>On road price<% } else { %>Ex-showroom price<%} %>)</span></h2>
