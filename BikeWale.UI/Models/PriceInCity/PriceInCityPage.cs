...
namespace Bikewale.Models
{
    ...
    public class PriceInCityPage
    {
        ...
        /// Modified By : Deepak Israni on 5 Decemeber 2018
        /// Description : Set hasAreaAvailable through city object from GetPriceQuoteCities.
        /// </summary>
            ...
            try
            {
                if (modelId > 0 && cityId > 0)
                {
                    ...
                    if (cities != null)
                    {
                        ...
                        hasAreaAvailable = selectedCity != null && selectedCity.HasAreas;

                        if (hasAreaAvailable && areaId > 0)
                        {
                            objVM.IsAreaAvailable = selectedCity.HasAreas;
            ...
            try
            {
                if (Status == StatusCodes.ContentFound)
                {
                    ...
                    if (objVM.BikeVersionPrices != null && objVM.BikeVersionPrices.Any())
                    {
                        ...
                        IEnumerable<BikeQuotationEntity> priceAvailableVersions = objVM.BikeVersionPrices.Where(r => r.ExShowroomPrice > 0).ToList();
                        if (priceAvailableVersions != null && priceAvailableVersions.Any())
                        {
                            objVM.BikeVersionPrices = priceAvailableVersions;
                            objVM.VersionPriceAvailable = true;
                        }
                        ...
                        if (objVM.IsNew)
                        {
                            ...
                            {
                                Entities.BikeBooking.v2.PQOutputEntity pQOutput = GetDealerPriceQuote(objVM);
        ...
        /// Modified by : Deepak Israni on 20 Feb 2018
        ...
        public PriceInCityPageAMPVM GetDataAMP()
        {
            ...
            try
            {
                if (Status == StatusCodes.ContentFound)
                {
                    ...
                    IEnumerable<BikeQuotationEntity> objBikePQList = null;
                    ICollection<BikeQuotationAMPEntity> objBikePQAMPList = new List<BikeQuotationAMPEntity>();

                    if (objVM.BikeVersionPrices != null && objVM.BikeVersionPrices.Any())
                    {
                        objBikePQList = objVM.BikeVersionPrices.Where(r => r.ExShowroomPrice > 0).ToList();
                        
                        BikeQuotationAMPEntity objPq = null;

                        if (objBikePQList != null && objBikePQList.Any())
                        {
                            objVM.BikeVersionPrices = objBikePQList;
                            objVM.VersionPriceAvailable = true;
                        }
                        else
                        {
                            objBikePQList = objVM.BikeVersionPrices;
                        }
                    ...
                    if (objVM.FormatedBikeVersionPrices != null && objVM.FormatedBikeVersionPrices.Any())
                    {
                        ...
                        IDictionary<uint, BikeQuotationAMPEntity> values = new Dictionary<uint, BikeQuotationAMPEntity>();
                        uint verId;

                        foreach (var item in objVM.FormatedBikeVersionPrices)
                        {
                            verId = item.BikeQuotationEntity.VersionId;
                            if (!values.ContainsKey(verId))
                            {
                                values.Add(verId, item);
                            }
                        }
                        ...
                        if (objVM.IsNew)
                        {
                            BindPriceInNearestCities(objVM);
                            BindPriceInTopCities(objVM);
                            objVM.IsAreaAvailable = objVM.CookieCityEntity != null && objVM.CookieCityEntity.HasAreas && areaId == 0;
        ...
        private void BindManufacturerLeadAdAMP(PriceInCityPageAMPVM priceInCityAMPVM)
        {
            ...
            if (priceInCityAMPVM.LeadCampaign != null && priceInCityAMPVM.LeadCapture != null)
            {
                ...
                try
                {
                    ...
                    string hash = Bikewale.PWA.Utils.PwaCmsHelper.GetSha256Hash(priceInCityAMPVM.LeadCampaign.LeadsHtmlMobile);
                    str = MvcHelper.Render(hash, priceInCityAMPVM.LeadCampaign, priceInCityAMPVM.LeadCampaign.LeadsHtmlMobile);
        ...
        private void BindBikeBasicDetails(PriceInCityPageVM objVM)
        {
            try
            {
                //CityPriceEntity selectedCity = _objCityCache.GetCityInfoByCityId(cityId);
        ...
        private void BindBikeInfoRank(PriceInCityPageVM objVM)
        {
            try
            {
                ...
                objVM.BikeInfo = (new BikeInfoWidget(_bikeInfo, _objCityCache, modelId, cityId, BikeInfoTabCount, Entities.GenericBikes.BikeInfoTabType.PriceInCity, _objModelEntity, _bikeSeries)).GetData();
        ...
        private Entities.BikeBooking.v2.PQOutputEntity GetDealerPriceQuote(PriceInCityPageVM objVM)
        {
            Bikewale.Entities.BikeBooking.v2.PQOutputEntity objPQOutput = null;
            bool isDealerSubscriptionRequired = (objVM.CookieCityEntity != null && objVM.CookieCityEntity.HasAreas && areaId > 0) || !(objVM.CookieCityEntity != null && objVM.CookieCityEntity.HasAreas);
            try
            {
                ...
                objPQEntity.ManufacturerCampaignPageId = IsMobile ? ManufacturerCampaignServingPages.Mobile_PriceInCity : ManufacturerCampaignServingPages.Desktop_PriceInCity;
                objPQOutput = _objDealerPQ.ProcessPQV2(objPQEntity, isDealerSubscriptionRequired);
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                if (objPQOutput != null && isDealerSubscriptionRequired)
                {
                    if (objPQOutput.IsDealerAvailable && objPQOutput.ManufacturerCampaign == null)
                    {
                        try
                        {
                            ...
                            objVM.MPQString = EncodingDecodingHelper.EncodeTo64(Bikewale.Common.PriceQuoteQueryString.FormQueryString(cityId.ToString(), objPQOutput.PQId, areaId.ToString(), objPQOutput.VersionId.ToString(), objPQOutput.DealerId.ToString()));
                        }
                        catch (Exception ex)
                        {
        ...
        private void GetManufacturerCampaign(PriceInCityPageVM objData, Entities.BikeBooking.v2.PQOutputEntity pQOutput)
        {
            try
            {
                if (_objManufacturerCampaign != null && !(objData.HasCampaignDealer))
                {
                    ...
                    if (pQOutput != null && pQOutput.ManufacturerCampaign != null)
                    {
                        ...
                        if (campaigns.LeadCampaign != null)
                        {
                            ...
                            objData.LeadCampaign = new Bikewale.Entities.manufacturecampaign.v2.ManufactureCampaignLeadEntity()
                            {
                                ...
                                LeadSourceId = (Platform == PQSources.Desktop) ? (int)LeadSourceEnum.PriceInCity_TopCard_Desktop : (Platform == PQSources.Amp ?
                                               (int)LeadSourceEnum.PriceInCity_TopCard_AMP : (int)LeadSourceEnum.PriceInCity_TopCard_Mobile),
                            ...
                            objData.LeadCampaign.PageUrl = string.Format("{0}/m/popup/leadcapture/?q={1}", BWConfiguration.Instance.BwHostUrl, Bikewale.Utility.TripleDES.EncryptTripleDES(string.Format("modelid={0}&cityid={1}&areaid={2}&bikename={3}&location={4}&city={5}&area={6}&ismanufacturer={7}&dealerid={8}&dealername={9}&dealerarea={10}&versionid={11}&leadsourceid={12}&pqsourceid={13}&mfgcampid={14}&pqguid={15}&pageurl={16}&clientip={17}&dealerheading={18}&dealermessage={19}&dealerdescription={20}&pincoderequired={21}&emailrequired={22}&dealersrequired={23}&url={24}&sendLeadSMSCustomer={25}&organizationName={26}&platformid={27}",
                                ...
                                objData.LeadCampaign.SendLeadSMSCustomer, objData.LeadCampaign.Organization, Platform)));
                        }
                        if (campaigns.EMICampaign != null)
                        {
                            objData.EMICampaign = new ManufactureCampaignEMIEntity()
                            {
                                ...
                                LeadSourceId = (Platform == PQSources.Desktop) ? (int)LeadSourceEnum.PriceInCity_EmiCalculator_Desktop : (Platform == PQSources.Amp ?
                                               (int)LeadSourceEnum.PriceInCity_EmiCalculator_AMP : (int)LeadSourceEnum.PriceInCity_EmiCalculator_Mobile),
                                PqSourceId = (int)PQSource,
                            ...
                            objData.EMICampaign.PageUrl = string.Format("{0}/m/popup/leadcapture/?q={1}", BWConfiguration.Instance.BwHostUrl, Bikewale.Utility.TripleDES.EncryptTripleDES(string.Format("modelid={0}&cityid={1}&areaid={2}&bikename={3}&location={4}&city={5}&area={6}&ismanufacturer={7}&dealerid={8}&dealername={9}&dealerarea={10}&versionid={11}&leadsourceid={12}&pqsourceid={13}&mfgcampid={14}&pqguid={15}&pageurl={16}&clientip={17}&dealerheading={18}&dealermessage={19}&dealerdescription={20}&pincoderequired={21}&emailrequired={22}&dealersrequired={23}&url={24}&sendLeadSMSCustomer={25}&organizationName={26}&platformid={27}",
                                objData.BikeModel.ModelId, objData.CityEntity.CityId, string.Empty, string.Format(objData.BikeName), string.Empty, string.Empty, string.Empty,
                                ...
                                objData.EMICampaign.SendLeadSMSCustomer, objData.EMICampaign.Organization, Platform)));
        ...
        private void BindMoreAboutScootersWidget(PriceInCityPageVM objData)
        {
            try
            {
                MoreAboutScootersWidget obj = new MoreAboutScootersWidget(_modelCache, _objCityCache, _version, _bikeInfo, Entities.GenericBikes.BikeInfoTabType.PriceInCity, _objModelEntity, _bikeSeries);
                obj.modelId = modelId;
