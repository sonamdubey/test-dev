...
docReady(function () {
    ...
    $.getVideos = function () {        
        $('#loading').show();
        var cacheVideos = bwcache.get("catVideo_" + cacheKey + "_" + pageNo);
        if (cacheVideos) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.videos && response.videos.length > 0) {
                        ...
                        isNextPage = true;
                        bwcache.set("catVideo_" + cacheKey + "_" + pageNo, objVideos, 60);
                        window.location.hash = "pn=" + pageNo;                        
                    }
                },
                complete: function (xhr) {
                    if (xhr.status != 200) {
                        isNextPage = false;
                        bwcache.set("catVideo_" + cacheKey + "_" + pageNo, null, 60);
                    }
                    $('#loading').hide();
                }
            });
