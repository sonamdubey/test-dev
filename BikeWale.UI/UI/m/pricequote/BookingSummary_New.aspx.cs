...
namespace Bikewale.Mobile.PriceQuote
{
    ...
    public class BookingSummary_New : System.Web.UI.Page
    {
		...
		protected string clientIP = String.Empty, pageUrl = String.Empty, bikeName = String.Empty, location = String.Empty, jsonBikeVarients = String.Empty, pqId = String.Empty;
        ...
        protected void BeginTransaction(string sourceType)
        {
            ...
            if (objCustomer != null && objCustomer.objCustomerBase != null && objCustomer.objCustomerBase.CustomerId > 0)
            {
                ...
                {
                    HttpContext.Current.Response.Redirect(String.Format("{0}/m/pricequote/bookingsummary_new.aspx?MPQ={1}", BWConfiguration.Instance.BwHostUrlForJs, EncodingDecodingHelper.EncodeTo64(PriceQuoteQueryString.QueryString)), false);
                    HttpContext.Current.ApplicationInstance.CompleteRequest();
                    this.Page.Visible = false;
                }
            }
