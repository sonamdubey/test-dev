...
namespace Bikewale.Used
{
	...
	{			
		...
		public bool IsBuyerEligible(string buyerMobile)
		{
			...
			try
			{
				cmd = new SqlCommand("Classified_RestricBuyer_SP", con);
				cmd.CommandType = CommandType.StoredProcedure;
			
				prm = cmd.Parameters.Add("@RequestDate", SqlDbType.Date);
				prm.Value = DateTime.Today;

                prm = cmd.Parameters.Add("@Mobile", SqlDbType.Char, 10);
                prm.Value = buyerMobile;
