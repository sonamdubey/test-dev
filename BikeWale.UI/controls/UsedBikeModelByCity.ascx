    ...
    <div class="jcarousel">
        <ul>
            <%foreach (var ModelDetails in UsedBikeInCityList)
              {%>
            <li>
                <a href="<%=string.Format("/used/{0}-{1}-bikes-in-{2}/",ModelDetails.MakeMaskingName,ModelDetails.ModelMaskingName,CityMaskingName) %>" title="<%=string.Format("Used {0} {1] bikes in {2}",ModelDetails.MakeMaskingName,ModelDetails.ModelName,CityName)%>" class="card-target-block">
