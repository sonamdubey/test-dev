...
class ImageCard extends React.Component {
	...
	render() {
	    ...
	    var imageResolution = this.props.imageResolution?this.props.imageResolution:'174x98';
	    return (
                    ...
                    {imagesList.map(function(bikeImage) { 
                        return (
                            <li className="image-grid-list__item">
						        <LazyLoad>
							        <img className="swiper-lazy" src={createImageUrl(bikeImage.HostUrl, bikeImage.OriginalImgPath, imageResolution)} alt={title} title={title} />
						        </LazyLoad>
					        </li>);
                    })}
				</ul>
