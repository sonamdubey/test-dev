...
function Button(props) {
	...
	return (
		<button {...props} type="button">
			{
				showTooltip
				? <span className="rheostat-handle-tooltip">{props.children}</span>
				: ''
			}
		</button>
	)
...
const propTypes = {
	...
	// a custom handle tooltip
	handleTooltip: PropTypes.bool,
	// a custom method to format tooltip value
	...
	// a custom function to format pit points
	pitPointLabel: PropTypes.func,
	...
	// snap handle on drag move
	snapOnDragMove: PropTypes.bool,
	// disable snap on click
	disableSnapOnClick: PropTypes.bool,
	// the values
...
class Rheostat extends React.Component {
	...
	handleClick(ev) {
		...
		if (positionPercent === this.state.handlePos[handleId]) {
			return
		}

		if (this.props.disableSnapOnClick) {
			// Move the handle there
			this.slideTo(handleId, positionPercent, () => this.fireChangeEvent())
		}
		else {
			const validPositionPercent = this.getSnapPosition(positionPercent)
	...
	getTooltipLabel(value) {
		const { handleTooltipLabel } = this.props

		if (handleTooltipLabel) {
			return handleTooltipLabel(value)
		}
		else {
			return value
		}
	...
	getPitPointsLabel(value) {
		const { pitPointLabel } = this.props

		if (pitPointLabel) {
			return pitPointLabel(value)
		}
		else {
			return value
		}
			...
			>
				...
				{handlePos.map((pos, idx) => {
					...
					return (
						...
						>
							{handleTooltip ? this.getTooltipLabel(values[idx]) : ''}
						</Handle>
					)
				})}
